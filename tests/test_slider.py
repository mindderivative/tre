...
add_slider`/the new `"thumb_position"` arm of `Node.animate`/`Node.get`
...
slider` returns a real, usable `Node`, and `"thumb_position"` reaches
...
def test_add_slider_defaults_to_zero():
    ...
    assert slider.get("thumb_position") == 0.0
...
def test_add_slider_accepts_a_custom_initial_value():
    ...
    assert slider.get("thumb_position") == 0.75
...
def test_add_slider_clamps_an_out_of_range_initial_value():
    ...
    assert slider.get("thumb_position") == 1.0


def test_animate_thumb_position_reaches_the_real_slider():
    window = Window(width=200, height=40)
    slider = window.add_slider(background=(0x03, 0xDA, 0xC6, 0xFF), width=180, height=32)
    slider.animate("thumb_position", 0.6, duration_ms=0)  # must not raise
...
def test_thumb_position_property_is_unknown_on_a_non_slider_node():
    ...
    with pytest.raises(ValueError, match="Rect has no property 'thumb_position'"):
        rect.animate("thumb_position", 0.5)
    with pytest.raises(ValueError, match="Rect has no property 'thumb_position'"):
        rect.get("thumb_position")
