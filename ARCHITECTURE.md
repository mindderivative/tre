...
>
> **Sharpened by the §1/§5 node-model decision:** the dispatch is now concretely two-level — try `property` against `PaintProperties`' own field names first (universal, always present), then against the node's `NodeKind` payload's field names if it has one (e.g. `"check_progress"` only resolves on a `Checkbox`). A property name that matches neither should name the node's actual kind in the error ("Slider has no property 'check_progress'"), not just say "unknown property."
>
