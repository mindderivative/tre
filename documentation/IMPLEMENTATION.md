...
**A real, two-process architecture, not one self-verifying binary** (see REVIEW.md finding #126 for the full eight-push investigation). `canvas_accessibility_demo` renders and tags only, then hands its tagged `AccessibilityNode`s to a separate `canvas_accessibility_verify` binary -- confirmed via `ldd` to link zero Vulkan/X11/Wayland libraries -- via a small plain-text handoff file. `canvas_accessibility_verify` performs the real `A11yBridge::connect`/`publish` and the real AT-SPI2 verification, mirroring `tre-a11y`'s own already-reliable round-trip test exactly, just with externally-supplied, real-render-derived data instead of one synthetic literal. This split was forced by a real, confirmed root cause: `accesskit_unix`'s own background thread, which performs the real AT-SPI2 registration, cannot complete that registration inside a process that also links real Vulkan/X11 shared libraries, regardless of timeout length -- proven only after ordering, `xvfb-run` wrapper nesting, job-level CPU contention, `org.a11y.Status.IsEnabled` forcing, and a widened 60s timeout were each tried in turn and directly disproved by real evidence (the decisive proof: in one single CI run, `tre-a11y`'s own test succeeded at 30.06s while the demo's own next step, same job, timed out completely at a full 60s). The split also happens to match real AT-SPI2 practice more closely than the original design: a real screen reader is always a separate process from the application it inspects.
