...
### Step 5.3.3: The Capstone -- A Real Rendered Scene, Verified Live -- Status: Complete (2026-09-08), closing Step 5.3 in full

Third and closing sub-step of Step 5.3. A new demo, `canvas_accessibility_demo`, draws and tags 3 rects from the exact same local coordinates -- a plain `Generic` rect, a plain `Button` rect, and a **rotated** `Image` rect via `Affine2::from_translation_rotation_scale` -- then checks three independent systems against one shared source of truth in a single render: the real GPU framebuffer (read back and confirmed non-background at each rect's own real transformed center, including the rotated one), the tagged `AccessibilityNode` IR, and a real, live AT-SPI2 registry (queried by this same process acting as its own second D-Bus client, matching Step 5.3.2's own round-trip pattern). Neither prior sub-step could prove this alone: 5.3.1 never touched a real bus, and 5.3.2's own round-trip test used a synthetic, never-rendered node.
...
Verified by `cargo fmt`/`clippy -D warnings`/`build`/`test` clean across the workspace, plus the demo's own successful run confirmed 3 times in a row against this development machine's real, live AT-SPI2 stack: every rect's real `Component.GetExtents` matches its own real IR bounds exactly (including the rotated rect's real axis-aligned bounding box, not a naively-guessed one), and all 3 rects' distinct `AccessibilityRole`s survive as 3 pairwise-distinct real AT-SPI2 roles. `demo/phase5_step5_3_3/` (README, run script, output PNG) added, matching every prior demo-bearing step. All pre-existing examples re-run manually as the lighter "did adding a new dev-dependency break an unrelated build" check (this is the first time `tre-rhi-vulkan` depends on `tre-a11y`), zero regressions. **Checking the actual GitHub-hosted CI run after pushing this step surfaced a real, environment-specific failure that took seven real pushes to fully resolve** (REVIEW.md #126, full account there). In order, each a real hypothesis directly falsified by real evidence rather than assumed: `org.a11y.Status.IsEnabled` forcing (no measurable effect); CPU contention from 20 prior demos in `vulkan-validation` (an isolated, uncontended job still failed identically); `xvfb-run`'s own wrapper script nested inside `dbus-run-session` (removing it made no difference); call ordering (reordering the demo so accessibility ran before any Vulkan/window code made no difference either). **The actual, confirmed cause:** running `tre-a11y`'s own already-reliable round-trip test -- no Vulkan/X11 code of its own at all -- inside the same job with Xvfb/Vulkan packages installed and `DISPLAY` set measured 30.06s, triple its usual ~10.05s elsewhere, proving the real AT-SPI2 discovery delay simply is longer in that specific environment, regardless of which binary runs or what it does first. Both discovery timeouts widened from 30s to 60s accordingly, real margin over the measured worst case. The demo also keeps its own new, isolated `accessibility-validation` CI job and its accessibility-before-rendering ordering -- both real, independent improvements, just not what actually fixed this. **This closes Step 5.3 (5.3.1-5.3.3) in full, and proves DESIGN.md Section 5.2's "100% alignment" claim end to end for Linux.**
