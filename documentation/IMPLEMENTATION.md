...
**Real, disclosed remaining scope**: this sandbox has no input-injection or window-management tool (no `xdotool`/`wmctrl` -- confirmed absent, the identical gap Phase 19 Step 19.4 already disclosed for an unrelated reason) to synthesize a real window resize unattended, so the resize test's own `InputEvent::Resized` -> swapchain-rebuild -> `resize_event` logging path could not be exercised end-to-end in this environment -- only the continuous-render/telemetry half was verified directly. The code path itself is not new or unproven in isolation: it's a direct mirror of `windowed_renderer.rs`'s own already-real-GPU-verified resize-recovery sequence (confirmed working via 120 real frames and an observed real window resize in this session's own prior Python `windowed_renderer` demo run). A human physically resizing the window during a real run is needed to close this gap, matching Phase 17/19's own established "a human still needs to look at some real UI results" disclosure discipline.

## Phase 20 Summary

A manually triggered, never-CI-gated performance test suite now exists: a real windowed `ramp_test` (geometric primitive-count scaling across three real workload kinds, FPS/CPU/memory/GPU telemetry to console and a structured JSON-Lines log) and a real windowed `resize_test` (a fixed scene under live, human-driven window resizing, with a dedicated swapchain-rebuild-latency record). One real, load-bearing bug was found and fixed via the first actual run against real hardware (an undersized ring buffer, not a genuine rendering bottleneck), and one real scope substitution was disclosed rather than silently made (gradient fill instead of MSDF text for the "shaders" workload, since no Rust-side precedent for the latter exists anywhere in the workspace). The resize test's own interactive path is verified only up to the limits of what an unattended sandbox with no window-management tooling can exercise -- a real, disclosed gap, not an overclaimed one.
