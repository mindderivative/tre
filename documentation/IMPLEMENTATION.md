...
Verified by `cargo fmt`/`clippy -D warnings`/`build`/`test` clean across the workspace, plus the demo's own successful run confirmed 3 times in a row against this development machine's real, live AT-SPI2 stack: every rect's real `Component.GetExtents` matches its own real IR bounds exactly (including the rotated rect's real axis-aligned bounding box, not a naively-guessed one), and all 3 rects' distinct `AccessibilityRole`s survive as 3 pairwise-distinct real AT-SPI2 roles. `demo/phase5_step5_3_3/` (README, run script, output PNG) added, matching every prior demo-bearing step. `vulkan-validation`'s CI job gained `dbus-user-session`/`at-spi2-core`, with only this one demo's own run line wrapped in `dbus-run-session -- xvfb-run -a ...` -- every other line in that job untouched. All pre-existing examples re-run manually as the lighter "did adding a new dev-dependency break an unrelated build" check (this is the first time `tre-rhi-vulkan` depends on `tre-a11y`), zero regressions. **Checking the actual GitHub-hosted `vulkan-validation` run after pushing this step surfaced one real, environment-specific failure** (REVIEW.md #126): the demo missed the same `IsEnabled`-false race the `test` job's own round-trip test nearly missed too, since this job runs it later, under more system load. Fixed the same way in both jobs -- `dbus-send` explicitly sets `IsEnabled=true` before the relevant command runs. **This closes Step 5.3 (5.3.1-5.3.3) in full, and proves DESIGN.md Section 5.2's "100% alignment" claim end to end for Linux.**
