...
All three sections of GUI-readiness recommendations 9/12 are complete: real multi-line/word-wrap text rendering (Step 15.1, `Text.wrap_width`), multi-line editing (Step 15.2, `EditableText` vertical caret movement + `selection_rects`), and Shift+arrow selection extension (Step 15.3, real horizontal movement + extend semantics for all four directions). Along the way, this phase found and fixed four real, previously-latent bugs -- a signed-metric sign error, a cross-paragraph glyph-cluster offset bug in `tre_text::shape::shape_run` predating this phase entirely, a redundant-and-wrong pixel-to-line rounding indirection, and a line-boundary tie-break inconsistency -- each diagnosed via real, concrete repros (a debug print against a real font, a stray 6px ink sliver in a real render, a `down/down/up/up` sequence landing short) rather than assumption, and each disclosed in the relevant step's own README/IMPLEMENTATION.md entry rather than silently patched over.
