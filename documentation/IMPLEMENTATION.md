...
Verified by `cargo fmt`/`clippy -D warnings`/`build`/`test` clean across the workspace, plus the demo's own successful run confirmed 3 times in a row against this development machine's real, live AT-SPI2 stack: every rect's real `Component.GetExtents` matches its own real IR bounds exactly (including the rotated rect's real axis-aligned bounding box, not a naively-guessed one), and all 3 rects' distinct `AccessibilityRole`s survive as 3 pairwise-distinct real AT-SPI2 roles. `demo/phase5_step5_3_3/` (README, run script, output PNG) added, matching every prior demo-bearing step. All pre-existing examples re-run manually as the lighter "did adding a new dev-dependency break an unrelated build" check (this is the first time `tre-rhi-vulkan` depends on `tre-a11y`), zero regressions. **Checking the actual GitHub-hosted CI run after pushing this step surfaced a real, environment-specific failure that took three real pushes to fully resolve** (REVIEW.md #126, full account there): the demo, originally the last of 21 commands in the CPU-heavy `vulkan-validation` job, never completed AT-SPI2 discovery there -- not even at a widened 30s timeout, a complete timeout rather than a near-miss -- while the lightweight `test` job's own round-trip test succeeded reliably both times at ~10.05s. An `IsEnabled`-forcing fix was tried and directly disproved first. The real fix: the demo now runs in its own new, minimal `accessibility-validation` job, mirroring `test`'s own proven-reliable "nothing else competing for the runner's CPU" shape, rather than sharing a job with 20 other CPU-bound software-rendered demos. **This closes Step 5.3 (5.3.1-5.3.3) in full, and proves DESIGN.md Section 5.2's "100% alignment" claim end to end for Linux.**
