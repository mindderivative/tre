...
All four sections of GUI-readiness recommendations 9/12/13 are complete: real multi-line/word-wrap text rendering (Step 15.1, `Text.wrap_width`), multi-line editing (Step 15.2, `EditableText` vertical caret movement + `selection_rects`), Shift+arrow selection extension (Step 15.3, real horizontal movement + extend semantics for all four directions), and word/line caret jumps (Step 15.4, real UAX #29 word boundaries + Home/End). Along the way, this phase found and fixed five real, previously-latent bugs -- a signed-metric sign error, a cross-paragraph glyph-cluster offset bug in `tre_text::shape::shape_run` predating this phase entirely, a redundant-and-wrong pixel-to-line rounding indirection, a line-boundary tie-break inconsistency, and a real tie between a line's own trailing-whitespace-inclusive end and the next line's start -- each diagnosed via real, concrete repros (a debug print against a real font, a stray 6px ink sliver in a real render, a `down/down/up/up` sequence landing short, a hypothetical End-then-Down misfire caught before shipping) rather than assumption, and each disclosed in the relevant step's own README/IMPLEMENTATION.md entry rather than silently patched over. Recommendation 13's other half, advanced UAX #14 line-breaking, remains open.
