...
* **Implementation status (Phase 10 Step 10.3, 2026-09-12): a real, working, deliberately bounded first slice.** By 2026-09-12 the engine's public surface had grown far beyond what existed when this step was drafted (2026-09-09) -- focus, Rust-side accessibility tagging, `tre-tween`/`tre-animation`, SVG, tray/clipboard/file dialogs, MSDF text all postdate it. Exposing all of it in one pass would repeat the exact mistake Step 10.4's own real Python binding explicitly avoided, so this pass ships the identical bounded slice Step 10.4 shipped first: headless rendering plus `Rectangle`/`Circle`/`Polygon`/`Path`, **solid fill only** -- no `Text`, no gradients/textures, no windowed rendering yet. Gradients/textures/`Text`/windowing are real, disclosed future work (Steps 10.3.1, 10.3.2, ...), the same numbering pattern Step 10.2 already used for its own six follow-up steps.

  Built: `TreErrorCode` (`src/error.rs`, a `#[repr(C)]` shadow of `EngineError` plus two FFI-only additions -- `InvalidArgument` for this crate's own insert-time validation, `PanicCaught` for what `catch_unwind` converts an unwinding panic into); `TreShapeRegistry`/`TreShapeId` (`src/registry.rs`, boxed opaque handles over `tre_engine::ShapeRegistry`/`ShapeId`, plus `TrePathCommand`, the one `#[repr(C)]` tagged-union shadow this slice needs for `Path`); `TreHeadlessRenderer` (`src/renderer.rs`, a real headless renderer built directly against `tre-rhi-vulkan`, no PyO3 involved); `TreFrameBuffer` (`src/frame_buffer.rs`, the pointer+length+capacity shadow of the raw BGRA8 `Vec<u8>` a render call hands back); a shared `ffi_guard` helper (`src/lib.rs`) every exported function routes through; `cbindgen`-generated `include/tre_ffi.h` (`build.rs`/`cbindgen.toml`); and the real C test harness (`tests/c/harness.c` + `tests/c_harness.rs`).

  Task-by-task account against the four tasks above:

  1. **Done, for this step's own bounded scope.** Every opaque handle is a boxed value behind a `#[repr(transparent)]` pointer wrapper, matching `ShapeId`'s own "opaque handle plus getter/setter functions, never raw struct-layout access across the boundary" precedent literally -- `TreShapeId` is deliberately boxed even though the real `ShapeId` is a tiny `Copy` value, since its fields are private in `tre-engine` with no public accessor to mirror honestly otherwise.
  2. **Done.** `ffi_guard<T>(default: T, f: impl FnOnce() -> T + UnwindSafe) -> T` wraps every exported function's body in `std::panic::catch_unwind`, generic over both `TreErrorCode`-returning fallible calls and infallible-shaped calls (`tre_shape_registry_new`, `tre_rgba8`) that still get a safe default (a null handle) on the vanishingly unlikely case of an internal panic.
  3. **Done -- already scaffolded from day one** (`[lib] crate-type = ["cdylib", "staticlib", "rlib"]`), confirmed here by actually building both: `libtre_ffi.so`/`libtre_ffi.a` both exist after `cargo build -p tre-ffi`, in both debug and `--release` profiles.
  4. **Done.** `tests/c/harness.c` builds a registry, inserts a red `Rectangle`, rejects an invalid (negative-width) insert with `TRE_ERROR_CODE_INVALID_ARGUMENT`, renders headlessly, and asserts *exact* pixel bytes (not just "didn't crash") at the rectangle's own center and at a background pixel outside it. `tests/c_harness.rs` compiles and links it against the crate's own just-built artifact and runs it, satisfying TECHNICAL.md Section 9.4.1's literal "`cargo test`'s FFI test target" wording rather than a separate, CI-only shell step.

  **Real defects/findings from actually building and running this** (not merely compiling it):

  - The `cc` crate's `Build`/`get_compiler()` API -- the plan's own original choice for compiling `tests/c/harness.c` -- cannot be used outside a `build.rs`: it reads Cargo-injected environment variables (`OPT_LEVEL`, `TARGET`, `HOST`, ...) that exist only inside a build-script invocation, not a plain `cargo test` binary. Found by actually running it and hitting `error occurred in cc-rs: environment variable OPT_LEVEL not defined`, not assumed from the crate's docs. Fixed by invoking the system C compiler directly via `std::process::Command` (respecting `$CC`), and dropping the `cc` dev-dependency this crate never ends up needing.
  - Linking `tests/c/harness.c` directly against `libtre_ffi.a` (the `staticlib`) failed with several dozen undefined `g_*`/`gdk_*`/`gtk_*` symbol references from `tre-platform`'s real GTK/GLib/GDK/GIO dependencies (`tray-icon`/`rfd`) -- those system libraries are only linked in via `cargo:rustc-link-lib` directives Cargo itself feeds to `rustc` when producing a *final* linked artifact (a `cdylib`, or a binary), not when a foreign `cc` invocation links the raw `.a` archive directly. Fixed by linking the harness against `libtre_ffi.so` (the `cdylib`) instead, which is already a fully self-contained shared object -- both artifacts are still asserted to exist in the test (Task 3's own requirement), only the `cdylib` is functionally exercised.

  **Verification.** `cargo fmt --all -- --check` / `cargo clippy --workspace --all-targets -- -D warnings` / `cargo build --workspace --all-targets` / `cargo test --workspace` all clean, in both debug and `--release` profiles -- `cargo test`'s own real C-harness test (`tre-ffi`'s `c_harness` integration test) passes in both, including the exact-pixel-byte assertions. `--release` reproduces exactly the same 5 pre-existing, already-disclosed `debug_assert!`-only failures (2 in `tre-engine`, 3 in `tre-memory`, both release-mode-only since `debug_assert!` compiles out) this document already discloses elsewhere -- no new regression. The C harness was also compiled and run once fully outside `cargo test` (a direct `cc` invocation against `target/release/libtre_ffi.so`) as an independent sanity check that passing wasn't an artifact of the test harness's own path-discovery logic.
...
## Phase 20: Rendering Engine Performance & Optimization Test Suite (Added 2026-09-13)

The project owner handed over a written specification for a manually triggered performance/optimization test suite: profile the engine under simulated production load (real native window, `--release` build), collect FPS/CPU/GPU/memory telemetry to both live console output and a structured log file, and run two kinds of test -- a 30-second automated geometric load-ramp per rendering feature, and an interaction-driven native-window-resize stress test. Nothing like this existed before this phase: Phase 9's own testing strategy covers correctness tests and the zero-allocation/balance-assertion CI gates, and TECHNICAL.md Section 9.2's `criterion` benchmark is a single headless, CI-gated microbenchmark -- neither is a manually run, windowed, multi-metric stress-test tool.

Three real design decisions were confirmed with the project owner via `AskUserQuestion` before implementing (full rationale in `PLAN.md`'s own archived copy of this phase's plan): a new dedicated crate rather than another `tre-rhi-vulkan` example (this tool needs real CLI argument parsing, which no existing example does); GPU utilization via Linux/AMDGPU sysfs `gpu_busy_percent` rather than inventing Vulkan timestamp-query infrastructure (zero new dependencies, matches this project's own real dev GPU and its established precedent for disclosed platform-specific implementations -- DirectX 12/Metal are "unimplemented, deferred" for the identical reason); and covering all three workload kinds (shapes/textures/shaders) in this first pass rather than a narrower slice.

### Step 20.1: `tre-perf-suite` Crate -- Status: Complete (2026-09-13)

**Real, working, and verified end to end.** New binary-only crate `crates/tre-perf-suite` (added to the workspace `members` list), depending only on `tre-engine`/`tre-platform`/`tre-rhi-vulkan`/`raw-window-handle`/`bytemuck` -- all already used identically by the ~19 existing `tre-rhi-vulkan` examples, no genuinely new dependency. CLI argument parsing is hand-rolled (`std::env::args()`) rather than adding `clap`: the flag surface (`--profile`/`--workload`/`--duration`/`--tier-seconds`/`--out`) is small enough that a new dependency wasn't warranted, matching this workspace's own "justify every dependency explicitly in writing" standard. Telemetry log records are hand-formatted JSON Lines for the identical reason -- the record shape is flat and fully controlled, so no `serde`/`serde_json` was needed either.

**Window/device/swapchain/pipeline construction** (`ramp_test.rs`, `resize_test.rs`) mirrors `crates/tre-rhi-vulkan/examples/main_loop_demo.rs`'s and `crates/tre-python/src/windowed_renderer.rs`'s own identical sequence (probe window -> `VulkanDevice::new` -> real window + `VulkanSwapchain` -> `register_shape_pipelines`), written fresh rather than factored into a shared helper -- matching every one of that sequence's ~20 other independent call sites across the workspace. Both profiles route through `RenderingCanvas::flatten_into` directly (REVIEW.md finding #224's own non-consuming, zero-copy sibling to `flatten()`) rather than `FrameArena`/`stitch_into` -- this tool has exactly one canvas per frame, the single-canvas case that capability exists for.

**Telemetry (`telemetry.rs`)**: FPS/frame time from `tre_engine::FrameClock` (already proven elsewhere); process CPU% and memory RSS parsed directly from `/proc/self/stat` (`utime`+`stime`, past the `comm` field's own closing `)` since `comm` can itself contain spaces per `proc(5)`) and `/proc/self/status`'s `VmRSS:` line -- no new dependency; GPU utilization from the Linux `amdgpu` driver's own `/sys/class/drm/card*/device/gpu_busy_percent` sysfs file, reporting `None`/"unavailable" (never a fabricated number) when no such file exists. `sysconf(_SC_CLK_TCK)`'s value is hardcoded to its near-universal Linux value of 100 rather than adding a `libc` dependency to call `sysconf` for a number that's 100 in practice everywhere this tool runs -- disclosed in the code, not silently assumed.

**Workloads (`workload.rs`), one disclosed substitution found during implementation**: `shapes` (solid-fill `Rectangle`s) and `textures` (the same geometry with `FillStyle::Texture`) mirror `shape_registry_zero_alloc_demo.rs`/`texture_fill_demo.rs` exactly. `shaders` was originally planned as MSDF `Text` rendering (the engine's own most fragment-shader-costly pipeline) -- investigating the real implementation found this would need `TextFlattenContext`/`FontRegistry`/glyph-atlas plumbing that **no existing Rust example exercises at all** (every real Rust `Text`-via-`ShapeRegistry` precedent lives only on the Python side), real new scope disproportionate to this tool. Substituted with `FillStyle::Gradient` instead -- real, meaningfully more per-pixel shader work than a solid fill, and fully proven already in `gradient_fill_demo.rs` with zero new plumbing needed.

**Test Profile 1 (`ramp_test.rs`)**: 30 seconds total (configurable via `--duration`) per selected workload, doubling the primitive count every `--tier-seconds` (default 2s: `1 -> 131,072` across 18 tiers). **A real bug found and fixed via the first actual run, not assumed up front**: the initial 16MiB ring-buffer capacity looked generous on paper but panicked at a 65,536-primitive tier -- a real limitation of this tool's own buffer sizing (accounting for the ring buffer's multi-frame-in-flight headroom, not just one frame's raw vertex/index bytes), not a genuine engine breaking point the spec's own "identify the exact breaking points" goal cares about. Raised to 64MiB and the tier ceiling lowered from an untested 1,048,576 to a verified-safe 131,072; re-running confirmed no crash all the way to that ceiling, with real, rising CPU% (from ~0% at low tiers to a sustained ~58-62%, briefly exceeding 100% i.e. more than one core, at 131,072 primitives) visible in the telemetry.

**Test Profile 2 (`resize_test.rs`)**: a fixed 500-rectangle scene renders continuously while `InputEvent::Resized` triggers a full swapchain-plus-pipeline reconstruction (a new `Surface` struct, mirroring `windowed_renderer.rs`'s own `WindowSlot::create`-on-`SwapchainOutOfDate` sequence -- the workspace's only existing precedent for rebuilding a live swapchain), timed and logged as a dedicated `resize_event` record distinct from the regular per-frame samples. Also catches `EngineError::SwapchainOutOfDate` from `submit_frame` as a reactive fallback, matching `windowed_renderer.rs`'s own belt-and-suspenders handling of the identical race.

**Verified:** `cargo fmt --all -- --check`/`cargo clippy --workspace --all-targets -- -D warnings`/`cargo build --workspace --all-targets`/`cargo test --workspace` all clean in debug and `--release` (unchanged pass/fail counts everywhere -- this phase touched no existing crate's code). Real, hands-on runs against actual GPU hardware: all three workloads (`shapes`/`textures`/`shaders`) ramped successfully with live console output and a real, line-by-line-valid-JSON `.jsonl` log confirmed via `python3 -m json.loads` per line; a full 40-second `shapes` run reached the 131,072-primitive ceiling without crashing post-fix, showing real CPU-cost growth in the telemetry as primitive count increased (FPS itself stayed largely vsync-capped around 58-62 at this scale -- this development machine's GPU is not yet the bottleneck even at 131,072 simple rectangles, a real, honest finding in its own right, not a tool defect). The resize test was confirmed to open a real window, render continuously, and produce continuous telemetry samples without error over a bounded run.

**Real, disclosed remaining scope, closed the same day**: this sandbox has no input-injection or window-management tool (no `xdotool`/`wmctrl` -- confirmed absent, the identical gap Phase 19 Step 19.4 already disclosed for an unrelated reason) to synthesize a real window resize unattended, so the resize test's own `InputEvent::Resized` -> swapchain-rebuild -> `resize_event` logging path could not be exercised end-to-end inside the sandbox itself. The project owner ran it by hand instead, and it crashed on the very first real resize (see Step 20.2 below) -- proving the gap was real, not merely theoretical.

### Step 20.2: Real Interactive Resize Crash, Found and Fixed -- Status: Complete (2026-09-13)

**Real, working, and verified end to end.** The project owner's own first real, manual resize crashed immediately: `thread 'main' panicked ... failed to create VulkanSwapchain: DeviceLost`, preceded by a genuine Wayland compositor error, `wp_fifo_manager_v1#63: error 0: Attempted to create a second fifo surface for the wl_surface`. Root cause (REVIEW.md finding #230): the plain `surface = Surface::create(...)` reassignment constructs the *new* `Surface` -- requesting a brand-new `VkSurfaceKHR`/fifo surface from the compositor -- while the *old* `Surface`, still bound to the same underlying `wl_surface`, has not yet been dropped (Rust only drops a variable's previous value after the assignment's right-hand side is fully evaluated). Wayland's `wp_fifo_manager_v1` protocol permits only one fifo surface per `wl_surface` at a time, so the second request was correctly rejected, corrupting the surface and surfacing downstream as `DeviceLost`. `crates/tre-python/src/windowed_renderer.rs`'s own `WindowSlot`-via-`HashMap::insert` resize path carries this identical latent ordering bug -- disclosed, not fixed here, since it's shared code outside this crate's own scope.

**Fixed** by making `resize_test.rs`'s `surface` an `Option<Surface>` and adding a `rebuild_surface` helper that explicitly sets it to `None` -- running the old `Surface`'s `Drop` (which destroys its own `VkSurfaceKHR`) -- *before* constructing the replacement, guaranteeing the old fifo-surface binding is fully released first.

**Verified by the project owner's own second real, hands-on run**: an extended interactive session, roughly 36 seconds of continuous window dragging producing 30+ live resize events, real logged rebuild latencies between 0.68ms and 2.25ms, no crash, and a clean exit (`Interaction-Driven Resize Test complete.`, exit code 0) on closing the window -- the exact scenario that crashed immediately before this fix. The Phase 20 Step 20.1 disclosed gap above is now closed by a real human run, not merely argued to be safe by analogy.

## Phase 20 Summary

A manually triggered, never-CI-gated performance test suite now exists: a real windowed `ramp_test` (geometric primitive-count scaling across three real workload kinds, FPS/CPU/memory/GPU telemetry to console and a structured JSON-Lines log) and a real windowed `resize_test` (a fixed scene under live, human-driven window resizing, with a dedicated swapchain-rebuild-latency record). Two real, load-bearing bugs were found and fixed, both via actual runs rather than assumed safe: an undersized ring buffer in the ramp test (Step 20.1, against real GPU hardware), and a Wayland fifo-surface ordering bug in the resize test that crashed on the project owner's own first real interactive resize (Step 20.2) -- the second found only because the sandbox's own disclosed inability to simulate a resize was honestly reported rather than glossed over, and the project owner then ran it by hand. One real scope substitution was also disclosed rather than silently made (gradient fill instead of MSDF text for the "shaders" workload, since no Rust-side precedent for the latter exists anywhere in the workspace).

## Phase 21: Release Prep (v0.1.0 Alpha) (Added 2026-09-13)

The project had shipped 20 real phases but had never been formally released: no git tags, no `LICENSE` file, no `CHANGELOG.md`, and the MkDocs site only ever built locally. The project owner asked for a release plan; scope, framing, and license were each confirmed via `AskUserQuestion`/`ExitPlanMode` before any file changed (full rationale in `PLAN.md`'s own archived copy of this phase's plan): a GitHub tag + Release plus a GitHub Pages docs deploy (explicitly not a crates.io or PyPI publish in this pass), framed as an early alpha (Linux + Vulkan only, DX12/Metal and Windows/macOS windowing named as roadmap placeholders rather than implied as supported), under a dual MIT OR Apache-2.0 license -- the Rust ecosystem default, chosen after confirming via a targeted repo-wide grep that no license had ever been decided on.

**Real, working, and verified end to end.** `LICENSE-MIT`/`LICENSE-APACHE` added at the repo root (two files, matching how `rust-lang/rust`/`tokio-rs/tokio` structure the identical dual license). `license = "MIT OR Apache-2.0"` added to the workspace's `[workspace.package]`, and `license.workspace = true` added to every one of the workspace's 16 crates' `[package]` tables (this phase's own plan initially miscounted 15 -- corrected against a real `find` before editing anything, see `LOG.md`'s archived copy) -- confirmed afterwards via `cargo metadata` showing `"MIT OR Apache-2.0"` resolved for all 16. `CHANGELOG.md` added: one curated `[0.1.0]` entry organized by subsystem, built from this document's own phase headings rather than copied from its prose, stating the Linux+Vulkan-only scope and DX12/Metal/Windows/macOS limitations up front rather than burying them. `README.md`'s "Status" section was reframed from an engineering-log tone to release-facing (v0.1.0 early alpha, links to `CHANGELOG.md` and the Pages docs URL), with a new "Platform support" table separating real components from placeholders, and a new "License" section. `mkdocs.yml` gained its previously-unset `site_url`. A new `.github/workflows/docs.yml` (kept separate from `ci.yml` -- different trigger semantics and toolchain) builds the MkDocs site on every push/PR/dispatch and deploys it to GitHub Pages via `actions/deploy-pages`, gated to `push` on `main` only.

**Verified:** `cargo fmt --all -- --check`, `cargo clippy --workspace --all-targets -- -D warnings`, and `cargo build --workspace` all passed with zero warnings after the metadata-only `Cargo.toml` changes, confirming no effect on compilation. `mkdocs build --strict` (via the `mkdocs-mcp` server, since plain `mkdocs` is not on this sandbox's shell `PATH` even with `.venv` activated -- disclosed in `LOG.md`, and irrelevant to the new CI job, which installs its own `mkdocs` on a fresh runner) built all 27 pages with zero warnings, confirming the new `site_url` introduced no regression.

**Real, disclosed remaining scope**: three things cannot be completed as local file changes, and are not part of this phase's commit, per the project's standing policy of never touching public/remote state without separate, explicit authorization -- enabling GitHub Pages itself in the repo's Settings (Source: GitHub Actions); pushing this phase's commit to `origin/main`; and tagging `v0.1.0`, pushing that tag, and creating the actual GitHub Release from `CHANGELOG.md`'s `[0.1.0]` section. The `docs.yml` deploy job is therefore verified by inspection plus the local strict `mkdocs build` above, not a live Pages deployment, until the first of those three happens. This phase also has no `demo/phase21/` folder, unlike every prior phase -- there is no runtime/engine behavior to demo (licensing, changelog, README, and a docs-only CI workflow), so no artificial demo was manufactured just to satisfy the letter of that convention.

## Phase 21 Summary

TRE is now release-ready at the file level for a v0.1.0 early-alpha GitHub Release plus a GitHub Pages docs site: a real dual MIT/Apache-2.0 license applied consistently across all 16 crates, a curated `CHANGELOG.md`, a release-facing `README.md` with an explicit platform-support table, and a working, strictly-verified docs-deploy CI workflow. What remains is exclusively the set of actions this project's own standing policy reserves for explicit, separate authorization: enabling Pages, pushing to `origin/main`, and cutting the actual tagged Release.
