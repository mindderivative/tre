...
* **Implementation status (Phase 7 Step 7.2.1, updated 2026-09-08; REVIEW.md finding #130):** *Backdrop & Gaussian Blurs* -- the hand-written RHI-level Dual-Kawase chain (`dual_kawase_blur_demo.rs`) is now working: its own real pixel assertions pass consistently across real GPU runs (REVIEW.md finding #130's own full account has the real root cause, which turned out to be a stale push-constant clobber, not the bindless array originally suspected). This is RHI-level plumbing only, not yet a `Canvas`/`push_layer`/`pop_layer`-level capability -- Step 7.2.2 (wiring it to the layer stack so any layer can request a real backdrop blur) is real, separate, not-yet-started future work. *Blend Modes* were **never built at all** -- confirmed via a repo-wide grep for Multiply/Screen/Overlay/Soft Light/Color Dodge (no matches outside this bullet) and by `planning/archive/PLAN_PHASE7_STEP7_2_1.md`'s own explicit scope decision naming them "out of scope... never part of IMPLEMENTATION.md's own real Step 7.2 task list." The only real blend state anywhere in this engine is the single default premultiplied-alpha "over" mode every pipeline already carries unconditionally (IMPLEMENTATION.md Step 6.4.1).
