...
The project's own high-level UI framework is being built in **Python** and is the engine's first and primary consumer. The engine's public API is exposed across a stable C-ABI boundary (Section 2.7) so that any UI framework -- a custom C++ GUI toolkit, a modern desktop UI runtime, or a language other than Python entirely -- can bind to it without modifying the engine core. *Updated 2026-09-09:* Python itself is now a deliberate, disclosed exception to that boundary, not an exercise of it -- see Section 2.7's own "Cross-Language Boundary: Two Real Paths" for the real architecture and the performance rationale.
...
Every subsystem must define what happens when its steady-state assumptions break, not just how it behaves in the common case. The engine never uses a Rust panic for an expected or recoverable failure (per the project-wide panic-free-on-the-happy-path rule); every fallible operation returns `Result<T, EngineError>`, and callers are required to handle the failure path explicitly. Panics are reserved strictly for programmer errors (invariant violations, indexing bugs), and per Section 2.7 are caught at whichever real cross-language boundary is in play (`tre-ffi`'s own C-ABI `catch_unwind` wrapping, or PyO3's own built-in panic-to-exception conversion for the engine's direct Python binding) rather than allowed to unwind into the calling UI framework.
...
* **Atlas exhaustion beyond LRU capacity (corrected 2026-09-09, Phase 9 Step 9.1):** If Guillotine bin-packing fails and LRU eviction (Section 10.2) cannot free enough contiguous space for an incoming glyph or icon, the engine silently drops that one insertion request for the frame rather than stalling the render loop, allocating an emergency atlas, or blocking. An earlier draft of this bullet described falling back to a lower-fidelity placeholder (a bounding-box glyph or solid-color swatch); the real implementation (`tre-atlas`'s `AtlasOwner::process_insert`) has never rendered one -- it returns early the moment `AtlasPacker::insert` reports no space, after the real eviction pass above has already run. This still satisfies this section's own "report, don't block" contract (the request is dropped, not silently corrupted or hung on), and is real, tested behavior (`crates/tre-atlas/src/owner.rs`'s own adversarial test suite) -- a genuine placeholder-glyph fallback remains real, separate, not-yet-needed future work if a real caller ever demonstrates the drop alone isn't sufficient.
...
The engine remains **language-agnostic** with respect to any *other* UI framework driving it: the `Canvas` API (Section 6) is exposed across a stable C-ABI boundary (`extern "C"` functions, `#[repr(C)]` types only -- see TECHNICAL.md Section 9.4 for the binding mechanism, built by the `tre-ffi` crate, IMPLEMENTATION.md Phase 10 Step 10.2), so any UI framework written in a language capable of calling a C ABI -- a custom C++ GUI toolkit, a modern desktop UI runtime -- can bind to it without modifying the engine core.

#### Cross-Language Boundary: Two Real Paths (revised 2026-09-09)

Earlier drafts of this section claimed Python bound to the engine through the exact same C-ABI boundary as any other language, "on equal footing," receiving "no privileged access." **That claim is no longer accurate, by deliberate decision, and this section is corrected to state the real architecture plainly rather than leave the stale claim standing:**

* **`tre-ffi` (Phase 10 Step 10.2) remains the stable, `#[repr(C)]`/`extern "C"` boundary for every *other* language** -- C, C++, and any future non-Python dynamic-language binding. Nothing about that boundary's own design changes: opaque handles, C-compatible shadow types, `EngineError` result codes, explicit `tre_*_free` ownership transfer (TECHNICAL.md Section 9.4).
* **The project's own Python UI framework binds directly to the engine's native Rust API via [PyO3](https://pyo3.rs/) (IMPLEMENTATION.md Phase 10 Step 10.3), bypassing `tre-ffi` entirely.** PyO3's `#[pyclass]`/`#[pymethods]` machinery wraps `tre-engine` (and, once built, the Phase 10 Step 10.1 shape-primitive layer) types directly -- no C-ABI marshalling round trip (Rust type -> C-compatible shadow type -> PyO3 conversion), no opaque-handle indirection for high-frequency calls (e.g., a shape's own per-frame property mutation), and Rust's own ownership/`Drop` semantics integrate with CPython's reference counting natively instead of requiring explicit `tre_*_free` calls. This is a real, measurable performance decision -- double marshalling through a C-compatible shadow representation has a real per-call cost a first-party binding has no reason to pay -- not an oversight or a return to a Rust-specific interface for its own sake.

**This is a genuine, disclosed departure from this section's own prior "equal footing" principle, not a silent one.** Python -- as the project's own first-party UI framework, not a third-party integration -- now receives real, privileged access (direct Rust-type binding) that a UI framework written in C++, C#, or any other C-ABI-calling language does not. The engine's *core* remains language-agnostic in the sense that matters architecturally -- nothing in `tre-engine` itself assumes Python, and `tre-ffi`'s own C-ABI is still real, still complete, and still the correct integration point for any language other than Python. What changed is only which *binding layer* Python itself uses to reach that same core.

*FFI Safety, `tre-ffi` boundary:* A Rust panic must never unwind across the C-ABI boundary -- doing so is undefined behavior. Every `extern "C"` entry point wraps its body in `std::panic::catch_unwind` and converts a caught panic into an `EngineError` result code (Section 2.6), so a programmer error inside the engine (e.g., an out-of-bounds index during `RhiDevice::begin_frame`) surfaces to a `tre-ffi` caller as a recoverable error rather than crashing the host process or corrupting its stack.

*FFI Safety, Python's own direct PyO3 boundary:* The identical requirement applies, enforced by a different, real mechanism -- PyO3 itself catches an unwinding Rust panic at its own generated `#[pymethods]`/`#[pyfunction]` call boundary and converts it into a raised Python exception before it can cross into the CPython interpreter's own call stack (PyO3's standard, built-in behavior, not custom `catch_unwind` wrapping this project writes itself, since there is no hand-written `extern "C"` entry point in this path for such wrapping to attach to). A programmer error inside the engine still cannot crash or corrupt the host Python process; only the mechanism enforcing that differs from the `tre-ffi` path.
...
**Reference Integration -- Python UI Framework:** The project's own high-level UI framework (Python, Section 2.7) targets the Desktop Application Toolkit category above and serves as the reference, dogfooding integration. *Updated 2026-09-09:* Python binds directly via PyO3, not through `tre-ffi`'s C-ABI (Section 2.7's own "Two Real Paths" correction) -- it therefore no longer validates the C-ABI boundary itself; `tre-ffi`'s own C-ABI is validated by its own dedicated, non-Python test harness (IMPLEMENTATION.md Phase 10 Step 10.2), and Python's own dogfooding validates the direct PyO3 binding path instead.
...
* **Implementation status (Phase 5 Step 5.3.1, 2026-09-07):** "Spatial Boundary Tagging" is real -- `RenderingCanvas::tag_accessibility_node` records a node's real, transform-correct world-space bounds (the axis-aligned bounding box of all four transformed local corners, correct under rotation, not just translation/scale) into a flat per-frame list, surfaced through `flatten()`/`FlattenedFrame::accessibility_nodes` and correctly carried across threads via `SubCanvas`/`FrameArena`/`stitch_into` (Section 6's own aggregator).

* **Implementation status (Phase 5 Step 5.3.2, 2026-09-08):** the "OS Accessibility Bridge" bullet is now real for Linux -- a new `tre-a11y` crate publishes every tagged node onto the real AT-SPI2 bus via `accesskit`/`accesskit_unix`, verified against a real, live registry (not a mock): a second, independent D-Bus connection genuinely discovers the published application and confirms `Component.GetExtents` matches exactly what was tagged. Windows UI Automation/macOS NSAccessibility remain deferred (TECHNICAL.md Section 2.3's own "Linux complete" precedent); the real end-to-end proof wiring this bridge into an actual rendered, GPU-submitted scene is Step 5.3.3's capstone.

* **Implementation status (Phase 5 Step 5.3.3, 2026-09-08):** the "$100\%$ alignment" claim above is proven end to end on a real desktop session, for Linux -- a single real render (`canvas_accessibility_demo`, handing its tagged nodes to a second, genuinely Vulkan-free process, `canvas_accessibility_verify`, for the real AT-SPI2 work -- see IMPLEMENTATION.md Step 5.3.3 for why) checks the real GPU framebuffer, the tagged `AccessibilityNode` IR, and a real, live AT-SPI2 query against one shared source of truth, including a rotated tagged node (the one genuinely tricky case). **Step 5.3 is not yet closed, however: CI's own `accessibility-validation` gate still fails**, for a precisely-identified reason unrelated to this rendering/tagging work itself -- see IMPLEMENTATION.md Step 5.3.3 and REVIEW.md finding #126 for the real root cause and current stopping point. Building this proof did catch a real, previously-undiscovered regression in Step 5.3.2's own role mapping (`AccessibilityRole::Generic` had been silently invisible to every real assistive technology since that step shipped, since it mapped onto `accesskit::Role::GenericContainer`, which `accesskit_consumer::common_filter` always excludes from the platform tree) -- fixed as part of this step; see IMPLEMENTATION.md Step 5.3.3.
...
* **Implementation status (Phase 7 Step 7.2.2, updated 2026-09-08; REVIEW.md findings #130/#152/#153):** *Backdrop & Gaussian Blurs* -- own-content blur is now a real, working `Canvas`-level capability: `RenderingCanvas::push_layer(&LayerDesc { blur: true, .. })` applies a real Dual-Kawase blur (`RhiCommandBuffer::apply_layer_blur`) to a layer's own newly-drawn content before compositing, proven end to end through `Canvas`/`execute_frame` (`layer_blur_demo.rs`), no hand-written RHI calls needed by the caller. **Own-content only, still not a *true backdrop* blur** -- it blurs the layer's own pixels, not whatever is visually behind it on the swapchain; that needs a real swapchain snapshot/copy capability this engine doesn't have yet (`planning/archive/PLAN_PHASE7_STEP7_2_1.md`'s own explicit scope choice, unchanged), real, disclosed future work. `LayerDesc.blur` is a fixed-depth `bool` toggle only -- no tunable radius/quality yet, deferred until a real caller needs it. *Blend Modes* were **never built at all** -- confirmed via a repo-wide grep for Multiply/Screen/Overlay/Soft Light/Color Dodge (no matches outside this bullet) and by `planning/archive/PLAN_PHASE7_STEP7_2_1.md`'s own explicit scope decision naming them "out of scope... never part of IMPLEMENTATION.md's own real Step 7.2 task list." The only real blend state anywhere in this engine is the single default premultiplied-alpha "over" mode every pipeline already carries unconditionally (IMPLEMENTATION.md Step 6.4.1).
