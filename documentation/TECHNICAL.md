...
* **Implementation status (Phase 9 Step 9.2, 2026-09-09):** real, as `tre_memory::DebugAllocGuard`/`RenderTickGuard` -- in `tre-memory`, not `tre-engine`, since `tre-engine` carries `#![forbid(unsafe_code)]` and `GlobalAlloc` requires `unsafe`; `tre-memory` is one of the four workspace locations Section 9.1 permits it in. `#[global_allocator]` is installed by the opting-in binary itself (`main_loop_demo.rs`), never by the library crate, matching the attribute's own whole-binary scope. Wiring the guard to the real continuous main loop required first fixing REVIEW.md finding #134 (main_loop_demo's own ~20+ allocations/frame) via new `RenderingCanvas::reset()`/non-consuming `stitch_into`/`FrameArena::flatten_into` reuse APIs, and surfaced two further real, previously-undetected allocation sources: `radix_sort_by_key`'s own `counts` histogram buffer (fixed, finding #157) and `VulkanDevice::begin_frame`'s per-frame `Box<dyn RhiCommandBuffer>` (disclosed, not fixed, finding #156 -- genuine `RhiDevice` trait-boundary redesign). `std::thread::scope` itself was also found to allocate an `Arc<ScopeData>` on every call (finding #158, real standard-library behavior); `main_loop_demo.rs`'s own guard coverage brackets that call rather than spanning it. See REVIEW.md's "Phase 9 Step 9.2 Implementation" section for the full account.
...
* **Implementation status (Phase 9 Step 9.1, 2026-09-09):** real, in `tre-engine`'s `radix_sort_by_key` -- a least-significant-digit radix sort, 4 passes of a 16-bit digit each ($4 \times 16 = 64$ bits), exactly as specified above. `flatten_run` had used `std::sort_unstable_by_key` (a comparison sort) from Step 5.1.3 through this step; the `sort_key` field's own doc comment already said "64-bit Radix Sort Key," but no prior step had actually built the algorithm this section names. Verified against adversarial key distributions (all-identical, reverse-sorted, every field's own bit boundary, maximum Depth ID) and a 200-run randomized differential test proving agreement with `sort_unstable_by_key` on every run, not just the specific cases. No small-$N$ comparison-sort fallback threshold -- a real, deliberate, disclosed scope boundary (this step is a correctness pass, not the performance-tuning one Section 9.2 owns); see `radix_sort_by_key`'s own doc comment.
...
* **Implementation status (Phase 4 Step 4.2.2, 2026-09-06):** the "Atlas Generation" bullet above is now real for a single glyph at a time -- `tre_text::generate_msdf` produces an actual `32x32` RGB8 MSDF via `fdsm` (a pure-Rust reimplementation of `msdfgen`'s own published algorithm), fed by this crate's own Step 4.1 outline-extraction geometry, correctly handling glyphs with true holes (verified against a real `'O'`) since MSDF encodes contours/winding directly in the distance field rather than needing a triangulated, hole-aware mesh. The "Shader Evaluation" formula above is still not built -- no shader or pipeline in this project samples an arbitrary texture yet, deliberately deferred to Step 4.2.3 rather than inventing throwaway GPU plumbing here. Atlas packing (Step 4.2.1) and multi-window atlas concurrency (Step 4.2.4) are separate, already-tracked pieces.

* **Implementation status (Phase 4 Step 4.2.3, 2026-09-06):** the "Shader Evaluation" formula above is now real -- a new `msdf.frag` implements it verbatim (median-of-channels signed distance, `fwidth`-based opacity), sampling a real MSDF texture through Phase 2 Step 2.1's existing bindless-texture infrastructure with no new descriptor-set or pipeline code at all (paired with `bindless_textured.vert` unchanged). A new `TextureFormat::Rgba8Unorm` (linear, no gamma) carries the MSDF texture itself, since the existing `_SRGB` formats would corrupt the encoded distance at every value except the two endpoints. Verified end to end against a real `'O'` glyph rendered at roughly 7x magnification: a real hollow ring survives to the screen, and a center-row pixel scan confirms genuinely intermediate (neither pure fill nor pure background) pixels at the ring's own edges -- real sub-pixel anti-aliasing, resolving the jagged-edge appearance a flat-color/triangulated render of the same glyph would have shown. Atlas packing (Step 4.2.1, already built) and multi-window atlas concurrency (Step 4.2.4) remain the only pieces left in this section.
...
* **Retired (Phase 10 Step 10.2 follow-up, 2026-09-09): the ear-clipping triangulator and the stencil-and-cover technique both described above are gone.** The project adopted [`lyon`](https://github.com/nical/lyon) as its one tessellation backend for both `tre-svg` and `tre-engine` (see IMPLEMENTATION.md's "Step 10.2 Follow-up" write-up and REVIEW.md's closing of finding #163). `lyon::tessellation::FillTessellator`'s real sweep-line algorithm resolves self-intersecting contours and multi-contour holes directly, so the GPU-side stencil-and-cover workaround -- and `triangulate.rs`/`stencil.rs`/`create_stencil_and_cover_pipelines`/`tre_engine::FillRule` along with it -- were deleted rather than kept as a second, now-redundant code path. The stencil image/attachment machinery on `VulkanSwapchain`/`HeadlessSwapchain`/`begin_frame`/`create_pipeline` described above stays in place (any future real use, not only this retired one), but nothing in the workspace enables the stencil test today.
...
* **Lock-Free Aggregation:** Sub-canvases are stitched into the main command arena using atomic pointer increments (`AtomicUsize::fetch_add`) to reserve output space in the global intermediate representation array, requiring zero mutex locks.
  ...
  *Implemented as of Phase 5 Step 5.2.2 (2026-09-07):* the generic, allocation-once-at-construction primitive is `tre_memory::ScatterArena<T>` -- a fixed-capacity buffer any number of threads can concurrently reserve disjoint, contiguous ranges of via one atomic `fetch_add`, then write into without a lock, following `MpscRingBuffer`'s own established per-slot `UnsafeCell<MaybeUninit<T>>` pattern (simpler here: no ring wraparound, no per-slot sequence number, since nothing is ever popped or reused mid-frame). `tre-engine`'s `FrameArena` bundles three of them (vertices/indices/commands); `RenderingCanvas::stitch_into`/`SubCanvas::stitch_into` merge one source's locally-recorded data in, rebasing index values and each command's `vertex_offset` to their new positions (a literal `copy_from_slice` only works for the vertex array itself -- indices and commands both carry offsets that must shift once concatenated). Verified under genuine concurrent stress: a real 8-thread/4,000-item test in `tre-memory` for the primitive itself, and a real 4-worker-thread test in `tre-engine` in which each thread stitches its own `SubCanvas` into a shared `FrameArena` as its own last action before exiting.
...
* **Multi-Window Atlas Concurrency:** DESIGN.md Section 10.3 requires that no window ever blocks on another window's atlas insertion. Two lock-free primitives implement this:
  ...
  * A fixed-capacity **single-writer/multi-reader (SWMR) open-addressed table** publishes completed atlas coordinates: the atlas owner is the only writer (`Ordering::Release` store into a slot), and any window's rendering thread reads (`Ordering::Acquire` load) without ever taking a lock or performing a CAS. This is a purpose-built structure, not a general-purpose concurrent hash map (e.g. not `dashmap`) -- entries are still only ever mutated by that one writer thread (Phase 4 Step 4.3.1 added real per-key removal via tombstone deletion, but removal remains writer-exclusive, same as insertion; readers still only ever `get`/`get_and_touch`, never a lock or a CAS), which is exactly the access pattern a plain atomic-publish table is sufficient for. Swapping the *hasher* used elsewhere for hot-path lookups (`FxHashMap`/`ahash`, Section 3.2) does not solve this problem by itself -- it only makes single-threaded lookups faster, not concurrent ones safe.

  *Implemented as of Phase 4 Step 4.2.4 (2026-09-06):* both primitives are real, generic types in `tre-memory` -- `MpscRingBuffer<T>` (Dmitry Vyukov's well-known bounded MPMC ring buffer design, simplified for the single-consumer case: no CAS needed on the dequeue side at all) and `SwmrSlotTable<K>` (needs zero `unsafe`, since both the key and value at each slot are plain `AtomicU64`s). `tre-atlas`'s `AtlasOwner` is a real, dedicated background thread built on top of them, verified under genuine concurrent stress: a real 8-thread/80,000-item test for the ring buffer, and a real multi-glyph, multi-producer-thread round trip (concurrent insertion requests, real packing, real MSDF generation, correct non-overlapping publish) for the full atlas owner. `tre-atlas` itself stays content-agnostic throughout -- the request type carries a boxed `RasterSource` trait object, not a direct dependency on `tre-text`.

  *Extended as of Phase 4 Step 4.3.1 (2026-09-06):* `SwmrSlotTable` gained `remove` (a second reserved sentinel, `TOMBSTONE_KEY`, distinct from the existing `EMPTY_KEY` -- classic linear-probing-with-tombstones, since the module's own prior "hit an empty slot means the key was never inserted" early-exit is only sound when nothing is ever removed), `get_and_touch` (per-slot recency tracking via `AtomicU64::fetch_max`, a separate concern from the key/value payload since *any* reader thread needs to write a recency stamp, not just the single writer), and a writer-only `scan_older_than` for enumerating stale entries. This is the data-structure foundation the atlas's real LRU eviction policy (DESIGN.md Section 10.2) needs. *The policy itself is now built, as of Phase 4 Step 4.3.3 (2026-09-06):* before attempting to pack each request, `AtlasOwner` checks `AtlasPacker::used_fraction()` (Step 4.3.2) against the 85% threshold, and if crossed, evicts every entry whose recency is more than 600 frames stale in one pass, pairing `SwmrSlotTable::remove` with `AtlasPacker::remove` per evicted entry so neither a leaked rect nor a resurrected key with no backing space can result. A freshly-inserted entry is stamped with its own creation frame immediately (treating insertion itself as an access, standard LRU-cache practice) specifically to avoid evicting brand-new content the instant a later insert crosses the threshold.

## 9. Toolchain & Development Environment

### 9.1 Language & Compiler Targets

* **Rust Edition:** 2021 minimum. MSRV (Minimum Supported Rust Version) is `rust-version = "1.88"` in the root `Cargo.toml`, enforced on every commit by CI's `msrv` job, which installs exactly that toolchain and runs `cargo check --workspace --all-targets --locked` (REVIEW.md finding #248, 2026-09-13; the previously-declared 1.75 was never verified and was false -- finding #247). `rust-toolchain.toml` pins the separate *development* toolchain (1.98.0) and is not the MSRV. Strictly leveraging `const` generics and compile-time `const fn` evaluation for memory alignments, and `#[repr(C)]` layouts for every type that crosses the FFI boundary (Section 9.4).
...
* **No unwinding past the FFI boundary:** A Rust panic must never propagate past an `extern "C"` function back into the calling UI framework (Section 9.4.1) -- doing so is undefined behavior per the Rust reference. This requires the *opposite* of the commonly-recommended `panic = "abort"` release-profile setting: the `tre-ffi` crate and its entire dependency graph build with the default `panic = "unwind"` strategy specifically so that `std::panic::catch_unwind` has stack unwinding available to catch. Every exported `extern "C"` entry point wraps its body in `catch_unwind`, converting any caught panic into an `EngineError` result code (DESIGN.md Section 2.6) before returning to the caller. `panic = "abort"` must never be set on any profile used to build the shipped `cdylib`/`staticlib` -- it would make every `catch_unwind` call a silent no-op, turning a recoverable panic back into a process-terminating crash and defeating the entire purpose of this rule. The identical `panic = "unwind"` requirement applies to `tre-python`'s own build (Section 9.4.2) for the same underlying reason -- PyO3's own built-in panic-to-exception conversion also needs real unwinding available to catch, even though this project writes no `catch_unwind` call of its own on that path.

* **`unsafe` policy:** `unsafe` is permitted only inside the RHI backend crates (raw graphics API FFI), the ring-buffer/arena allocators (Section 3, including the atlas's MPSC/SWMR concurrency primitives in Section 8), the `tre-ffi` crate (raw handle/pointer conversion and manual buffer-ownership transfer across the C-ABI boundary, Section 9.4), and, since IMPLEMENTATION.md Phase 10 Step 10.4, the `tre-python` crate (Section 9.4.2) -- one call, `PyHeadlessRenderer::new`'s own probe-`VkSurfaceKHR` teardown, the identical FFI cleanup every RHI demo already performs, now needed inside a real (non-example) crate for the first time since `HeadlessRenderer` builds its device via the same probe-window pattern those demos use. Every other crate in the workspace, including `tre-engine` itself, carries `#![forbid(unsafe_code)]`, so the four permitted locations above are the complete, closed set -- not merely the ones called out for emphasis. The `tre-platform` crate carried a fifth exception from Phase 1 Step 1 (`raw-window-handle` construction, and XCB FFI via `x11rb`'s `allow-unsafe-code` feature for a real `xcb_connection_t*`) through IMPLEMENTATION.md Phase 10; Phase 11 Step 11.1 migrated its windowing backend to `winit`, whose `Window`/`EventLoop` implement `raw-window-handle` 0.6's traits directly, so `tre-platform` no longer constructs any raw handle by hand and now also carries `#![forbid(unsafe_code)]`, removed from this list. The vector-math/SIMD crate (Section 2.2) is *not* on this list either: since it is built on the `wide` crate's safe public API rather than raw `core::arch` intrinsics, it requires no `unsafe` of its own. (If a future gap forces a raw intrinsic `wide` doesn't cover, add that crate back to this list explicitly at that point -- per the same reasoning that closed the list in the first place, don't leave an implicit, undocumented exception.) Every `unsafe` block requires an adjacent `// SAFETY:` comment stating the invariant being upheld, and `#![deny(unsafe_op_in_unsafe_fn)]` is set workspace-wide.
...
* **Build System:** Cargo workspace (Cargo 1.88+, matching `rust-version`): a core `tre-engine` crate, one crate per RHI backend (`tre-rhi-vulkan`, `tre-rhi-dx12`, `tre-rhi-metal`, platform-gated via `#[cfg(target_os = ...)]`), a `tre-platform` crate owning native window creation (ARCHITECTURE.md Section 1's "Platform & Event Layer" -- added Phase 1 Step 1, Linux-only for now, backed by `winit` since Phase 11 Step 11.1), a `tre-ffi` crate that owns the entire C-ABI surface for every language other than Python (Section 9.4.1), and a separate `tre-python` crate (Section 9.4.2, IMPLEMENTATION.md Phase 10 Step 10.4) that depends on `tre-engine` directly and builds its own PyO3 extension module -- `tre-ffi` and `tre-python` are two independent build outputs, neither depends on the other. `tre-engine` and the RHI backend crates are still statically linked (as `rlib`s) into `tre-ffi`'s own final `cdylib`/`staticlib` -- every crate's code ships inside that one binary -- but `tre-ffi` is the *only* crate, of that binary's own dependency graph, whose items are `pub` and re-exported as `extern "C"` symbols; every other crate's items stay `pub(crate)`-or-narrower from `tre-ffi`'s perspective. This achieves the same "hide RHI backend symbols from a C-ABI consumer" goal that CMake-based linker visibility flags (`-fvisibility=hidden` / `.def` export lists) would otherwise be used for, without relying on a linker feature. `tre-python` builds its own, separate native extension module (via `maturin`/PyO3's own build tooling) directly from `tre-engine`, never through `tre-ffi`'s `cdylib`.
...
* **Continuous Integration:** CI pipelines must include hardware-accelerated headless rendering instances to run automated performance regression tests (`cargo bench` via `criterion`), verifying CPU frame processing performance against a CI-hardware-calibrated baseline (see below) on target hardware, and must also run under the zero-allocation debug guard (Section 3.4) as a hard gate, not merely a benchmark. `cargo clippy` and `cargo test` run on every commit alongside the performance suite. The Python binding (Section 9.4) is exercised by its own `python` CI job (REVIEW.md finding #254, 2026-09-13): it builds the real `tre_python` wheel with `maturin` from `crates/tre-python/Cargo.toml`, installs it into the runner's Python, and runs every `demo/*/demo.py` script as a self-asserting end-to-end test on the same software-Vulkan/Xvfb/D-Bus recipe the Rust example jobs use -- before that job existed, nothing in CI built or ran the extension at all.

  * **Implementation status (Phase 9 Step 9.2, 2026-09-09):** the `criterion` performance suite is real but minimal -- one representative CPU-only benchmark (`tre-engine/benches/frame_processing.rs`, `record_and_flatten_10k_nodes`), run and threshold-checked in `ci.yml`'s `test` job, not the full hardware-accelerated per-demo suite this bullet envisions; a real, honestly-measured gap against the originally-documented absolute budget is disclosed as REVIEW.md finding #159, deliberately left as a failing CI gate rather than silently passing. The zero-allocation guard is compiled directly into `main_loop_demo`'s own binary (`#[global_allocator]`), so it already runs as a real hard gate every time CI's `vulkan-validation` job runs `main_loop_demo` (Step 8.1.2) -- no separate CI step was needed. (REVIEW.md finding #261, 2026-09-14: that job now runs `main_loop_demo` and 41 other examples via a single `cargo test -p tre-rhi-vulkan` invocation, each promoted into `tre-rhi-vulkan/tests/` as a `[[test]] harness = false` target, rather than 42 separate `cargo run --example` steps.) See Section 3.4's own implementation-status annotation for the full account.

  * **Gate redesign (REVIEW.md finding #227, 2026-09-13):** this gate originally checked the benchmark's mean against a hard *absolute* ceiling of 0.50ms, calibrated on the project owner's own local development hardware. A real CI run of already-optimized code (finding #224) measured ~1.8x that local figure -- the opposite of every local measurement, which showed the same code getting faster, not slower. Re-running the *identical* code on CI five times back-to-back (`gh run rerun --job`, zero code changes) found a tight cluster (1.7% coefficient of variation, 4.6% full range) at that same ~1.8x-slower level, which rules out "GitHub Actions' shared runners are noisy" as the explanation: five samples of identical code scattered this tightly is a stable, repeatable hardware difference, not run-to-run variance. An absolute budget calibrated on one machine cannot hold on another machine with a different, but internally consistent, performance level -- no amount of re-running changes that. The gate is therefore now a **relative check against a CI-hardware-calibrated baseline with a percentage tolerance**, not an absolute number: `crates/tre-engine/benches/frame_processing_ci_baseline.json` records `baseline_ns` (892,300ns, the mean of the five characterization runs above) and `tolerance_pct` (20%, roughly 4x the observed real-world scatter, chosen with enough margin to absorb normal noise while still catching a genuine regression back toward the pre-#224-fix code's own ~796,000ns performance class). `ci.yml`'s check step loads both the fresh benchmark result and this baseline file and fails only if the new measurement exceeds `baseline_ns * (1 + tolerance_pct/100)`. This baseline is deliberately CI-hardware-specific and must never be compared against, or re-derived from, a local `cargo bench` run -- doing so would silently reintroduce the exact absolute-number-on-the-wrong-hardware problem this redesign fixes. Re-establishing the baseline (a genuine, intentional performance change to the benchmarked code) should be done from several fresh CI runs of the new code, the same way this one was, not a single sample.
...
Added alongside the Rust engine / Python UI framework language decision. **Revised 2026-09-09:** this section previously specified one boundary -- the `tre-ffi` C-ABI -- shared by every language binding, Python included. That is no longer the real architecture: `tre-ffi` remains the stable boundary for every language *other* than Python, but the project's own Python UI framework now binds directly to `tre-engine`'s native Rust API via PyO3, bypassing `tre-ffi` entirely, for real, measured performance reasons (avoiding a double marshalling round trip through a C-compatible shadow representation on every call, and avoiding opaque-handle indirection for high-frequency calls such as a Phase 10 Step 10.1 shape's own per-frame property mutation). See DESIGN.md Section 2.7's own "Cross-Language Boundary: Two Real Paths" for the full disclosure of this departure from the section's original "one boundary, no privileged access" principle.

#### 9.4.1 `tre-ffi`: the C-ABI boundary for C, C++, and other non-Python bindings

The `tre-ffi` crate (Section 9.2) defines a `#[repr(C)]`, `extern "C"` API covering the engine's entire public surface -- the correct, and now only, integration point for any language other than Python.
...
* **Memory ownership:** Any buffer the engine allocates and hands across the boundary (e.g., a headless frame readback buffer, DESIGN.md Section 4.3) is freed by an explicit `tre_*_free` function, never by the caller's allocator -- Rust's allocator and the host language's allocator are never assumed compatible.
* **FFI safety:** Every exported function wraps its body in `std::panic::catch_unwind` (Section 9.1), converting any caught panic into an `EngineError` result code before it can unwind across the boundary.
* **Testing:** `cargo test`'s FFI test target, exercised against a real, dedicated non-Python C test harness (not the Python package's own test suite, which no longer touches `tre-ffi` at all -- see 9.4.2) -- Section 9.2's CI gate treats a failure here as a build failure.

#### 9.4.2 Python: a direct PyO3 binding to native Rust, not through `tre-ffi`

The project's own Python UI framework -- the engine's first and primary consumer -- binds via [PyO3](https://pyo3.rs/) directly against `tre-engine` (and, once built, the Phase 10 Step 10.1 shape-primitive layer), in a dedicated crate (`tre-python`, IMPLEMENTATION.md Phase 10 Step 10.4) that depends on `tre-engine` directly and does **not** depend on `tre-ffi` at all.

* **Binding mechanism:** PyO3's `#[pyclass]`/`#[pymethods]` macros wrap native Rust types and methods directly -- no C-compatible shadow representation, no opaque-handle-plus-getter/setter indirection. Pythonic ergonomics live at this same layer: context managers for `Canvas.save()`/`restore()` scope pairs, Python exceptions raised from `EngineError` via a `From<EngineError> for PyErr` impl (PyO3's own standard mechanism, cheaper than round-tripping through an integer result code and a separate Python-side lookup), and buffer-protocol views over headless frame readback buffers (DESIGN.md Section 4.3) to avoid an extra copy into Python.
* **Memory ownership:** No `tre_*_free`-style explicit-free API exists on this path -- PyO3's own `#[pyclass]` `Drop` glue integrates Rust ownership with CPython's reference counting natively; a Python object going out of scope drops its wrapped Rust value the ordinary way.
* **Concurrency:** The GIL is released (`Python::allow_threads`) for the duration of any blocking engine call (e.g., an `RhiDevice::begin_frame` fence wait) so Python-side multi-threading is not serialized behind engine calls.
* **FFI safety:** PyO3's own built-in mechanism catches an unwinding Rust panic at its generated call boundary and raises it as a Python exception before it can reach the CPython interpreter's own stack -- no hand-written `catch_unwind` wrapping is needed or present on this path, since there is no hand-written `extern "C"` entry point here for such wrapping to attach to (unlike 9.4.1's `tre-ffi` path).
* **Testing:** The Python package's own test suite exercises `tre-python`/`tre-engine` directly -- it no longer shares entry points with `tre-ffi`'s own FFI test target (9.4.1), so the two are gated as two separate, still-both-required CI checks (Section 9.2), not one shared suite. Phase 9 Step 9.1's correctness suite is still exercised through Python here, just against the direct binding rather than the C-ABI.
