...
**The actual, confirmed root cause (seventh push):** rather than guess again, `tre-a11y`'s own already-reliable round-trip test (zero Vulkan/X11 code of its own) was added as an extra diagnostic step inside `accessibility-validation`'s own job -- same packages installed, same `DISPLAY` set, completely unrelated binary. That identical test took **30.06 seconds** there, versus its usual ~10.05s in the plain `test` job -- tripling purely from being in an environment with Xvfb/Vulkan packages installed and a display configured, regardless of which process runs or what it does. Ordering, wrapper nesting, and CPU contention were all real, reasonable hypotheses, each directly falsified by real evidence in turn -- exactly the discipline this project's own "prove it, don't assume it" standard calls for, even when (especially when) an earlier guess feels intuitive. **Real, final fix:** both `tre-a11y`'s round-trip test and `canvas_accessibility_demo`'s own discovery timeout widened from 30s to 60s, real margin over the 30.06s worst case actually measured. The now-inconclusive diagnostic step and superseded reasoning were removed from `ci.yml`'s own comments, replaced with the confirmed explanation.

## Summary table (Phase 5 Step 5.3.3 CI Verification)

| # | Finding | Doc(s)/Code | Severity | Resolution |
|---|---|---|---|---|
| 126 | `canvas_accessibility_demo`'s real AT-SPI2 discovery delay triples (confirmed via `tre-a11y`'s own unrelated test: 10.05s -> 30.06s) inside any CI job with Xvfb/Vulkan packages installed and `DISPLAY` set, regardless of ordering, wrapper nesting, CPU contention, or which binary runs -- all four directly tested and ruled out before the real cause was isolated | .github/workflows/ci.yml, tre-a11y, tre-rhi-vulkan (example code) | Should-fix | Fixed — both discovery timeouts widened 10s→60s, real margin over the 30.06s worst case actually measured; demo also given its own isolated CI job as a real, independent improvement |
