...
### 130. [Should-fix] Sampling a bindless texture while the current render target is an offscreen texture (not the swapchain) produces all-zero output -- real root cause not found, investigation STOPPED
...
**Net result: all four newly-proposed mechanisms are ruled out, joining the original twelve (thirteen total, now including a real synchronization-validation pass) -- the real root cause remains unfound.** The one genuinely new, durable asset from this session is the synchronization-validation technique itself (`VK_LAYER_VALIDATE_SYNC=1`), now a real, proven-working diagnostic available for the next investigation session, beyond what the original twelve hypotheses' own CLI-based methods could check. The report's own recommended next step -- a real RenderDoc frame capture inspecting `dual_kawase_blur_demo.rs`'s actual descriptor-set/image-layout state at the moment of the failing draw -- remains the most promising untried path and still requires interactive GUI tooling this environment cannot run unattended. Step 7.2.1 stays open.

## Summary table (Phase 7 Step 7.2.1)

| # | Finding | Doc(s)/Code | Severity | Resolution |
|---|---|---|---|---|
| 130 | Sampling a bindless texture while the active render target is an offscreen texture (not the swapchain) produces all-zero output; extensive elimination (13 independent hypotheses across two sessions, including a real synchronization-validation pass) did not find the real root cause | tre-rhi-vulkan (`dual_kawase_blur_demo.rs`, `kawase_downsample.frag`, `kawase_upsample.frag`) | Should-fix | **STOPPED, unfixed** -- one real, independently-correct fix (`begin_render_to_texture_no_end`) landed along the way and is kept; an external research review's 4 additional hypotheses were also checked and ruled out, yielding one durable new diagnostic tool (`VK_LAYER_VALIDATE_SYNC=1`) for the next session; the sampling failure itself remains unresolved, real reproduction case kept out of CI, Step 7.2.1 not closed |
