...
**Status: fixed, pending the first real run.** Every part of the job that can be verified from this machine has been; the two things only the first CI run (or a local lavapipe install) will settle are the GPU demos on the runners' software driver under Xvfb, and the tray demos with no `StatusNotifierWatcher` host at all -- both named here rather than assumed.
