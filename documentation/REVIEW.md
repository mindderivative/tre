...
| # | Finding | Doc(s)/Code | Severity | Resolution |
|---|---|---|---|---|
| 130 | Sampling a bindless texture while the active render target is an offscreen texture (not the swapchain) produces all-zero output; extensive elimination (12 independent hypotheses tested and ruled out) did not find the real root cause | tre-rhi-vulkan (`dual_kawase_blur_demo.rs`, `kawase_downsample.frag`, `kawase_upsample.frag`) | Should-fix | **STOPPED, unfixed** -- one real, independently-correct fix (`begin_render_to_texture_no_end`, resolving a genuine double-`cmd_end_rendering` bug) landed along the way and is kept; the sampling failure itself remains unresolved, real reproduction case kept out of CI, Step 7.2.1 not closed |
