...
| # | Finding | Doc(s)/Code | Severity | Resolution |
|---|---|---|---|---|
| 130 | Sampling a bindless texture while the active render target is an offscreen texture (not the swapchain) produces all-zero output; 13 hypotheses on the bindless path itself were ruled out, but a non-bindless experiment reproduced correct output under the identical lifecycle | tre-rhi-vulkan (`dual_kawase_blur_demo.rs`, `dual_kawase_nonbindless_experiment.rs`) | Should-fix | **Root cause narrowed to the bindless array itself, real path forward found, not yet applied to the real implementation** -- `begin_render_to_texture_no_end` (independently-correct) kept; `VK_LAYER_VALIDATE_SYNC=1` is a durable new diagnostic; `dual_kawase_nonbindless_experiment.rs` proves a plain, conventional descriptor binding (matching how other engines actually do real-time blur) reads correctly under the exact same render-to-texture lifecycle where bindless reads all-zero, across 4 consistent real runs -- added to CI; converting the real Dual-Kawase implementation to this approach is real follow-on work, not yet done; Step 7.2.1 stays open |
