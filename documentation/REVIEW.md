...
**Status: fixed, pending the first real run.** Every part of the job that can be verified from this machine has been; the two things only the first CI run (or a local `openbox` install) will settle are the focus demo under a real WM, and the tray demos with no `StatusNotifierWatcher` host at all -- both named here rather than assumed.
