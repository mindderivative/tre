...
### 178. [Decision] `PlatformConnection::scale_factor`'s `i32` return type throws away real per-window fractional-DPI precision `winit` now supplies

The previous backends' `scale_factor` was either a single connection-wide, last-seen-`wl_output`-scale value (Wayland, explicitly disclosed as "single-monitor dev setups only") or a hardcoded `1` (X11, `Xft.dpi`/RandR detection explicitly out of scope). `winit::window::Window::scale_factor() -> f64` gives a real per-window value (Wayland `wp-fractional-scale` falling back to integer scale; X11 `Xft.dpi`/RandR) -- a strict improvement in the underlying data. Phase 11 Step 11.1's own scope decision (preserve `PlatformConnection`'s public API exactly, to avoid touching all 40 dependent demo call sites) keeps the method's return type at `i32`, so the new value is rounded to the nearest integer before returning, throwing the newly available fractional precision back away.

**Resolution:** not fixed, by deliberate scope decision, not oversight -- disclosed here and in IMPLEMENTATION.md's Step 11.1 write-up. A future, deliberately API-breaking step could widen this method to `f64` once its 40 call sites are being touched anyway for some other reason.
...
| 178 | Nice-to-have | Documented | `scale_factor`'s preserved `i32` return type rounds away the real per-window `f64` precision `winit` now supplies; deliberate, scope-bounded, not fixed this step |
