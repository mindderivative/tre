...
| # | Finding | Doc(s)/Code | Severity | Resolution |
|---|---|---|---|---|
| 126 | `org.a11y.Status.IsEnabled` only flips true once `at-spi-bus-launcher`'s own one-shot, unretried self-connect to the a11y bus (inside `ensure_a11y_bus()`) succeeds and installs the `EventListenerRegistered` subscription -- confirmed via direct `dbus-monitor`/`busctl` evidence that this self-connect never completes in the `accessibility-validation` job, and via `at-spi-bus-launcher`'s own real source that a failed attempt there is silent (one `g_critical` log, no retry) | tre-a11y, tre-rhi-vulkan (example code), .github/workflows/ci.yml | Should-fix | Open — crash, hang, and the gsettings-schema theory are all now ruled out by direct evidence (process confirmed alive in a healthy state immediately before `busctl list`, responsive throughout, clean self-terminated exit when we killed it); the connection attempt itself neither logs an error nor becomes visible on the bus, with no explanation left in GLib's own message log -- next diagnostic (not yet built) adds `G_DBUS_DEBUG=all` to see the actual wire-level authentication handshake |
