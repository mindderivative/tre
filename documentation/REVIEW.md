...
**Status: fixed and shipped for `ResizeStrategy::Coarse` in `resize_test.rs`.** This eliminates the compositor-resample blur/jitter finding #235 first identified and every subsequent bucket-tuning pass (round-to-nearest, 32px) could only ever cap, not remove -- zero resample now happens during an active drag, not merely a smaller one. `DRAG_COARSE_STEP`/`SETTLE_FRAMES` (governing how *often* the swapchain itself rebuilds, finding #237's own separate DMA-BUF-import-stall concern) are untouched by this change and remain exactly as shipped. **Not yet ported to `crates/tre-python/src/windowed_renderer.rs`** (the plan's own next step, pending a separate verification pass against the 300-shape live scene via `maturin develop --release`) -- `windowed_renderer.rs` still uses `submit_frame_with_logical_size` unchanged for now. Windows (`IDXGISwapChain2::SetSourceSize`) and macOS (`CAMetalLayer`'s `presentsWithTransaction`/`waitUntilScheduled`) remain deliberately deferred, real-but-different techniques, exactly as already disclosed above -- this fix is Linux/Wayland-only, gated implicitly by `winit`'s own existing per-platform `WindowExtWayland` availability (a no-op, not an error, on X11 or if the compositor never advertised `wp_viewporter` at all).
