...
**Second correction (same day, after a third real push):** the 30s timeout was still not enough -- `canvas_accessibility_demo` in `vulkan-validation` timed out completely (a real, full 30s timeout, not a near-miss), while `test`'s own round-trip test (same 30s bound, unrelated job) again succeeded at ~10.05s. This rules out "needs a slightly longer timeout" entirely: the discovery either completes in ~10s or, in `vulkan-validation` specifically, never completes at all. The one real, load-bearing difference between the two jobs: `vulkan-validation` runs 20 other CPU-bound, software-rendered (lavapipe) Vulkan demos before ever reaching this one, on a shared, resource-constrained hosted runner, while `test` runs nothing else CPU-heavy first. **Real fix:** moved `canvas_accessibility_demo` out of `vulkan-validation` entirely into its own new, minimal `accessibility-validation` job -- installing the same Vulkan/D-Bus packages but running only this one command, mirroring the exact "lightweight, uncontended job" shape that has now succeeded reliably twice in `test`. `vulkan-validation` itself reverts to installing only what its own remaining 20 demos need (no D-Bus packages).

## Summary table (Phase 5 Step 5.3.3 CI Verification)

| # | Finding | Doc(s)/Code | Severity | Resolution |
|---|---|---|---|---|
| 126 | `canvas_accessibility_demo` never completed AT-SPI2 discovery when run as the last of 21 commands in the CPU-heavy `vulkan-validation` job, even at a 30s timeout, while the lightweight `test` job's own round-trip test succeeded reliably at ~10.05s both times (root cause of the ~10s baseline delay itself not conclusively identified; `IsEnabled` directly ruled out via two real CI runs) | .github/workflows/ci.yml, tre-a11y, tre-rhi-vulkan (example code) | Should-fix | Fixed — moved the demo into its own new, minimal `accessibility-validation` job, matching `test`'s own proven-reliable uncontended shape; discovery timeouts also widened 10s→30s as real margin |
