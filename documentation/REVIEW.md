...
**Extending the index once more (REVIEW.md finding #242: this paragraph had again gone stale, this time by 37 findings, the same recurring gap finding #145 already named once before).** **"CI Fully Broken Since Phase 11/Phase 14" (2026-09-12), below, is the newest section at the time** -- a from-first-principles investigation, at the user's own instruction to "check the CI run," found three real, stacked regressions that had silently broken nearly every CI job for a long stretch of real work (findings #206-208): `libgtk-3-dev`/`libayatana-appindicator3-dev` missing since Phase 14 Step 14.4's tray-icon dependency; `libxkbcommon-x11-0` missing since Phase 11 Step 11.1's `winit` migration, plus the `test` job never running under a virtual display at all; and a genuine Vulkan validation-layer/driver version-skew false positive on `VK_KHR_dynamic_rendering_local_read`, fixed at its real root only on a third iteration (stop *using* the unvalidatable extension on an affected runner, rather than an ever-growing list of message-matched exceptions) after two earlier attempts were each found insufficient by an actual CI round-trip, not local reasoning. A `/review-project` pass immediately after (findings #209-220) fixed a real GPU-memory leak (#212: `PyShapeRegistry` never freed a `Texture`), two real security-hardening gaps (#214: `tre-svg`'s SMIL parser had no untrusted-input hardening; #218: `tre-ffi`'s C-ABI never got the input-hardening pass `tre-python` already had), a real architecture generalization (#216: both `tre-python` renderers moved off concrete Vulkan types onto `RhiDevice`/`RhiSwapchain` trait objects), a real CI gap (#217: the `test` job had no Vulkan device at all, so `tre-ffi`'s own C harness test never actually ran there), and five smaller real fixes (#209, #211, #213, #215, #219-220) -- all fixed and re-verified. Findings #221-222 were left deliberately disclosed-not-fixed at the time (a `Text` shape's animation-triggered full re-shape cost; `execute_frame`'s per-frame `clip_stack` allocation) -- both are since resolved (see below). "`frame_processing` CI Fixes" (findings #223-224, #226-227) chased a real, still-open CI perf-budget gate through several honest iterations, including a genuine measured regression on CI's own hardware despite a real local improvement (#227) -- ending with the gate's own root cause (a cold-start-vs-steady-state measurement mismatch) fixed for real. **Finding #228 (Explicit Go-Ahead)** applied #222's own clip_stack fix workspace-wide, across all ~30 real call sites, in one dedicated pass once the project owner authorized touching that many call sites at once.

**The "Interaction-Driven Resize Test" investigation (findings #229-238), below, is this document's own longest single throughline** -- a real, live, project-owner-tested resize-drag investigation spanning a crash fix (#230: `DeviceLost` on the first real interactive resize, root-caused to a Wayland `wp_fifo_manager_v1` conflict from recreating the whole surface on every resize), a redundant-rebuild fix (#231), a surface-vs-swapchain architecture fix (#232, explicit go-ahead: resize never needs to recreate the surface, only the swapchain), two confirmed-not-a-bug/real-bug pairs (#233-234: resizing itself never squashes content; `tre-python`'s own windowed resize path was a real, separate, purely-reactive squash bug, fixed), and finding #235's own "trails then jumps" symptom -- isolated via split acquire/present timing to `vkAcquireNextImageKHR` itself blocking for hundreds of milliseconds, shipped as `ResizeStrategy::Coarse` (a coarse-bucketed swapchain during an active drag, content still projected at the true logical size so the resulting mismatch reads as a compositor resample rather than visible squash/stretch), then further investigated across several honest dead ends (round-to-nearest bucketing reverted for flipping the resample direction mid-drag; smaller bucket sizes tested and ruled out for trading resample amplitude against real added lag) before finding #237 pinned the underlying mechanism down precisely via a real `WAYLAND_DEBUG=1` protocol trace (compositor-side DMA-BUF import cost on every swapchain recreation, not GPU work or client-side pacing -- `oldSwapchain` was tried as a candidate fix, kept for its own correctness merits despite a measured-null result) and finding #238 finally eliminated the residual resample/jitter for real: a small, forked-and-patched `winit` exposing the `wp_viewport` crop it already owns, paired with a corrected render path (crop, not stretch) after a first attempt at combining the two made the jitter measurably worse, not better -- confirmed via two separate real live-drag tests, shipped in both `resize_test.rs` and `tre-python`'s own `windowed_renderer.rs`. Finding #236 (no viewport/frustum culling anywhere in `flatten_into`) was raised mid-investigation and deliberately flagged as future optimization work, not fixed, immaterial at every scale this codebase has actually measured. Finding #239 designed (not built, no code changed) how a future DX12 backend's own `IDXGISwapChain2::SetSourceSize` would integrate with the same crop contract, once `tre-rhi-dx12` is a real backend at all.

Findings #240-241 closed the two real, previously-disclosed-but-deferred RHI-fallibility gaps this same document's own earlier audit passes had flagged and explicitly left open pending a trait-signature change: #240 widened `RhiCommandBuffer::apply_layer_blur`/`RhiDevice::create_dynamic_ring_buffer` (finding #189) to return `Result` instead of panicking internally; #241 widened `execute_frame` itself (finding #141) the same way, across all 34 real call sites workspace-wide, closing the exact gap #240 had just run into and could not itself close. **Finding #242** worked through this document's own then-current open-findings punch list in order (#54, #188, #221, #236, #239): two real fixes (a panicking swapchain-format fallback and an input-queue doc-honesty gap; field-level doc comments on the five example types finding #188 itself named), one item rediscovered already fixed under a different, uncross-referenced finding number (#221, resolved by #225), and two confirmed already at their correct, previously-requested disposition (#236, #239) rather than built further without new direction.

**Extending the index a third time (2026-09-13): findings #243-246, caught proactively this time -- only 4 findings behind, not waiting for the gap to grow the way findings #145/#242 each did before this one.** A fresh, full four-lens `/review-project` pass across the entire workspace (finding #243) found Security clean, one real Performance fix (an animated wrapped `Text` shape's word-wrapping was recomputing every frame even on a shaping-cache hit -- `TextShapeCache` gained a matching `wrapped` cache), one real Architecture documentation gap (`ARCHITECTURE.md` never got an annotation for finding #235/#238's four newest RHI trait additions -- added), and three items flagged rather than fixed opportunistically, all resolved at the project owner's own explicit follow-up direction: finding #244 unified `begin_frame_with_timeout`/`begin_frame_with_viewport_crop` into one composable `begin_frame_with_options` (`VulkanDevice::begin_frame_impl` already treated the two as independent internally; only the public trait forced a choice); finding #245 deduplicated `PyWindowedRenderer::new`/`WindowSlot::create`'s construction logic via a new shared `WindowSlot::from_swapchain`, without reintroducing the direct `ash` dependency findings #203/#204 had deliberately removed; finding #246 bumped `glam` to its real newest MSRV-compatible version (`0.33.7`) and, on real investigation, found `pyo3` was already at its own MSRV ceiling (`0.27.2`, since every release from `0.28.2` onward requires Rust 1.83 against this workspace's `1.75`) rather than genuinely behind. Also folded into this same pass: finding #126 (the long-open `accessibility-validation` CI investigation, "STOPPING POINT (nineteenth push...)" below) was discovered to have already been fixed in Phase 18 Step 18.1, 2026-09-11 -- well before this preamble's own prior update -- with this document's own stale "stopped, unfixed" conclusion corrected to match, confirmed against a real, current CI run rather than trusted from the historical record alone.

**Extending the index a fourth time (2026-09-13): finding #247, a fresh full four-lens `/review-project` pass -- caught at 1 finding behind, the smallest gap yet.** Security examined the one area finding #243 had explicitly left for it (`tre-python`'s runtime GLSL-to-SPIR-V custom-shader path) and found one real hardening gap, fixed (no size cap on caller-supplied shader source, and the `shaderc` compile ran with the GIL held -- now capped at 1 MiB and compiled under `py.detach`, verified live), plus one genuine design-level exposure flagged for the project owner (a caller's own GLSL is linked against the engine's one universal pipeline layout and can index the shared bindless texture array out of range; no `robustBufferAccess`/`robustImageAccess` is requested at device creation, so that is undefined behavior per the Vulkan spec, not a defined black texel). Performance found one residual per-frame allocation one step downstream of finding #243's own wrap cache (one `Vec` per visual line, every frame, even on a full cache hit -- fixed via a new slice-taking `RenderingCanvas::draw_glyphs` primitive `draw_text` now delegates to). Architecture found the pass's most consequential item: the workspace's `rust-version = "1.75"` is demonstrably false, not merely unverified -- `cargo +1.75.0 check` fails on the first `reason = "..."` lint attribute (stable only since 1.81, used 151 times across 42 files), no CI job ever builds below the pinned 1.98.0, and the resolved `Cargo.lock` itself cannot even be parsed below 1.85 (`clap 4.6.6`, via `cbindgen`'s build dependency, declares `rust-version = 1.85`). Every deliberately-frozen dependency in the workspace (`pyo3`, `usvg`, `rustc-hash`, `skrifa`, `accesskit`, `wide`) cites 1.75 as its sole justification, and finding #246's own "raising the MSRV remains open" conclusion was reached against a floor the code has not satisfied for some time -- flagged, with the real numbers, for the project owner's decision rather than changed unilaterally, since #246 explicitly reserved it. Architecture's mechanical items were fixed: `ARCHITECTURE.md` Section 6 still described the two RHI methods finding #244 deleted (annotated -- the second consecutive break of the every-trait-change-gets-an-annotation convention #243 had just repaired); three `VK_KHR_dynamic_rendering_local_read`-shaped `RhiDevice`/`RhiSwapchain`/`RhiCommandBuffer` methods were the only backend-varying capabilities on the traits with no default body (given `false`/`false`/no-op defaults, rename left to the owner); `tre-rhi-vulkan`, the largest and `unsafe`-heaviest crate, had zero `#[test]` functions (ten added, covering its GPU-free helpers -- including the two validation-layer string predicates that gate whether CI catches a real Vulkan error at all); and three places still claimed Vulkan was the workspace's only C ABI boundary, false since the same step that wrote it added `libfontconfig` (corrected). Modernizer found no `[workspace.dependencies]` table at all -- five shared external crates were repeated as literal version strings across 22 manifest lines with nothing enforcing agreement (consolidated, behavior-neutral, `Cargo.lock` unchanged) -- plus six small idiom sites and six redundant clones in examples, all fixed. Full verification clean; workspace tests grew from 377 to 387 with every pre-existing count unchanged. See finding #247 below for the complete disposition, including the items awaiting the project owner.

**Finding #248 (2026-09-13), at the project owner's own explicit direction ("Raise rust-version to 1.85 and add an MSRV CI job"):** `rust-version` raised from the false `1.75` to `1.88` -- not the 1.85 asked for, and the reason is recorded honestly: finding #247's own report named 1.85 as "the real lockfile floor" from a bisection that stopped at 1.81, and on actually installing 1.85 the resolved `Cargo.lock` needs 1.88 (`image 0.25.10`, the `icu_*` 2.3 crates) with `fdsm 0.8.0`'s own `nalgebra ^0.34.1` requirement (MSRV 1.87 at every release) ruling out anything below 1.87 short of downgrading the MSDF generator itself, which was not asked for and not done. 1.88 is the floor the committed lockfile actually has, verified with a real `1.88.0` toolchain (`cargo check --workspace --all-targets --locked` clean), and a new `msrv` CI job reads the value straight out of `Cargo.toml`, installs exactly that toolchain (never `rustup show`, the step that hid the old violation), and re-checks it on every commit. Raising the floor also woke clippy's MSRV-gated lints: 47 sites across 44 files moved to `slice::as_chunks`/`as_chunks_mut` (stable 1.88), `is_multiple_of` (1.87), and `f32::midpoint` (1.85) -- the BGRA-to-RGBA swap loop in ~30 examples, two engine tessellation paths, the SVG tessellator, and `tre-text`'s glyph padder, which lost a fallible `try_into` in the process. Every manifest comment that had cited 1.75 as a pin's rationale was corrected (the `fdsm` one had claimed a compatibility that never existed); none of the now-unlockable dependency pins (`pyo3`, `usvg`, `rustc-hash`, `skrifa`, `accesskit`) was bumped, each left as its own separate decision. Full verification clean on both toolchains, 387/387 tests, two pixel-asserting Vulkan examples re-run live.

**Findings #249-#254 (2026-09-13), all at the project owner's own explicit direction, each its own commit.** Findings #249-#253 took the five dependency bumps #248's real floor unlocked, in rising order of risk: `rustc-hash` 2.1.3 (#249), `usvg` 0.48.1 (#250), and `skrifa` 0.47.0 (#251) needed zero source changes and were proven by clippy, the full suite, the 1.88 locked check, and pixel-asserting Vulkan demos; `accesskit` 0.25.0/`accesskit_unix` 0.23.0 (#252) needed real ones (`Tree` became `TreeInfo` and lost `app_name`, `TreeUpdate` gained `tree_id`, the blocking `zbus` dev-dependency had to move from 4 to 5 with it, and the AT-SPI2 object-path scheme changed to `(node_id << 64) | tree_index` -- caught only by the live D-Bus round trip, decoded in the test and the verify example with the source cited); `pyo3` 0.29.2 (#253) made by-value extraction opt-in for all 25 `Clone` pyclasses, sorted mechanically by letting the compiler enumerate the 14 Python actually passes by value. Finding #254 then closed the gap #253's own verification exposed -- CI had never built or run the Python extension at all, so all 30 demos had run only locally -- with a new `python` CI job that builds the real wheel via `maturin`, installs it, and runs every `demo/*/demo.py` on the software-Vulkan/Xvfb/D-Bus recipe the Rust jobs already proved (one demo skipped with its reason stated: it asserts a native file dialog stays open, which needs a live XDG portal). Rehearsed locally through the same wheel-install path: 29 of 30 demos pass, the 30th skipped by design. One real local lesson recorded in #254: never rehearse the AT-SPI2 demo inside a nested `dbus-run-session` on a live desktop -- its launcher recreates the session's own a11y socket path and leaves the live bus unreachable until the user unit is restarted.
...
**Both fixed: see Finding #242 below.**
...
**Current state, explicitly (superseded -- see the resolution note immediately below):** `accessibility-validation` still fails for real (the gate is genuine, not `continue-on-error`). The bug is understood and precisely located; the fix is not implemented. Per explicit user instruction, this investigation stops here rather than attempting and pushing that fix now. Resuming it later should start by implementing the session-bus fix above in both `canvas_accessibility_verify.rs` and `tre-a11y/tests/round_trip.rs`, and directly checking whether the `test` job's round-trip test was ever really passing or silently skipping.

**Resolved (Phase 18 Step 18.1, 2026-09-11 -- picked back up at the project owner's own later explicit direction, "yes, pick it back up"): the session-bus fix above was implemented in both files exactly as prescribed.** `ensure_accessibility_enabled` now builds its `org.a11y.Status` proxy on a real, separate `Connection::session()` connection instead of the a11y-bus connection it previously (wrongly) reused. Real, measured effect on the development machine: `tre-a11y`'s round-trip test went from racing a 10s timeout (previously ~10.05s, uncomfortably close) to completing in 0.11s -- confirming `IsEnabled` was true instantly all along once queried on the correct bus, and that every prior correction in this investigation (widening timeouts, splitting processes, retrying `RegisterEvent`) was chasing a symptom the wrong-bus bug alone fully explains. The "one important loose end" flagged at the stopping point -- whether `tre-a11y`'s own round-trip test had been silently skipping rather than genuinely passing -- is answered by this same measurement: it was genuinely passing, just uncomfortably close to its own timeout, exactly as the investigation's very first evidence (10.06s against a 10s budget) already suggested. See `IMPLEMENTATION.md`'s own Phase 18 Step 18.1 entry for the full account, including Step 18.2's follow-up finding that the "process split" this whole investigation's own design assumed was structurally necessary (the stale theory that `accesskit_unix` registration "never completes inside a process that also links Vulkan/X11") was never actually true for a real application -- only a real, separate, disclosed reason (matching actual AT-SPI2 deployment practice) to keep *verification* in its own process. **Confirmed against a real, current CI run (`gh run view`, run `34786829153`, 2026-09-13) that `accessibility-validation` genuinely passes today** -- not just re-asserted from the historical record, and Step 18.3 has since wired this bridge into `tre-python` itself and built real, passing demos on top of it.

## Summary table (Phase 5 Step 5.3.3 CI Verification)

| # | Finding | Doc(s)/Code | Severity | Resolution |
|---|---|---|---|---|
| 126 | `ensure_accessibility_enabled` (`canvas_accessibility_verify.rs` and `tre-a11y/tests/round_trip.rs`) queries `org.a11y.Status`'s `IsEnabled` property on the **a11y bus** connection, but `at-spi-bus-launcher` owns `org.a11y.Bus`/`IsEnabled` on the **session bus** (`g_bus_own_name(G_BUS_TYPE_SESSION, ...)`, confirmed directly in its real source) -- every `get_property` call therefore fails outright (wrong destination bus), silently collapsed to `false` by `.unwrap_or(false)`, indistinguishable from a real "not enabled" reading. Confirmed via a plain `dbus-send --session ... Properties.Get` returning `true` in a fresh session where our own code still saw `IsEnabled` as false | tre-a11y/tests/round_trip.rs, tre-rhi-vulkan/examples/canvas_accessibility_verify.rs | Should-fix | **Fixed (Phase 18 Step 18.1, 2026-09-11)** -- session-bus fix applied to both files exactly as identified; `tre-a11y`'s round-trip test went from a ~10.05s near-timeout to 0.11s; `accessibility-validation` confirmed genuinely passing on a real, current CI run (2026-09-13). Originally stopped, unfixed, per explicit user instruction, and picked back up later at the project owner's own explicit direction |
...
**Partially fixed: see Finding #242 below** -- the exact five named example types (plus their immediately-adjacent enum/companion types) now have field-level doc comments; the "dozens more" across the rest of the workspace remain deliberately out of scope, for the identical proportionality reason this entry already gave.
...
## CI Fully Broken Since Phase 11/Phase 14, Undiscovered Until Now (2026-09-12)

At the user's own explicit instruction ("check the CI run"), investigated why the push landing the new MkDocs documentation site showed a failed CI run -- and found the failure had nothing to do with that push at all. Reading back through the full run history (`gh run list`/`gh run view` against every push since Phase 7) surfaced three real, stacked, previously undisclosed regressions that had silently broken nearly every CI job for a long stretch of real work, none of it caught because nobody had checked a CI run's own job-level detail (only its pass/fail badge) in that window.

### 206. [Fixed] `libgtk-3-dev`/`libayatana-appindicator3-dev` missing from every job's install list since Phase 14 Step 14.4

Phase 14 Step 14.4 added `tray-icon`/`gtk` as a direct `tre-platform` dependency (system tray + native menu). Since `tre-platform` underlies nearly every crate and example in the workspace, this broke `clippy`/`build`/`test`/`vulkan-validation`/`accessibility-validation` identically, at the same `glib-sys`/`libappindicator-sys` build-script pkg-config lookup (`Package glib-2.0 was not found in the pkg-config search path`) -- before any of those jobs reached real Rust code at all. Confirmed via `Cargo.lock`: `libappindicator-sys` 0.9.0 (pulled in even with `tray-icon`'s own `default-features = false`, since `libappindicator` is not itself an optional feature of that crate) probes `ayatana-appindicator3-0.1` first, which Ubuntu 24.04's `libayatana-appindicator3-dev` package provides (the legacy `libappindicator3-dev` name this crate's own fallback expects is not present in that runner's repos). `libgtk-3-dev` alone provides `glib-2.0.pc` transitively (plus cairo/pango/gdk-pixbuf/atk). Also confirmed, not assumed: `libdbus-1-dev` is NOT needed for any of this -- `tre-a11y` binds AT-SPI2 via `zbus`, a pure-Rust D-Bus implementation with no system `libdbus` link requirement, verified directly against `Cargo.lock` (only `zbus` appears, no `dbus`/`libdbus-sys`).

Fixed by adding both packages to the `apt-get install` line in every affected job (`.github/workflows/ci.yml`).

### 207. [Fixed] `libxkbcommon-x11-0` missing since Phase 11 Step 11.1's winit migration, and the `test` job never ran under a virtual display

Fixing #206 let `build`/`clippy` compile successfully for the first time since Phase 14 -- which immediately surfaced two further, separate, previously undisclosed gaps that #206's own build failure had been masking:

- Every job that actually *runs* a compiled binary touching a window (`vulkan-validation`, `accessibility-validation`, and now `test`, once #206 was fixed) panicked with `Library libxkbcommon-x11.so could not be loaded.` -- winit's own X11 backend `dlopen()`s `libxkbcommon-x11.so.0` at runtime (confirmed via `xkbcommon-dl`'s own source: `open_with_sonames(&["libxkbcommon-x11.so.0", "libxkbcommon-x11.so"], ...)`), never a link-time dependency, so nothing in the existing `apt-get install` lines provided it. Confirmed present since Phase 11 Step 11.1 itself (2026-09-10), a full day before finding #206's own build break -- read that same day's own CI run log directly and found the identical panic there, not assumed from the mechanism alone.
- The `test` job's own `dbus-run-session -- cargo test --workspace` never ran under a virtual display at all. This was invisible until #206 was fixed, because `tre-platform`'s own real clipboard/tray unit tests (added Phase 14 Step 14.1/14.4) never got far enough to run before that build failure. Once #206 was fixed, they failed for real: `X11 server connection timed out` (clipboard) and `Failed to initialize GTK` (tray).

Fixed by adding `libxkbcommon-x11-0` to every job that runs a compiled binary (`test`, `vulkan-validation`, `accessibility-validation`), and wrapping the `test` job's `cargo test --workspace` in `xvfb-run -a`, matching the pattern `vulkan-validation`/`accessibility-validation` already established for their own example runs.

### 208. [Fixed] Vulkan validation-layer/driver version skew produces a false-positive `VUID-VkDeviceCreateInfo-pNext-pNext` abort

Fixing #206 and #207 let `vulkan-validation`/`accessibility-validation` reach real Vulkan device creation for the first time in this same investigation -- which surfaced a third, genuinely different kind of issue: a real `VK_EXT_debug_utils` ERROR-severity message,

```
VUID-VkDeviceCreateInfo-pNext-pNext | MessageID = 0x901f59ec |
vkCreateDevice(): pCreateInfo->pNext chain includes a structure with unknown
VkStructureType (1000232000). This error is based on the Valid Usage
documentation for version 275 of the Vulkan header. It is possible that you
are using a struct from a private extension or an extension that was added
to a later version of the Vulkan header...
```

`vulkan_debug_callback` (IMPLEMENTATION.md Step 2.4's own deliberate policy) calls `std::process::abort()` on any ERROR-severity message, so this aborted every real GPU example on that runner. Confirmed, not assumed, that this is a real validation-layer/driver version-skew false positive rather than a genuine bug in this engine's own `pNext` chain construction: `1000232000` is a real, legitimate struct type a newer `mesa-vulkan-drivers` build reports, which Ubuntu's own separately-versioned `vulkan-validationlayers` package (pinned to an older Vulkan header revision) doesn't yet recognize -- the two apt packages drift out of lockstep as Ubuntu updates each on its own schedule. The validation layer's own message says as much directly.

At the user's own explicit instruction ("Special-case it as a known false positive"), added a narrowly-scoped exception directly in `vulkan_debug_callback` (`crates/tre-rhi-vulkan/src/lib.rs`): a new `is_known_false_positive` helper, initially matching on the exact VUID string (`VUID-VkDeviceCreateInfo-pNext-pNext`) plus `"unknown VkStructureType"`.

**Real, second iteration, found only by actually re-running CI after deploying the first fix, not assumed sufficient from local reasoning alone.** The first fix correctly stopped that one `ERROR` from aborting -- but the process then continued past it and hit a second, different `ERROR` VUID from the identical root cause: a `WARNING`-severity message emitted by this same validation layer, right alongside both `ERROR`s, states outright `vkCreateDevice(): pCreateInfo->ppEnabledExtensionNames[3] VK_KHR_dynamic_rendering_local_read is not supported by this layer` -- confirming the validation layer genuinely doesn't know about that real, legitimate, hardware-gated extension (Step 10.2.3, REVIEW.md finding #170) at all, so it also doesn't recognize the layout token that extension's own dependency chain defines (`1000232000`, `VK_IMAGE_LAYOUT_ATTACHMENT_FEEDBACK_LOOP_OPTIMAL_EXT`) wherever it later appears:

```
VUID-VkImageMemoryBarrier-newLayout-parameter | MessageID = 0x43f5a56f |
vkCmdPipelineBarrier(): pImageMemoryBarriers[0].newLayout (1000232000) does
not fall within the begin..end range of the core VkImageLayout enumeration
tokens and is not an extension added token.
```

Broadened the match to the shared root: the literal numeric token `1000232000` plus either of the validation layer's own self-diagnostic phrasings for "I don't recognize this value" (`"unknown VkStructureType"`, `"does not fall within the begin..end range"`, or `"extension that was added to a later version of the Vulkan header"`) -- still narrow (a real, different Vulkan misuse this engine actually introduced would not happen to cite this exact, specific enum/struct-type integer), but anchored on the actual shared cause rather than one single VUID string that under-covered its own real downstream fallout. The message is still printed either way; only the abort is skipped for a confirmed match.

**Real, third iteration, found only by actually re-running CI after deploying the second fix.** The broadened match also proved insufficient. The fourth CI round-trip (run `34678472934`) showed `local_read_supported`'s entire code path cascading through *four* real validation messages in the one `walking_skeleton` process, in order: the original `VUID-VkDeviceCreateInfo-pNext-pNext` (suppressed correctly), `VUID-VkImageMemoryBarrier-newLayout-parameter` (suppressed correctly), `VUID-VkRenderingAttachmentInfo-imageLayout-parameter` and a `VUID_Undefined` at `vkUpdateDescriptorSets()` (both still cited `1000232000` in the same textual form, so both were also suppressed correctly) -- and then a fifth, structurally different message that finally aborted the process:

```
VUID-VkWriteDescriptorSet-descriptorType-04151 | MessageID = 0x62405e2d |
vkUpdateDescriptorSets(): pDescriptorWrites[0].pImageInfo[0] Descriptor update
with descriptorType VK_DESCRIPTOR_TYPE_INPUT_ATTACHMENT is being updated with
invalid imageLayout Unhandled VkImageLayout for image ... Allowed layouts are:
VK_IMAGE_LAYOUT_DEPTH_STENCIL_READ_ONLY_OPTIMAL, ...
```

This message never restates `1000232000` in its own text at all -- it just says "Unhandled VkImageLayout" -- so no text-matching filter anchored on that token could ever have caught it. This was the moment the pattern broke down for good: the real problem was never any *one* VUID string, or even the shared numeric token: it was that `local_read_supported` being `true` on this runner made the engine actually *exercise* `VK_KHR_dynamic_rendering_local_read` (real `INPUT_ATTACHMENT` images, descriptors, barriers) against a validation layer that cannot check any of it correctly, so *every* real Vulkan call touching that path was a potential new false positive -- an open-ended set no finite list of matched strings could ever fully enumerate, confirmed in practice by needing a new match clause after each of the last three CI round-trips.

At the user's own explicit follow-up instruction ("Don't request the extension when the validation layer doesn't support it"), replaced the whole message-matching strategy with a root-cause fix: stop *using* the extension at all on a runner whose validation layer cannot validate it, rather than continuing to special-case each new symptom of using it anyway. `VulkanDevice::new` already gated its entire local-read/advanced-blend code path (device extension, feature struct, `blend_read` descriptor set, swapchain `INPUT_ATTACHMENT` usage flags, barriers -- every one of the four call sites above) behind one single `local_read_supported: bool`, previously set purely from whether the *driver* advertises the extension. The fix teaches that same bool about the validation layer too:

- A new `DebugCallbackState { local_read_rejected: AtomicBool }`, boxed and registered as the debug messenger's own `pUserData` (per-`VulkanDevice`, not a global/static, since a global would race across concurrent `VulkanDevice::new()` calls under `cargo test`'s parallel test threads).
- `vulkan_debug_callback` now also checks every message (`is_local_read_rejected_by_layer`) against the validation layer's own two self-diagnostics for this extension -- the `WARNING` ("...VK_KHR_dynamic_rendering_local_read is not supported by this layer") and the resulting `ERROR` (`VUID-VkDeviceCreateInfo-pNext-pNext`, "unknown VkStructureType (1000232000)") -- and if either fires, records it on that state, independent of the existing abort-on-ERROR logic.
- Immediately after `create_device()` returns (the same call that would have triggered either diagnostic, since the feature struct is chained into `pNext` speculatively), `VulkanDevice::new` checks that flag and downgrades its local `local_read_supported` to `false` if the layer rejected it -- before any later code (the `blend_read` setup, swapchain `INPUT_ATTACHMENT` usage flags) reads that bool. The device keeps the extension harmlessly enabled (Vulkan doesn't punish an unused enabled extension), but the engine never touches an `INPUT_ATTACHMENT` image, descriptor, or layout again for the rest of the process -- so none of the four downstream VUIDs above (nor any *other* message that same unvalidatable path might have produced) can fire again.
- `is_known_false_positive` shrank back down to matching exactly the one, single, unavoidable false positive left: the `VUID-VkDeviceCreateInfo-pNext-pNext` complaint from the one `create_device` call that has to speculatively request the extension before it can find out whether the layer accepts it. Every other exception clause from the second iteration was removed -- they exist to suppress abort on messages that, after this fix, no longer occur at all.

**Verified.** `cargo fmt --all -- --check`/`cargo clippy --workspace --all-targets -- -D warnings`/`cargo build --workspace --all-targets` and the same three commands in `--release` all clean on `tre-rhi-vulkan` and the full workspace; `cargo test --workspace` fully green (all real test crates, 0 failed). `walking_skeleton` re-run locally (a real desktop where the validation layer/driver pair is in sync, so `local_read_rejected` never gets set and the normal local-read/advanced-blend path runs exactly as before) exits cleanly with no behavioral change. Findings #206-207 confirmed fixed via two full, separate CI round-trips on `main` (not just inspected locally): `clippy`/`build`/`rustfmt` went green, and `test` ran cleanly all the way through `cargo test --workspace`/the MSDF demo/the criterion bench, stopping only at the already-known, already-disclosed #159 perf-budget gate (intentionally red, unrelated to this investigation) -- confirmed by comparing against a pre-Phase-14 run where the same two gates (`accessibility-validation`'s #126, `test`'s own #159) were the *only* red gates, matching this document's own prior disclosed state exactly. Finding #208's own first and second fix attempts were each confirmed insufficient only by a real CI round-trip, never by local reasoning alone -- the exact pattern that motivated abandoning message-matching for a root-cause fix this third time. This third fix still needs a fifth CI round-trip, once pushed, to confirm `vulkan-validation`/`accessibility-validation` finally reach finding #126's own still-open, separately-tracked root cause rather than yet another new failure -- but unlike the prior two iterations, there is no longer a specific *further* VUID this fix would need to anticipate: the entire code path the outdated validation layer cannot check is now simply never exercised on a runner where it was rejected.

## Finding #209 Fixed: `tre-ffi`'s C Test Harness Located the Wrong `target/` Directory on GitHub Actions

Phase 10 Step 10.3's first CI round-trip (run `34687733428`, commit `9ddff9b`) surfaced a real environment difference this crate's own new C test harness (`crates/tre-ffi/tests/c_harness.rs`) had not accounted for: the `test` job's own `cargo test --workspace` step failed outright (not merely at the already-disclosed #159 perf-budget gate) with

```
thread 'c_harness_exercises_the_real_ffi_surface' panicked at crates/tre-ffi/tests/c_harness.rs:47:5:
expected .../libtre_ffi.a to exist -- tre-ffi's [lib] crate-type must still include "staticlib"
```

despite `cargo build -p tre-ffi` genuinely producing that file (confirmed locally, and nothing in this same CI run's own `build`/`clippy` jobs suggested otherwise). The root cause: `target_profile_dir()`'s original implementation located the build output by walking exactly two directories up from `std::env::current_exe()` (`target/<profile>/deps/<test-binary>` -> `target/<profile>/`) -- a real, commonly-used trick for exactly this "a plain `#[test]` has no `OUT_DIR`-equivalent" problem, and one that worked correctly on this project's own local development machine, but did not hold on GitHub Actions' own runner layout for this same command.

Fixed by trying two independent, more robust strategies before giving up: first, the workspace's own fixed structural relationship (`tre-ffi` lives at `<workspace_root>/crates/tre-ffi`, so `<workspace_root>/target/{debug,release}` is derivable directly from `CARGO_MANIFEST_DIR` without any assumption about test-binary path depth); second, a bounded upward walk from `current_exe()` checking every ancestor directory (and its own `target/{debug,release}` subdirectory) for the real built artifact, rather than assuming one exact depth. Both strategies check for the actual presence of `libtre_ffi.so` before accepting a candidate directory, so neither can silently succeed against a stale or wrong location.

**Verified.** `cargo fmt --all -- --check`/`cargo clippy --workspace --all-targets -- -D warnings`/`cargo build --workspace --all-targets`/`cargo test -p tre-ffi` all clean locally, including a fresh, non-cached local run of `c_harness_exercises_the_real_ffi_surface` itself. This fix still needs a real CI round-trip, once pushed, to confirm it actually resolves the correct directory on GitHub Actions' own runner -- the same "only a real CI round-trip proves an environment-specific fix" pattern this document has established repeatedly (findings #206-208).

## Finding #210 Fixed: `render()`/`render_canvas()`/`render_parallel()` Allocated a Fresh Canvas/`FrameArena` Every Frame

A `/review-project` full-workspace review's Performance lens found that every real, user-facing render entry point -- both `PyHeadlessRenderer`/`PyWindowedRenderer` methods in `tre-python` and `tre-ffi`'s own `render()` -- called `RenderingCanvas::new()` (or `std::mem::replace(&mut canvas.inner, RenderingCanvas::new())`) on every single call, instead of the crate's own established zero-allocation-reuse convention: build one `RenderingCanvas` once, then call `reset()` on it every subsequent frame (Phase 9 Step 9.2, REVIEW.md finding #134). `render_parallel` compounded this by also building a fresh `FrameArena::with_capacity(...)` (4 `ScatterArena` allocations) on every call. The project's own `RenderTickGuard`/`DebugAllocGuard` enforcement (which would have caught this) is wired into exactly two example programs (`main_loop_demo.rs`, `shape_registry_zero_alloc_demo.rs`) and nowhere in `tre-python`/`tre-ffi` -- the code paths a real Python UI application or C caller actually drives every frame -- so the gap had been invisible to the project's own guardrails since Step 10.4 first shipped these entry points.

The subtlety this fix had to account for: `RenderingCanvas::flatten(self)` genuinely consumes `self` (it destructures `vertices`/`indices`/`commands`/`accessibility_nodes` out of the struct), so simply replacing `RenderingCanvas::new()` with `reset()` before an eventual `.flatten()` call cannot work -- the canvas would still be thrown away at the consuming call. The only real non-consuming path already proven in this codebase is `shape_registry_zero_alloc_demo.rs`'s own sequence: `canvas.reset()` -> `registry.flatten_into(&mut canvas, ...)` -> `canvas.stitch_into(&arena)` (a non-consuming `&self` merge into a persistent `FrameArena`) -> `arena.flatten_into(&mut flattened)` (`FrameArena`'s own non-consuming sibling of `flatten()`, which drains-and-resets each internal `ScatterArena` into a persistent, reused `FlattenedFrame`). That demo used this pattern with a worker count of zero specifically to prove it works without real thread-based parallelism; this fix is that same proven pattern's first real production caller.

**Fixed**, across all three affected entry points:

- `tre-ffi`'s `Renderer` struct gained persistent `canvas: RenderingCanvas`/`arena: FrameArena`/`flattened: FlattenedFrame` fields (built once in `build_renderer`); `render()` now resets `canvas`, flattens the registry into it, `stitch_into`s it into `arena`, and drains `arena` into `flattened` every call, instead of constructing all three fresh. A new, small, fixed `arena` capacity (65536 vertices / 131072 indices / 8192 commands, no accessibility since this crate's own bounded first slice does no text) mirrors `tre-python`'s own established capacity/tradeoff.
- `tre-python`'s `PyHeadlessRenderer` and `PyWindowedRenderer` each gained the identical three persistent fields. `render()` now reuses its own private `scratch_canvas` (built once) instead of constructing a throwaway `PyCanvas` Python object every call; `render_canvas()` now `stitch_into`s the caller-supplied canvas (rather than consuming it via `flatten()`) and then `reset()`s it in place, truly fulfilling that method's own long-standing doc-comment promise ("leaving it freshly empty... so the same Python `Canvas` can be reused next frame") for the first time; `render_parallel()` now stitches into the same persistent `frame_arena` instead of building a fresh one. The `flatten_into` logic shared by `render`/the public `flatten_into` pymethod was factored into a private `Self::flatten_registry_into` associated function (not a method) specifically so it can take `&mut self.scratch_canvas` as one argument alongside `&self.device`/`&mut self.text_atlas` (other fields of the same `self`) as independent, disjoint borrows -- a plain `&mut self`-receiver method could not do this, since `scratch_canvas` is itself a field of `self`. `submit_and_read_bgra`/`submit_frame_to_window` were changed from taking `frame: &FlattenedFrame` as a parameter to reading `self.flattened` directly, for the identical reason (a caller cannot pass `&self.flattened` as an argument to a method whose receiver is `&mut self`). The three `PARALLEL_ARENA_*_CAPACITY` constants (previously `render_parallel`-only) were renamed `RENDER_ARENA_*_CAPACITY` and their doc comment widened, since all three render paths now share them.
- A real, disclosed behavior change this fix introduces: `render()`/`render_canvas()` on a single registry now share the same fixed-capacity `FrameArena` `render_parallel` already imposed a cap through (`RenderingCanvas::stitch_into` returns `false`, mapped to a clean `TreError`/`TreErrorCode::TransientPoolBudgetExceeded`, rather than growing mid-frame) -- previously, a single-registry scene had no such ceiling at all (`RenderingCanvas::flatten()`'s own `Vec`s grew unbounded). The chosen capacity is the same one already proven "comfortably above what a real scene needs" for `render_parallel`'s own *combined*, multi-registry case, so it is strictly more generous for the single-registry `render()`/`render_canvas()` case than it already was for the multi-registry one.

**Verified.** `cargo fmt --all -- --check`/`cargo clippy --workspace --all-targets -- -D warnings`/`cargo build --workspace --all-targets`/`cargo test --workspace` all clean in both debug and `--release` (tre-ffi's own real C harness passes in both). Beyond the test suite, rebuilt the real `tre_python` PyO3 extension (`maturin develop --release`) and ran four real, GPU-backed demos against it: Phase 10 Step 10.4's own demo (which already asserts a *second* `render()` call on the same, unmutated registry reproduces the exact same frame -- the precise regression this fix's own reuse-without-reset bug class would have broken), Phase 12 Step 12.5's `flatten_into`/`render_canvas` composition demo, Phase 12 Step 12.6's `render_parallel` demo, and Phase 12 Step 12.1's real on-screen `WindowedRenderer` demo (120 real frames rendered to a live window, plus multi-window creation) -- all four passed with exact pixel assertions unchanged. Two pre-existing, unrelated release-mode test failures (in `tre-engine` and, newly surfaced by running with `--no-fail-fast` this time, `tre-memory`) are `should_panic`/violation-detection tests whose checks are `debug_assert!`-gated and therefore compiled out under `--release` by design -- confirmed via `git status` that neither crate was touched by this fix, and left as a disclosed, unrelated gap rather than silently worked around.

## Finding #211 Fixed: `tre-rhi-vulkan/src/lib.rs` Had Grown to 5,424 Lines With No Internal Module Boundaries

The `/review-project` Architecture lens found that `tre-rhi-vulkan/src/lib.rs` -- unlike `tre-engine/src/lib.rs`, already split into `canvas.rs`/`shapes.rs`/`input.rs`/`text.rs`/`focus.rs`/`rhi.rs` at REVIEW.md finding #205 -- had never been split at all, despite containing at least ten genuinely separable concerns as top-level items in one file: `VulkanDevice` (construction/lifecycle/the `RhiDevice` trait impl), `VulkanSwapchain`, `VulkanCommandBuffer`, the Dual-Kawase blur subsystem (`PushConstants`/`BlurResources`/`create_blur_resources` and its own private helpers), `VulkanBuffer`/`VulkanPipelineState` (small opaque resource wrappers), the transient-pool/GC machinery (`TransientPool`/`TransientPoolStats`/`DeferredRelease`/`gc_thread_loop`/`BindlessRegistry`), `VulkanTexture`, and `VulkanRingBuffer` -- interleaved with no internal module boundaries at all, making it the single largest file in the workspace by real (non-generated) content.

Split into nine new files, one per concern, mirroring `tre-engine`'s own precedent exactly: `buffer.rs` (`VulkanBuffer`/`VulkanPipelineState`, grouped since each is only a handful of lines), `ring_buffer.rs` (`VulkanRingBuffer`), `blur.rs` (the Dual-Kawase subsystem -- also the one file the Security review's SAFETY-comment fix, finding #210's sibling from the same review, already touched), `transient_pool.rs` (the GC/bindless-registry machinery), `texture.rs` (`VulkanTexture`), `swapchain.rs` (`VulkanSwapchain`), `command_buffer.rs` (`VulkanCommandBuffer`, whose `apply_layer_blur` is `blur.rs`'s own real caller), and finally `device.rs` (`VulkanDevice` itself, by far the largest at ~2,560 lines but -- like `tre-engine`'s own `shapes.rs` at 4,388 lines -- kept as one file since it is genuinely one cohesive concern, not several bundled together). `lib.rs` itself is now 144 lines: the module doc, `mod`/`pub use` wiring, and the small shared constants/helpers (`align_up`, `texture_format_to_vk`, `bytes_per_pixel`, the GC/bindless/ring-buffer tuning constants) multiple modules need, exactly matching the shape `tre-engine/src/lib.rs` already established.

The mechanical challenge: Rust's field/item privacy is asymmetric -- a private item is visible to its own module and every descendant, but never to a sibling or ancestor. Moving `VulkanTexture`/`VulkanCommandBuffer`/`VulkanDevice` etc. into child modules broke every construction site left behind in the (now thinner) crate root, which had lost access to their newly-child-scoped private fields; conversely, the small shared constants/helpers left in `lib.rs` needed no visibility change at all, since every module in the crate is a descendant of the root and could already see them. Fixed by widening exactly the fields/methods actual compile errors flagged to `pub(crate)` (never fully `pub` -- external crate visibility is unchanged throughout) and re-exporting each moved type at the crate root (`pub(crate) use device::FrameSync;` etc.) so existing cross-references via `crate::X` kept resolving regardless of which file `X` physically moved to. The crate's public API (`tre_rhi_vulkan::{VulkanDevice, VulkanSwapchain, VulkanCommandBuffer, VulkanBuffer, VulkanPipelineState, VulkanTexture, VulkanRingBuffer, TransientPoolStats, HeadlessSwapchain, HEADLESS_FORMAT, register_shape_pipelines}`) is byte-for-byte unchanged -- confirmed by every existing external caller (`tre-ffi`, `tre-python`, all ~19 `tre-rhi-vulkan` examples) building against it with zero changes required on their side.

**Verified.** `cargo fmt --all -- --check`/`cargo clippy --workspace --all-targets -- -D warnings`/`cargo build --workspace --all-targets`/`cargo test --workspace` all clean in both debug and `--release` (the same two pre-existing, unrelated `tre-engine`/`tre-memory` release-mode failures from finding #210 remain, confirmed untouched by this split). `cargo build -p tre-rhi-vulkan --examples` confirmed all ~19 examples still compile unchanged against the crate's public API. Beyond compilation, ran real GPU examples covering every moved subsystem: `walking_skeleton` (device/swapchain/command-buffer construction and frame submission), `dual_kawase_blur_demo` (the blur subsystem standalone), and `layer_blur_demo` (`apply_layer_blur`, the real cross-module caller from `command_buffer.rs` into `blur.rs`) -- all passed with their own exact pixel assertions. Rebuilt the `tre_python` PyO3 extension and re-ran the Phase 10 Step 10.4 headless demo and the Phase 12 Step 12.1 windowed-renderer demo (120 real on-screen frames, multi-window) against the split crate -- both passed unchanged.

## Finding #212 Fixed: `PyShapeRegistry` Kept Every `Texture` Ever Used Alive Forever, Never Freeing GPU Memory

The `/review-project` Performance lens found that `PyShapeRegistry::textures_kept_alive` (Phase 12 Step 12.4) was a plain `Vec<Arc<Box<dyn RhiTexture>>>` that `resolve_fill` pushed a fresh clone into on every `insert_rectangle`/`insert_circle`/`insert_polygon`/`insert_path` call resolving a `Texture` fill -- and `remove()` only ever called `self.inner.remove(id.0)`, never touching this `Vec` at all. Two compounding real gaps followed: every distinct `Texture` a Python app ever used stayed GPU-resident (its real `VkImage`/`VkImageView`/`VkDeviceMemory`, plus its bindless array slot) for the entire lifetime of the registry, regardless of how many times the shape referencing it was removed -- a real, unbounded VRAM leak for any long-running app with dynamic texture content (a screenshot effect, a generated icon carousel); and even a single, repeatedly-reused `Texture` grew the `Vec` by one entry on every insert that resolved it, unboundedly, for an app inserting/removing the same texture-filled shape once per frame.

The original design's own goal -- keep a texture's GPU resource alive independent of Python's own garbage collector, since `FillStyle::Texture` stores only a raw bindless `u32`, nothing that itself holds a strong reference -- was sound; the gap was that nothing ever pruned an entry once nothing needed it anymore.

**Fixed** by keying the retained references per shape instead of appending them to a flat list: `textures_kept_alive` is now a `HashMap<ShapeId, Arc<Box<dyn RhiTexture>>>` (a new `ResolvedTexture` type alias for the value, since clippy's own `type_complexity` lint flagged the raw nested type once it appeared in `resolve_fill`'s return type too). `resolve_fill` now returns `(FillStyle, Option<ResolvedTexture>)` instead of just the `FillStyle` -- `Some` only for an actual `Texture` fill -- since the shape's own `ShapeId` doesn't exist yet at the point `resolve_fill` runs (before `self.inner.insert`); each of the four `insert_*` call sites inserts the shape first, then associates the resolved texture (if any) with the id `insert` just returned. `remove(id)` now also removes `id`'s own entry from `textures_kept_alive` before removing the shape itself -- if that was the last strong `Arc` (the Python `Texture` object already garbage-collected too), `RhiTexture`'s own `Drop` runs right there, destroying the real GPU resource and releasing its bindless slot immediately, not merely marking it eligible for some future sweep that never existed.

**Verified.** `cargo fmt --all -- --check`/`cargo clippy --workspace --all-targets -- -D warnings`/`cargo build --workspace --all-targets`/`cargo test --workspace` all clean in both debug and `--release` (the same pre-existing, unrelated `tre-engine`/`tre-memory` release-mode failures from findings #210/#211 remain, confirmed untouched). Beyond the test suite, proved the fix with a real before/after round trip against the live GPU, not just compilation: a script cycling 6,000 distinct 8x8 textures (comfortably past the bindless array's 4,096-slot capacity), each created, used by one rectangle shape, then that shape removed and the Python `Texture` reference dropped, immediately before the next iteration. Run against the pre-fix code (`git stash` on this exact change), the script failed exactly as the leak predicts -- `tre_python.TreError: bindless texture array has no free slots left` at texture #4097. Run against the fix, all 6,000 cycled successfully with no error. Also re-ran the Phase 12 Step 12.4 Gradient/Texture fill demo (confirms a *live*, not-yet-removed shape's texture reference is correctly still intact and renders exactly, since this fix only prunes on `remove()`) and the Phase 10 Step 10.4 headless demo (general regression) -- both passed unchanged.

## Finding #213 Fixed: `tre-python`'s Two Renderer Types Duplicated Their Render/Flatten Logic Verbatim

The `/review-project` Architecture lens found that `PyHeadlessRenderer` (`crates/tre-python/src/renderer.rs`) and `PyWindowedRenderer` (`crates/tre-python/src/windowed_renderer.rs`) each independently implemented an identical `flatten_registry_into` private associated function (byte-for-byte the same body: `mark_all_dirty`, resolve the text atlas context, build a `TextFlattenContext`, call `ShapeRegistry::flatten_into`) plus near-identical `render`/`render_canvas`/`render_parallel` bodies -- the same `reset()`/`stitch_into()`/`flatten_into()` sequence (REVIEW.md finding #210) and the same real-parallelism sequence (registry-count cap check, per-registry `SubCanvas` worker threads, stitching every one into a shared `FrameArena`), including identical error-message text, copied into both files with no shared trait or helper. A real risk given this project's own history of exactly this kind of divergence bug (REVIEW.md finding #196: a fix applied to one copy of near-identical render logic, once, left a sibling copy silently broken).

Not fixed: the review's separate, lower-priority observation that both renderer types store concrete `VulkanDevice`/`VulkanSwapchain` types rather than the backend-agnostic `RhiDevice`/`RhiSwapchain` trait objects the architecture otherwise commits to -- the review's own write-up already flagged this as speculative work against a second backend that doesn't exist yet, and the user's own instruction for this fix named the duplication specifically, not the trait-object generalization.

**Fixed** by extracting the four duplicated sequences into free functions in `renderer.rs` (already the de facto shared-helper module `windowed_renderer.rs` imports constants/error-helpers from), each taking the specific device/atlas/canvas/arena/frame pieces both renderer types own as separate parameters rather than a `&mut self` receiver -- the same "disjoint borrow via free function" technique `flatten_registry_into` itself already used, now shared instead of duplicated: `flatten_registry_into` (moved out of `impl PyHeadlessRenderer` into a plain `pub(crate) fn`), `render_single_registry` (the `render()` body), `render_canvas_shared` (the `render_canvas()` body), and `render_parallel_shared` (the `render_parallel()` body, by far the largest). Both renderer types' own `render`/`flatten_into`/`render_canvas`/`render_parallel` pymethods are now thin wrappers: call the shared function to populate `self.flattened`, then call their own type's real final step (`PyHeadlessRenderer::submit_and_read_bgra`'s GPU readback vs `PyWindowedRenderer::submit_frame_to_window`'s present-with-resize-retry) -- the one genuinely different piece between the two renderer types, correctly left unshared. `windowed_renderer.rs` shrank from 720 to 633 lines with its own duplicate `flatten_registry_into` removed entirely; `renderer.rs` grew from 590 to 669 lines holding the four now-shared functions (with fuller doc comments than either private copy had alone), for a net reduction in real logic even though total line count moved only modestly -- the actual win is that each of the four sequences now exists exactly once, not twice.

**Verified.** `cargo fmt --all -- --check`/`cargo clippy --workspace --all-targets -- -D warnings`/`cargo build --workspace --all-targets`/`cargo test --workspace` all clean in both debug and `--release` (the same pre-existing, unrelated `tre-engine`/`tre-memory` release-mode failures from findings #210-212 remain, confirmed untouched). Beyond the test suite, rebuilt the `tre_python` PyO3 extension and exercised all eight affected pymethods across both renderer types against real GPU hardware: the Phase 10 Step 10.4 headless `render()` demo (including its own second-call-reproduces-the-same-frame regression check), the Phase 12 Step 12.5 headless `flatten_into`/`render_canvas` composition demo, the Phase 12 Step 12.6 headless `render_parallel` demo, the Phase 12 Step 12.1 windowed `render()`/multi-window demo (120 real on-screen frames), the Phase 19 Step 19.4 windowed `render_canvas` demo, and a new small script exercising `WindowedRenderer.render_parallel` specifically (the one shared-function/final-step combination no existing demo covered) -- all six passed unchanged.

## Finding #214 Fixed: `tre-svg`'s SMIL Parser Had No Untrusted-Input Hardening At All

The `/review-project` Security lens found that `tre_svg::smil::parse_smil` -- explicitly documented as handling the same untrusted-SVG use case its sibling `parse_svg` does, via a separate direct `roxmltree` XML pass since `usvg` itself discards `<animate>`/`<animateTransform>` entirely -- took an unbounded `&str` straight into `roxmltree::Document::parse`, then unconditionally collected every `<animate>`/`<animateTransform>` element's own keyframes into growing `Vec`s, with no byte-size ceiling and no output-size cap of any kind. `parse_svg` itself, by contrast, enforces a `max_bytes` ceiling before `usvg` ever sees the data plus an incremental `max_points` budget threaded through the whole tree walk, and is proptest-fuzzed for exactly this reason -- `parse_smil` had none of it, and no proptest coverage existed for this module at all. A denial-of-service-only gap (no memory corruption found), but a real asymmetry against the crate's own stated threat model.

**Fixed** by giving `parse_smil` the same two-part hardening `parse_svg` already has: a `max_bytes` ceiling checked on `svg_source.len()` before `roxmltree::Document::parse` ever runs (a new `SvgError::TooLarge` reuse -- the same variant `parse_svg`'s own byte check already produces, since both parsers reject the identical "raw input too large" condition on the identical underlying SVG source), and an incremental `max_keyframes` budget summed across every real `<animate>`/`<animateTransform>` element found, checked after each element is parsed and before it's pushed into the result (a new `SvgError::TooManyKeyframes { count, max }` variant, mirroring `TooManyPoints`'s own shape). Disclosed asymmetry from `parse_svg`'s own `max_points`: SMIL keyframes don't expand the way curve-flattened path points do (one keyframe is exactly one already-present numeric token in the source), so `max_bytes` alone already bounds worst-case keyframe count too -- `max_keyframes` exists to give a caller a real, separately-tunable ceiling on output size, matching the established two-budget shape rather than relying on the byte cap alone. `tre-python`'s own `parse_smil` pyfunction gained matching `max_bytes`/`max_keyframes` keyword arguments (defaulted to generous constants, mirroring `PySvg::parse`'s existing `max_bytes`/`max_points` convention exactly) -- and, since that wrapper must convert `data: Vec<u8>` to `&str` via `String::from_utf8` before it can call `parse_smil` at all, gained its own extra `data.len() > max_bytes` check on the *raw* bytes first, so an oversized buffer is rejected before spending real time on UTF-8 validation, not just before the XML parse that follows it.

Also added: a `parse_smil_never_panics_or_hangs_on_arbitrary_bytes`/`parse_smil_never_panics_or_hangs_on_random_values_attribute` proptest pair (mirroring `parse_svg`'s own `mod proptests` exactly -- same generous 2-second bounded-time assertion, same "random bytes" plus "real XML skeleton with a randomly-generated attribute" two-pronged approach), closing the "no proptest exists for this module" half of the finding, not just the missing budget checks.

**Verified.** `cargo fmt --all -- --check`/`cargo clippy --workspace --all-targets -- -D warnings`/`cargo build --workspace --all-targets`/`cargo test --workspace` all clean in both debug and `--release` (the same pre-existing, unrelated `tre-engine`/`tre-memory` release-mode failures from findings #210-213 remain, confirmed untouched); `tre-svg`'s own test suite grew from 28 to 33 tests (3 new unit tests plus 2 new proptests), all passing in both profiles. Beyond the test suite, rebuilt the `tre_python` PyO3 extension and confirmed both new rejection paths work end to end through the real Python API (not just Rust unit tests): `max_bytes=4` correctly rejects even a small, well-formed document with the exact expected error message; a 2,000-keyframe document correctly rejects under `max_keyframes=100`; and the existing Phase 13 Step 13.7 SMIL-parsing demo (real `<animate>`/`<animateTransform>` extraction plus a real SMIL-driven render) still passes unchanged against the new generous default budgets.

## Finding #215 Fixed: Six Hand-Written Error Enums Repeated `Display`/`Error` Boilerplate `thiserror` Already Removes Elsewhere in the Ecosystem

The `/review-project` Modernizer lens found that every one of this workspace's six error enums (`EngineError` and `GradientError`/`FontError` in `tre-engine`, `PlatformError` in `tre-platform`, `SvgError` in `tre-svg`, `TextError` in `tre-text`) hand-wrote a `match`-per-variant `std::fmt::Display` impl whose arms just repeat the variant's own doc comment, plus an empty `impl std::error::Error for X {}` beneath it -- on the order of 100+ lines of pure boilerplate workspace-wide with no behavior difference from what a `thiserror::Error` derive produces. Flagged genuinely low priority (an unusually modern codebase otherwise -- edition 2021, MSRV-appropriate idioms throughout, zero unsound patterns found), and left open pending the project owner's own call on taking one new dependency versus the boilerplate savings.

**Resolved at the project owner's explicit request** (adopting `thiserror`, the recommended option, over leaving the enums hand-rolled). All six enums now derive `thiserror::Error` alongside their existing derives (`Clone`/`Copy`/`PartialEq`/`Eq` preserved exactly where each already had them), with each variant's hand-written `write!(...)` call moved verbatim into a `#[error("...")]` attribute -- named-field interpolation (`{count}`, `{max}`, ...) and positional tuple-field interpolation (`{0}`) both map directly from the old `Display` match arms, so every message string is byte-for-byte unchanged. The manual `impl Display`/`impl Error` blocks are deleted entirely; `thiserror`'s derive generates both. `tre-text/src/error.rs`'s now-unused `use std::fmt;` import was removed alongside its `Display` impl. `thiserror = "2.0.20"` was added to the four owning crates' `Cargo.toml`s (`tre-engine`, `tre-platform`, `tre-svg`, `tre-text`) -- the newest version `cargo add --dry-run` resolved against this workspace's `rust-version = 1.75`, the same pinning technique already used for every other version-sensitive dependency here.

**Verified.** `cargo fmt --all -- --check`/`cargo clippy --workspace --all-targets -- -D warnings`/`cargo build --workspace --all-targets`/`cargo test --workspace` all clean in both debug and `--release` (the same pre-existing, unrelated `tre-engine`/`tre-memory` release-mode `should panic` failures from findings #210-214 remain -- confirmed untouched via `git status` against those exact files, since this fix touches neither). No existing test in this workspace actually asserted on any of these six enums' `Display`/`to_string()` text (checked directly, not assumed) -- the correctness bar for a pure representation change like this one is that every derived message is byte-for-byte identical to the hand-written one it replaces, so each of the ~25 variants' `write!` body was compared against its new `#[error("...")]` attribute by hand, including the two multi-line `\`-continued strings (`GradientError::StopsNotAscending`, matching `parse_svg`'s own reflow style) where the line-continuation's whitespace-stripping behavior had to be traced through explicitly to confirm the rendered string doesn't gain or lose a space. `tre-python/src/svg.rs`'s real, running `svg_err` mapping (`SvgError::TooLarge`/`TooManyKeyframes` -> `PyValueError::new_err(e.to_string())`) is the one path with an actual end-to-end runtime check: re-running this session's own SMIL-hardening verification script confirmed the exact same error text (`"exceeding the 4-byte limit"`, `"exceeding the 100-keyframe limit"`) still surfaces through the real Python API after the derive swap.

## Finding #216 Fixed: `tre-python`'s Two Renderer Types Stored Concrete Vulkan Types Instead of RHI Trait Objects

The `/review-project` Architecture lens found that `PyHeadlessRenderer` and `PyWindowedRenderer` (`crates/tre-python`) both stored their own device/swapchain as the concrete `VulkanDevice`/`HeadlessSwapchain`/`VulkanSwapchain` types rather than the backend-agnostic `RhiDevice`/`RhiSwapchain` trait objects the architecture otherwise commits to everywhere else in the render path (`execute_frame`/`submit_frame`, `ShapeRegistry::flatten_into`, `TextAtlas::new` already took `&dyn RhiDevice`/`&dyn RhiSwapchain`). Harmless while only one real backend (Vulkan) exists, but real, disclosed debt: the day a second backend is added, `tre-python` would need its own rework rather than picking up the new backend automatically through the trait. Originally left open pending the project owner's own call, since generalizing it is speculative work against a backend that doesn't exist yet.

**Resolved at the project owner's explicit request**, after two rounds of investigation surfaced real scope the initial framing hadn't captured, each disclosed and confirmed with the project owner before proceeding rather than silently narrowed or silently expanded:

- Two concrete-only capabilities were the actual blockers, not merely "the field type": `VulkanDevice::create_custom_pipeline` (an inherent method compiling the Phase 13 Step 13.8 custom-shader API, taking a raw `vk::Format`) and `HeadlessSwapchain::read_pixels_bgra8` (an inherent method with no `VulkanSwapchain` equivalent at all -- a real windowed swapchain was never built with host readback in mind). Both moved onto the `RhiDevice`/`RhiSwapchain` traits themselves: `create_custom_pipeline` now takes this crate's own backend-agnostic `TextureFormat` and returns `Box<dyn RhiPipelineState>`; `read_pixels_bgra8` is a new trait method, with `VulkanSwapchain`'s own impl returning a new `EngineError::PixelReadbackUnsupported` (only `HeadlessSwapchain` supports it for real -- its own inherent method stays, delegated to via the same "inherent method wins method-call resolution" pattern, so none of the ~30 existing `tre-rhi-vulkan` examples calling it directly needed to change). Both traits also gained a `Send + Sync` supertrait bound, matching `RhiBuffer`/`RhiTexture`/`RhiPipelineState`'s own identical, earlier fix -- needed for `Box<dyn RhiDevice>`/`Box<dyn RhiSwapchain>` to cross a `Python::detach` call and, for `RhiDevice`, to be shared read-only across `render_parallel`'s own worker threads.
- A third, deeper blocker surfaced only once the windowed renderer's own `create_window`/resize-recovery paths were investigated directly: `register_shape_pipelines` (compiling this crate's own 8 built-in shape pipelines) and `VulkanDevice::create_surface` both need a *concrete* device, called *repeatedly* throughout `PyWindowedRenderer`'s lifetime (once per new window, and again on every swapchain-recreation-on-resize) -- not once at construction the way `PyHeadlessRenderer`'s identical calls are. Closing this too would mean adding window/surface-swapchain construction to the `RhiDevice` trait itself (a new `raw-window-handle` dependency in the core `tre-engine` crate) for a capability that doesn't reasonably belong there -- backend-selection/window-surface bootstrapping is the same category `VulkanDevice::new` itself already occupies, not steady-state per-frame rendering. Surfaced to the project owner directly rather than silently building it or silently dropping the rest of the generalization.

**The actual, complete result**: `PyHeadlessRenderer` fully generalizes -- both `device: Box<dyn RhiDevice>` and `swapchain: Box<dyn RhiSwapchain>`, since its own `register_shape_pipelines`/`HeadlessSwapchain::new` calls happen exactly once, before boxing, matching `PyWindowedRenderer`'s *per-window* construction sequence but not its *repeated, whole-lifetime* need for a concrete device. `PyWindowedRenderer`'s per-window `WindowSlot::swapchain` generalizes the identical way (built as a concrete `VulkanSwapchain` inside `WindowSlot::create` for `register_shape_pipelines`/`.format()`, then boxed for storage); its own top-level `device` field stays a concrete `VulkanDevice`, documented in place as a real, disclosed boundary rather than a narrowed one. Two of this session's own test-only `FakeDevice` mocks (`tre-engine/src/lib.rs`, `tre-engine/src/shapes.rs`) needed the identical `RefCell`/`Cell` -> `Mutex`/`Atomic*` fallout fix `FakeStyleBuffer` already went through when `RhiBuffer` gained the same bound, plus a new `create_custom_pipeline` stub (`unimplemented!()`, matching every other method these fakes don't exercise).

**Verified.** `cargo fmt --all -- --check`/`cargo clippy --workspace --all-targets -- -D warnings`/`cargo build --workspace --all-targets`/`cargo test --workspace` all clean in both debug and `--release` (the same pre-existing, unrelated `tre-engine`/`tre-memory` release-mode `should panic` failures from findings #210-215 remain, confirmed untouched). Beyond the test suite, rebuilt the `tre_python` PyO3 extension and ran real GPU demos covering every path this change touches: the Phase 10 Step 10.4 headless demo (`device`/`swapchain` now trait objects, including the second-call-reproduces-the-same-frame regression check), the Phase 13 Step 13.8 custom shader demo (`create_custom_pipeline` through the new trait method, both the success path and the real-compiler-error rejection path), the Phase 16 Step 16.1 SDF soft-shadow demo (a second, independent `create_custom_shader` consumer), the Phase 12 Step 12.1 windowed multi-window demo (`create_window`'s own concrete-device path plus `WindowSlot::swapchain`'s new boxed storage, 120 real submitted frames plus a second window created and closed), a `WindowedRenderer.render_parallel` script, the texture-leak fix's own 6,000-texture cycling script, and the SMIL-hardening script -- all passed unchanged, confirming zero behavioral regression across this session's entire prior work in addition to the new change itself. `crates/tre-ffi/include/tre_ffi.h` regenerated automatically (its own `build.rs` reruns `cbindgen` on every build) with the new `TRE_ERROR_CODE_PIXEL_READBACK_UNSUPPORTED` variant; `tre-ffi`'s own C harness test (real GPU, `cargo test -p tre-ffi`) still passes unchanged, since it only ever calls `HeadlessSwapchain::read_pixels_bgra8`'s untouched inherent method directly.

## Finding #217 Fixed: CI's `test` Job Had No Vulkan Device, So `tre-ffi`'s Own C Harness Test Never Actually Ran There

Carried over from this session's own CI work, not from the four-lens review: `.github/workflows/ci.yml`'s `test` job never installed a software Vulkan ICD, unlike the `vulkan-validation`/`accessibility-validation` jobs (which both install `libvulkan1`/`mesa-vulkan-drivers`/`vulkan-validationlayers` for their own example runs against a real, if software-backed, device). `tre-ffi`'s C harness test (`crates/tre-ffi/tests/c_harness.rs`, Phase 10 Step 10.3) -- a real GPU test, part of the `test` job's own `cargo test --workspace` step -- was the first thing in the whole workspace suite that actually needed a device there, and this hosted runner has none by default, so that test would fail (or, if `xvfb-run`'s own exit-code handling masked it, silently never really exercise the FFI surface it exists to prove).

**Fixed** by adding the identical three packages the other two jobs already install (`libvulkan1 mesa-vulkan-drivers vulkan-validationlayers`) to the `test` job's own install step -- matching their proven-working package set exactly rather than guessing at a smaller subset.

**Verified.** YAML syntax confirmed valid (`python3 -c "import yaml; yaml.safe_load(...)"`). This fix's real effect can only be confirmed by an actual GitHub Actions run of the `test` job, which standing policy doesn't authorize without a separate, explicit push instruction -- not verified end to end this session, disclosed rather than assumed. The change itself is minimal and low-risk: purely additive to an apt package list, copying a package set two sibling jobs in this same file already exercise successfully.

**Confirmed by a real run** (2026-09-13, at the project owner's explicit follow-up request to push and observe it): run 34727926216 (commit `bd7845b`), compared directly against the immediately preceding run (34689114083, commit `dc44054`, before this fix). In the prior run the `test` job failed at exactly `xvfb-run -a dbus-run-session -- cargo test --workspace` (no device). In this run that identical step passes. The `test` job still shows an overall failure, but only at its separate, already-disclosed, intentionally-failing `frame_processing` CPU-budget gate (~0.80ms measured against a 0.50ms budget) -- unrelated to this fix. The run's `vulkan-validation` job also failed, crashing at `blend_mode_demo`; checked against the same prior run and confirmed identical there too -- pre-existing, not a regression from this fix or from anything else in this session's pushed commits.

## Finding #218 Fixed: `tre-ffi`'s C-ABI Never Received the Input-Hardening Pass Already Applied to `tre-python`'s Identical Entry Points

A fresh `/review-project` pass's Architecture and Security lenses independently converged on the same root cause: `tre-python`'s shape-insert and renderer-construction entry points were hardened against untrusted numeric input (REVIEW.md findings #196-198, #202) by validating finiteness/non-negativity and capping `Polygon.sides`, but `tre-ffi` -- explicitly documented (DESIGN.md Section 2.7) as carrying equal responsibility for boundary validation, since `tre_engine::ShapeRegistry::insert` itself never validates -- never received the equivalent pass. Concretely, `crates/tre-ffi/src/registry.rs`'s `tre_shape_registry_insert_polygon` had no upper bound on `sides: u32`, letting a caller pass a value near `u32::MAX` and drive an attempted multi-gigabyte allocation inside `generate_polygon_points_into` (`tre_engine::shapes`) -- the identical bug class #196-198 already fixed for `tre-python`, unmirrored here. `insert_rectangle`/`insert_circle`/`insert_polygon` also never checked `x`/`y` for finiteness, and `insert_path` performed no numeric validation at all on any command's coordinates before they reached `lyon`'s tessellator (a real panic/hang surface, not just a wrong render). Separately, `crates/tre-ffi/src/renderer.rs`'s `tre_headless_renderer_new` had no `width`/`height` validation at all -- `width`/`height == 0` is undefined behavior on real Vulkan hardware (`VUID-VkImageCreateInfo-extent-00944`) in a release build with no validation layer, and an unbounded value is an unbounded-GPU-allocation DoS, the identical bug class REVIEW.md finding #197 already fixed for `tre-python`'s equivalent constructor. The Security lens additionally found three `extern "C"` functions (`insert_rectangle`/`_circle`/`_polygon`) whose validation and shape construction ran *before* entering the `ffi_guard`-wrapped `catch_unwind` closure, deviating from this crate's own documented "every exported function wraps its whole body" contract (a policy-compliance gap, not a live exploit today, since nothing in the unguarded portion can currently panic).

**Fixed** by mirroring `tre-python`'s own proven validation constants and checks directly into `tre-ffi`'s equivalent entry points: a `MAX_POLYGON_SIDES = 4096` constant (identical value to `crates/tre-python/src/shapes.rs`'s own) now rejects `sides` outside `3..=4096`; `x`/`y` finiteness is checked on `insert_rectangle`/`insert_circle`/`insert_polygon`/`insert_path`; a new `command_is_finite` helper validates every field a given `TrePathCommand`'s `kind` actually reads (mirroring `PyPath::move_to`/`line_to`/`quad_to`/`cubic_to`'s identical per-command checks, applied here over the whole array at once since this crate's own API takes all commands in one call rather than incrementally); and a `MAX_DIMENSION = 8192` constant (identical to `tre-python`'s own) now rejects a zero or oversized `width`/`height` in `tre_headless_renderer_new` before `build_renderer` ever runs. The three under-guarded functions now wrap their entire body in `ffi_guard`, matching `insert_path`'s own already-correct shape.

**Verified.** `cargo fmt --all -- --check`/`cargo clippy --workspace --all-targets -- -D warnings`/`cargo build --workspace --all-targets`/`cargo test --workspace` all clean in both debug and `--release` (the same pre-existing, unrelated `tre-engine`/`tre-memory` release-mode `should panic` failures remain, confirmed untouched). Beyond the test suite, wrote a real end-to-end verification program (a temporary `tre-ffi` example, calling the exported `extern "C"` functions directly -- no GPU device needed, since `ShapeRegistry::insert` and the dimension check both run before any Vulkan call) proving every new rejection path: `sides = u32::MAX` and `sides = 4097` both correctly rejected while `sides = 4096` (the cap itself) succeeds; `NaN`/`+inf` on rectangle/circle coordinates correctly rejected; a `NaN` path-command coordinate correctly rejected; `width = 0` and an oversized `height` both correctly rejected on the renderer constructor. `tre-ffi`'s own real C harness test (`cargo test -p tre-ffi`, real GPU) still passes unchanged. The verification example was removed after confirming (a throwaway check, not a shipped addition).

## Finding #219 Fixed: `TransientPool` Never Actually Adopted the `FxHashMap` TECHNICAL.md Already Documents as Settled

The Performance lens found that `crates/tre-rhi-vulkan/src/transient_pool.rs`'s `TransientPool::free` was a plain `std::collections::HashMap`, even though `documentation/TECHNICAL.md` Section 3.2 states as already-decided fact: *"The pool uses `FxHashMap`... or `ahash`'s `AHashMap`... in place of `std::collections::HashMap`'s default SipHash... these keys are internal, engine-generated values with no untrusted input reaching them, so SipHash's DoS resistance buys nothing here and only costs cycles on a per-`push_layer` hot-path lookup."* Neither `rustc-hash` nor `ahash` appeared anywhere in the workspace as a direct dependency -- a genuine documentation-vs-implementation drift on the exact hot path (`Canvas::push_layer`, keyed by a 3-field tuple) the doc names specifically, not a new design question.

**Fixed** by adding `rustc-hash = "=2.1.1"` (the newest version resolvable against this workspace's `rust-version = 1.75`, confirmed via `cargo add --dry-run`; exact-pinned after a bare `"2.1.1"` range was first caught silently resolving 2.1.3, which needs rustc 1.77, in `Cargo.lock`) as a direct `tre-rhi-vulkan` dependency, and swapping `TransientPool::free`'s type from `HashMap` to `rustc_hash::FxHashMap` -- `#[derive(Default)]` needed no change, since `FxHashMap` implements `Default` too.

**Verified.** Full workspace fmt/clippy/build/test clean in debug and release (same pre-existing, unrelated failures remain). Beyond the test suite, ran `memory_pools_demo` (exercises `TransientPool`'s real hit/miss/bucket-rounding/GC behavior directly) and `layer_blur_demo`/`render_to_texture_demo` (both real `push_layer`/`pop_layer` consumers of the pool) against real GPU hardware -- all three passed with their own existing pixel/stat assertions unchanged, confirming the hasher swap is behavior-transparent as expected (a hash function change to a `HashMap`'s bucketing has no observable effect on the stored key/value semantics).

## Finding #220 Fixed: Two Small Modernization Wins -- `Option::get_or_insert_with` and Three Verified-Safe `const fn` Additions

The Modernizer lens, after confirming `cargo clippy --workspace --all-targets` (this project's own chosen lint bar, `clippy::all` plus `clippy::pedantic` on `tre-engine`/`tre-math`) reports zero warnings and that thiserror adoption/RHI trait-object generalization (findings #215-216) are both already done, found two minor, real, mechanically-safe items: `crates/tre-rhi-vulkan/src/command_buffer.rs`'s `apply_layer_blur` hand-wrote a check-then-insert-then-unwrap lazy-init sequence that `Option::get_or_insert_with` (stable since Rust 1.20, far below this workspace's MSRV) replaces with identical behavior in one line instead of seven; and three pure, allocation-free functions (`tre-memory/src/swmr.rs`'s `is_reserved`, `tre-atlas/src/key.rs`'s `unpack_slot_value`, `tre-atlas/src/lib.rs`'s `PackedRect::overlaps`) were real, verified-safe `const fn` candidates the project's own lint configuration doesn't surface (`clippy::nursery`'s `missing_const_for_fn`, deliberately not enabled anywhere in this workspace due to that lint's own known false-positive rate).

**Fixed** by applying both: `apply_layer_blur`'s lazy-init block now reads `*guard.get_or_insert_with(|| create_blur_resources(...))`; the three functions above are now `const fn`/`pub const fn` (additive capability only -- callable from const contexts too, zero behavior change for every existing caller).

**Not applied**: the Modernizer's own broader observation that 64 more `missing_const_for_fn` hits exist across 8 crates under `-W clippy::nursery` -- correctly flagged as needing individual five-minute review each (that lint's own false-positive rate around trait-impl signatures and non-const-compatible bodies means blind batch application is unsafe), not a mechanical batch fix, and not attempted here.

**Verified.** Full workspace fmt/clippy/build/test clean in debug and release (same pre-existing, unrelated failures remain). `apply_layer_blur`'s real behavior re-confirmed via the same `layer_blur_demo` GPU run as finding #219 above (unchanged pixel assertions).

## Finding #221 Documented, Not Fixed: Animating a `Text` Shape's Opacity/Position Forces a Full Re-Shape (Bidi + `rustybuzz`) Every Frame

The Performance lens found that `ShapeRegistry::flatten_into`'s dirty check (`crates/tre-engine/src/shapes.rs`) is shape-kind-agnostic: any shape with a non-empty `active_animations` is fully re-flattened every frame regardless of which property is animating. For `Text`, that means `flatten_text` (`crates/tre-text`) unconditionally re-runs a full `BidiInfo::new` + `unicode_script` segmentation pass plus a complete `rustybuzz` shape call -- real allocations, every frame -- even when only a cosmetic property like opacity is animating and the text content/font/size are completely unchanged. This is exactly the defect `tre_text`'s own module doc describes retained-mode `Text` as existing to eliminate, reappearing through the generic animation path.

**Real reachability caveat, disclosed rather than downplayed**: `ShapeSlot::active_animations` has no public "attach an animation" method anywhere in the workspace today. It's reachable only via `ShapeRegistry::get_mut`'s public `&mut ShapeSlot` (a real, currently-compiling path for any direct Rust or future `tre-ffi` consumer), but neither `tre-python` nor `tre-animation` currently populates it -- so this is a real, executable defect in the engine's own public API and design, not yet observable through the project's primary (Python) integration.

**Not fixed this pass**: a real design decision is needed on the fix's own shape -- distinguishing "content-affecting" vs. "cosmetic-only" animations at the `ShapeSlot`/`AnimationId` level, or caching the shaped run keyed by `(text, font, px_size, wrap_width)` and only re-shaping when that key changes. Either requires touching the animation-attachment design this crate doesn't yet expose a public way to exercise, not a mechanical, low-risk change to make opportunistically inside a review pass. Left open pending the project owner's own direction on animation-system design generally.

**Resolved: see Finding #225 below**, fixed at the project owner's own explicit request via exactly the caching-key option proposed above (`(text, font, px_size, wrap_width)`) -- this entry was simply never cross-referenced at the time, the same stale-index gap finding #145 already named as a recurring problem in this document.

## Finding #222 Documented, Not Fixed: `execute_frame`'s `clip_stack` Allocates a Fresh `Vec` on Every Frame

The Performance lens found `crates/tre-engine/src/rhi.rs`'s `execute_frame` allocates `let mut clip_stack: Vec<ScissorRect> = Vec::new();` fresh on every call -- once per rendered frame, for essentially any real scene that clips anything (scroll views, rounded panels, nested containers). TECHNICAL.md Section 2.1 already discloses that RHI submission isn't yet covered by this project's own `RenderTickGuard` zero-allocation enforcement, but names one specific culprit (`begin_frame`'s per-frame `Box<dyn RhiCommandBuffer>`, finding #156); `clip_stack` is a second, previously-unnamed instance of that same disclosed gap, not the already-known one.

**Not fixed this pass, disclosed rather than silently applied**: the mechanical fix itself is simple -- thread a caller-owned `&mut Vec<ScissorRect>` scratch buffer into `execute_frame`, cleared rather than reallocated each call, matching every sibling piece of per-frame scratch state elsewhere in this codebase (`FrameArena`'s own scratch fields, `ShapeRegistry::polygon_points_scratch`). But `execute_frame` is a widely-used public function with roughly 30 real call sites across `tre-engine`'s own test module, `tre-ffi`, `tre-python` (two sites), and ~19 `tre-rhi-vulkan` examples -- a small per-call code change with real workspace-wide blast radius, changing a public signature well beyond this one crate. Left open for the project owner's own explicit go-ahead before touching that many call sites in one pass, rather than assumed low-risk enough to apply autonomously.

## Finding #223 Fixed: `vulkan-validation` CI Job Crashed at `blend_mode_demo` on a Software Vulkan Implementation With No Real GPU

`crates/rhi-vulkan/examples/blend_mode_demo.rs` (Phase 10 Step 10.2.3's own real proof of `VK_KHR_dynamic_rendering_local_read` blend-mode rendering) contained a hard `assert!(device.local_read_blend_supported(), ...)`, deliberately written to fail loudly if the real capability it exists to prove wasn't actually present. CI's `vulkan-validation` job runs this example against a software Vulkan implementation (`mesa-vulkan-drivers`' lavapipe, no real GPU on a hosted runner) -- confirmed directly from the job's own log: the validation layer itself reports `VK_KHR_dynamic_rendering_local_read is not supported by this layer`, so `local_read_blend_supported()` correctly returns `false` there, and the demo's own assertion panics (exit code 101), failing the whole job. This project's own real dev GPU (RADV) does support the extension -- confirmed locally, the demo passes there with all its own real pixel assertions intact.

**Fixed** by converting the hard assertion into a graceful skip (`eprintln!` + early `return`, exit code 0) when the capability isn't present -- matching this workspace's own already-established convention for exactly this class of gap (`tre-a11y/tests/round_trip.rs`'s identical "skip, don't fail, when this environment doesn't provide a real capability" pattern for its own AT-SPI2 bus dependency). Not a loosening of the demo's own real assertion -- on any device that *does* support the extension (including CI's own `accessibility-validation` job, which shares the same software driver but never runs this particular example), the full real proof still runs and still asserts exact pixel values; the skip only ever triggers on a device that genuinely cannot exercise the capability at all.

**Verified.** Re-ran `blend_mode_demo` locally against real GPU hardware (RADV) -- unchanged: all six blend-mode swatches plus the background readback still pass with the exact same expected pixel values as before this change. The skip path itself cannot be exercised on this machine's real GPU (which does support the extension), so its own correctness rests on directly reading CI's log output confirming `local_read_blend_supported()` returns `false` there -- the exact condition the new `if` branches on -- and on the pattern being byte-for-byte identical in shape to `tre-a11y`'s own already-proven-in-CI skip mechanism, not a new, unproven idiom.

## Finding #224 Fixed: `frame_processing` Benchmark Measured Cold-Start Allocation, Not the Steady-State Cost the CI Budget Gate Is About -- Plus a Real New Engine Capability That Closes the Gap for Real

CI's `test` job runs `record_and_flatten_10k_nodes` and fails the build if its measured time exceeds TECHNICAL.md Section 9.2's documented 0.50ms CPU frame-processing budget -- previously measuring ~0.80ms on CI's own hardware (REVIEW.md finding #159, confirmed with the project owner at the time as real, disclosed, deliberately-not-fixed-here future work). Investigating directly: the benchmark built a fresh `RenderingCanvas::new()` and called the consuming `RenderingCanvas::flatten(self)` inside every timed iteration, so every sample paid full cold-allocation cost (`vertices`/`indices`/`commands` growing from empty, plus `flatten`'s own internal `segment_and_flatten` allocating fresh sort scratch on every single call) on top of the real per-frame processing work the budget is actually about -- the opposite of this project's own established, documented zero-allocation-reuse convention (`reset()`/`FrameArena::flatten_into`). A real running application never rebuilds these buffers every frame, so the benchmark was never actually measuring the steady-state cost TECHNICAL.md Section 9.2 describes.

Rewriting the benchmark to reuse the canvas via `reset()` and route through the existing `stitch_into`/`FrameArena::flatten_into` path (the real production sequence `tre-python`'s own single-registry `render()` already uses, per REVIEW.md finding #210) measured *slower* on this machine (~525µs vs. the original's ~469µs) -- a real, honest, negative result, not discarded: `FrameArena`/`stitch_into` exist specifically to *combine multiple* canvases' worth of data into one shared sort, and a lone canvas paying that extra full-data-copy cost for nothing to combine is pure overhead with no offsetting benefit, on top of which this session's own earlier work (see this file's own "Key Technical Concepts") had already flagged that `RenderingCanvas::flatten(self)` had no non-consuming sibling the way `FrameArena` does.

**Fixed for real** by adding that missing sibling: `RenderingCanvas::flatten_into(&mut self, out: &mut FlattenedFrame)`, a genuine new engine capability (`crates/tre-engine/src/canvas.rs`), not a benchmark-only trick. `vertices`/`accessibility_nodes` are swapped (not copied) into `out`'s own `Vec`s -- valid since sorting/batching never transforms either, only passes them through -- and `commands`/`indices` feed directly into the same `sort_and_batch_into` core `FrameArena::flatten_into` already shares, using two new persistent `sort_scratch`/`counts` fields on `RenderingCanvas` itself (mirroring `FrameArena`'s identically-named fields exactly) as the reused scratch buffers. A real caller building one `RenderingCanvas` and one `FlattenedFrame` once, then calling `reset()` then this every frame, now reaches true zero-allocation steady state without `FrameArena` in the picture at all -- that type stays reserved for when there really are multiple canvases/`SubCanvas`es to merge (`render_parallel`'s own real use case, left unchanged). The benchmark itself was rewritten to build its canvas/output once and call `reset()`/`flatten_into()` per iteration, matching this new, real steady-state path.

**Scope disclosed, not silently expanded**: `tre-python`'s own `render_single_registry` and `tre-ffi`'s equivalent single-canvas renderer path still route through `stitch_into`/`FrameArena` rather than this new, cheaper `flatten_into` -- a real, legitimate follow-up opportunity (a further per-call win for `PyHeadlessRenderer.render()`/`PyWindowedRenderer.render()`/tre-ffi's own `tre_headless_renderer_render`), but migrating those production call sites needs its own careful GPU-verified pass, not folded into this CI-gate fix.

**Verified.** Full workspace fmt/clippy/build/test clean in debug and release (the same three pre-existing, unrelated release-mode `should panic` failures remain -- now three instead of two, since this fix's own new `canvas_flatten_into_still_enforces_the_balance_assertion` test is, by design, the identical kind of debug-assertion-reliant test as its two siblings, confirmed deliberate, not a new problem). Three new unit tests added directly (`canvas_flatten_into_produces_the_same_result_as_flatten_across_repeated_reused_frames`, `canvas_flatten_into_reuses_its_own_scratch_buffers_across_calls_without_reallocating`, `canvas_flatten_into_still_enforces_the_balance_assertion`), `tre-engine`'s own suite growing from 172 to 175. Beyond unit tests, re-ran `shape_registry_zero_alloc_demo`/`main_loop_demo` (real GPU, both already asserting zero heap allocations across 90-120 real frames) to confirm the two new struct fields don't disturb the engine's existing zero-allocation guarantees, plus `tre-ffi`'s own C harness and the Phase 10 Step 10.4 Python headless demo as broader regression checks -- all passed unchanged. The rewritten benchmark itself measures ~432µs on this machine (down from the original's ~469µs, and well below the original arena-routed rewrite's ~525µs), comfortably under the 500µs budget locally with real margin -- disclosed honestly: this machine measured the pre-fix code at ~469µs where CI's own hosted runner measured ~796µs (a real, confirmed ~1.7x hardware gap), so a proportionally slower measurement on CI remains possible and can only be confirmed by an actual CI run, not assumed from a local number alone.

## Finding #225 Fixed: Animating a `Text` Shape's Opacity/Position Forced a Full Bidi + `rustybuzz` Re-Shape Every Frame

The Performance lens found `ShapeRegistry::flatten_into`'s dirty check (`crates/tre-engine/src/shapes.rs`) is shape-kind-agnostic: any shape with a non-empty `active_animations` is fully re-flattened every frame regardless of which property is actually animating. For `Text`, that meant `flatten_text` unconditionally re-ran a full `unicode_bidi`/`unicode_script` segmentation pass plus a complete `rustybuzz` shape call every frame, even when only a cosmetic property like opacity was animating and the text content/font/size/wrap width were completely unchanged -- the exact per-frame cost this crate's own retained-mode `Text` primitive exists to eliminate, reappearing through the generic animation path. Disclosed alongside the finding: `ShapeSlot::active_animations` has no public "attach an animation" method anywhere in the workspace today (`tre-python`/`tre-animation` don't populate it), so this was a real, currently-executable defect in the engine's own public API, not yet observable through the project's primary Python integration -- but a real defect regardless of current reachability, since `ShapeRegistry::get_mut` already exposes a fully public, compiling path to it for any direct Rust consumer.

**Fixed at the project owner's explicit request**, via the caching-key option this report proposed. `crates/tre-engine/src/text.rs` gained a new `TextShapeCache` (private, `pub(crate)` for `ShapeSlot`'s own field) keyed by exactly the fields that affect shaping -- `text`, `font`, `px_size`, `wrap_width` -- holding the last-resolved `Vec<ShapedRun>`. A new `resolve_shaped_runs` helper compares the current `Text`'s own fields against the cache and only calls the real `tre_text::shape_text` when something shaping-relevant actually changed; every other field (`fill_color`, and everything under `common`: `opacity`, `transform.position`/`scale`/`rotation`, `visibility`, ...) can now change freely, every frame, with zero re-shaping cost. `ShapeSlot` gained a new `text_shape_cache: Option<TextShapeCache>` field (`None` for every non-`Text` shape), initialized fresh on every `ShapeRegistry::insert` call -- since `insert` always constructs a brand-new `ShapeSlot` rather than mutating a stale one from a reused generational slot, there is no staleness risk across slot reuse. `flatten_text`'s own signature gained the new `shape_cache: &mut Option<TextShapeCache>` parameter, threaded through from `flatten_into`'s single `ShapePrimitive::Text` call site; the cache is content-addressed, not animation-flag-addressed, so it self-corrects regardless of *how* a shape's fields changed (a real future animation system, or a direct `get_mut` rewrite) -- no separate "on mutation" invalidation hook was needed.

**Verified.** Full workspace fmt/clippy/build/test clean in debug and release (the same three pre-existing, unrelated release-mode failures remain, confirmed untouched). Three new white-box unit tests added directly in `text.rs` (a new local `mod tests`, since `TextShapeCache`/`resolve_shaped_runs` are private to that module): `resolve_shaped_runs_reuses_the_cached_runs_when_nothing_shaping_relevant_changed` (asserts the *same* `Vec` pointer is returned across two calls against an unchanged `Text`, proving no re-shape happened, not just that the output looks right), `resolve_shaped_runs_reshapes_when_the_text_content_changes` (a genuinely different, longer string produces a real, larger glyph count, proving a stale cache is never silently served), and `text_shape_cache_is_valid_for_checks_every_shaping_relevant_field_and_only_those` (a pure table of every field that must invalidate the cache -- text/font/px_size/wrap_width -- and, just as importantly, every field that must *not* -- fill_color, opacity, position, rotation -- the exact claim this fix rests on). `tre-engine`'s own suite grew from 175 to 178. Beyond unit tests, rebuilt the `tre_python` extension and re-ran four real GPU demos exercising `Text` through `ShapeRegistry` (Phase 12 Step 12.3's Font/Text demo, Phase 15 Steps 15.1/15.5's multi-line/word-wrap/UAX-14 demos, and Phase 13 Step 13.5's `EditableText` demo -- the last one a real stress test, since every keystroke changes `text.text` across many `render()` calls in sequence, exercising both the cache-hit and cache-miss paths repeatedly) -- all four passed with their own existing pixel/behavioral assertions unchanged.

## Finding #226 Fixed: `shape_registry_zero_alloc_demo` Had the Identical CI Gap Finding #223 Fixed, Previously Hidden

Pushing and observing finding #223's own `blend_mode_demo` fix (2026-09-13) surfaced a second, previously-invisible failure in the same `vulkan-validation` CI job: `crates/tre-rhi-vulkan/examples/shape_registry_zero_alloc_demo.rs` has the identical hard `assert!(device.local_read_blend_supported(), ...)` for its own non-`Normal`-`BlendMode` `Polygon` swatch. This was never actually reached in any prior CI run -- `blend_mode_demo` runs earlier in the job's own step list and always failed first, and the job's `shell: bash -e` aborts the whole job at the first failing step, masking every step after it. Fixing `blend_mode_demo` let the job run further and reach this real, pre-existing, identically-caused gap for the first time.

**Fixed** the same way, for the same reason: converted the hard assertion into the identical graceful skip (`eprintln!` + early `return`, exit 0) `blend_mode_demo`'s own fix already established. A real, disclosed coverage tradeoff, not glossed over: this demo's own real proof covers a *mixed* scene (`Rectangle`, gradient `Circle`, textured `Polygon`, the blend-mode `Polygon`, a bordered partial-arc `Circle`) -- skipping the whole demo on a capability-lacking device means CI's software Vulkan implementation specifically never exercises the zero-allocation guarantee for any of those other shape kinds either, not just the blend-mode one. A more surgical fix (skip only the blend-mode swatch, keep proving zero-allocation for the rest of the scene) was considered and rejected for this pass -- restructuring the demo's own scene-construction code carries real risk of quietly breaking the very zero-allocation guarantee it exists to prove, disproportionate to what this CI-gap fix calls for. This project's own real dev GPU still runs the demo's full, real proof unchanged, exactly as `blend_mode_demo`'s own fix already established.

**Verified.** Full workspace fmt/clippy/build clean; re-ran the demo against real GPU hardware (RADV, which does support the extension) -- unchanged: the full 120-frame mixed-scene zero-allocation proof still passes with its own exact assertions intact. Grepped the whole `tre-rhi-vulkan/examples` directory for every other use of `local_read_blend_supported` before considering this closed -- confirmed these were the only two, so no third instance is still hiding behind this one.

## Finding #227 Fixed: The `frame_processing` Optimization (Finding #224) Measured *Worse* on CI's Own Hardware, Despite a Real, Repeatable Local Improvement

Pushing finding #224's own `RenderingCanvas::flatten_into` optimization and observing a real CI run produced a result at odds with every local measurement: CI's `test` job measured the new code at **906.8µs** -- worse than the *pre-fix* code's own previously-observed **796µs** on that same job, and more than 2x this development machine's own repeatable **432µs** figure for the identical new code. Every local measurement (four independent `cargo bench` runs across the investigation, using both `hyperfine`-style repeated sampling via criterion's own statistics and direct before/after comparison) showed the new code strictly faster than the old; CI's own hosted runner showed the opposite.

**Not chased further with more code changes this pass.** The likely explanation, not yet confirmed: GitHub Actions' shared, virtualized hosted runners are widely known to have highly variable per-run CPU performance (noisy-neighbor contention, no dedicated-core guarantee), and a single criterion sample there is not a reliable enough signal to safely conclude the new code is *actually* slower in any way this dev machine's own controlled, repeated measurements would ever show -- but this is a real, disclosed hypothesis, not yet independently confirmed (e.g. by re-running the identical bench on CI multiple times back-to-back and observing the variance directly). This calls into question whether an absolute-microsecond hard CI gate is even a sound design for this specific budget check on this specific infrastructure, a genuine design question beyond what a single code-level optimization pass can resolve -- left open for the project owner's own explicit direction (options include: re-run to characterize CI's own noise floor directly before concluding anything; relax the gate to a checked-in-baseline-relative percentage rather than an absolute number; move this check to a self-hosted or otherwise more consistent runner; or drop it as a hard CI gate entirely and keep it as a local/informational-only measurement, matching how many real projects treat wall-clock microbenchmarks on shared cloud CI). No further optimization attempted blind against a measurement environment not yet understood to be reliable.

**Update (2026-09-13): the noise-floor hypothesis above is refuted by direct measurement.** At the project owner's explicit request, re-ran the *identical* `test` job (no code changes at all -- `gh run rerun --job`, same commit `d5463b1`, same run `34731056338`) four more times back-to-back, giving five independent samples of the exact same code on CI's own hardware:

| # | Trigger | `record_and_flatten_10k_nodes` mean |
|---|---|---|
| 1 | original push | 906.8µs |
| 2 | rerun | 905.1µs |
| 3 | rerun | 865.5µs |
| 4 | rerun | 892.8µs |
| 5 | rerun | 891.4µs |

Mean 892.3µs, standard deviation 14.8µs (coefficient of variation **1.7%**), full range 41.3µs (4.6% of the mean). This is a *tight* cluster, not the "highly variable per-run performance" the noisy-shared-runner hypothesis predicted -- if that hypothesis were right, five independent samples of identical code would be expected to scatter far more widely than a 1.7% CV. Every single one of the five runs still fails the 500µs budget by a wide, consistent margin (1.73x-1.81x over), and the mean sits almost exactly 2x this development machine's own repeatable ~432-469µs figure for the identical code.

**Conclusion: this is not CI noise. It is a real, stable, ~1.8-2x hardware performance gap between this development machine and GitHub Actions' hosted runner**, for this specific benchmark. An absolute-microsecond budget calibrated against local hardware was never going to hold on hardware that is consistently, reliably slower by roughly a factor of two -- no amount of re-running will make CI's own runner measure like this dev machine, because the two are just different hardware, not the same hardware with more or less contention on a given run. This changes which of the four originally-listed options make sense: "re-run to characterize the noise floor" is now done and its premise didn't hold, so the real remaining choices are between a relative/baseline-tolerance gate (compare CI's own measurement against CI's own prior baseline, not against a number calibrated on different hardware), moving the check to more consistent/dedicated hardware, or dropping the hard gate and keeping the measurement informational -- left for the project owner's own explicit direction, not decided here.

**Fixed 2026-09-13, at the project owner's explicit request: "Redesign the gate as relative/baseline-tolerance."** `.github/workflows/ci.yml`'s check step no longer compares against a hard absolute 0.50ms number; it now loads a new checked-in file, `crates/tre-engine/benches/frame_processing_ci_baseline.json`, containing `baseline_ns: 892300` (the mean of the five characterization runs directly above) and `tolerance_pct: 20`, and fails only if the fresh measurement exceeds `baseline_ns * 1.20` (1,070,760ns). 20% was chosen as roughly 4x the observed 5-run coefficient of variation (1.7%) and roughly 4x the observed full range as a fraction of the mean (4.6%) -- comfortable margin above this runner's own real scatter, while a genuine regression back toward the pre-#224-fix code's own ~796,000ns performance class (a >20% jump from 892,300ns) would still correctly fail. The baseline file itself carries an explicit warning that it is CI-hardware-specific and must never be compared against or re-derived from a local `cargo bench` run -- exactly the mistake the original absolute-budget design made. TECHNICAL.md Section 9.2 gained a "Gate redesign" implementation-status entry documenting the full rationale.

**Verified.** Ran the exact Python comparison logic the new CI step uses, standalone, against this development machine's own real `cargo bench -p tre-engine --bench frame_processing` output (421,461ns) -- correctly reports `PASS` against the CI baseline's 1,070,760ns ceiling (expected: local hardware is faster, so it should never itself trip a CI-hardware-calibrated gate). Also ran the identical logic with a synthetic 1,200,000ns input to confirm the fail path still triggers `sys.exit(1)` correctly. `python3 -c "import yaml; yaml.safe_load(...)"` confirms `ci.yml` itself is still valid YAML after the edit. The real, decisive verification -- confirming the new gate actually passes on a real CI run of the current code, and would actually fail on a deliberately-regressed one -- needs an actual CI run, not simulated locally; left for the next push.

## Finding #228 Fixed (Explicit Go-Ahead): `execute_frame`'s `clip_stack` -- All ~30 Call Sites Updated in a Dedicated Pass

Finding #222 (documented, not fixed) identified `execute_frame`'s `let mut clip_stack: Vec<ScissorRect> = Vec::new();` as a fresh per-frame heap allocation for any real scene that clips anything, and deliberately left the fix unapplied pending "the project owner's own explicit go-ahead before touching that many call sites in one pass" -- the blast radius (~30 call sites across `tre-engine`'s own test module, `tre-ffi`, `tre-python` (two renderers), and 19 `tre-rhi-vulkan` examples) was real, not assumed low-risk. The project owner gave that go-ahead explicitly, asking for the fix to be applied "workspace-wide in its own dedicated pass, updating all ~30 call sites deliberately rather than as a side effect of a review."

**Fixed.** `execute_frame` (`crates/tre-engine/src/rhi.rs`) gained a new `clip_stack: &mut Vec<ScissorRect>` parameter -- caller-owned scratch, `clear()`ed (not reallocated) at the top of every call, matching this crate's own zero-allocation-reuse convention elsewhere (`FrameArena`'s scratch fields, `ShapeRegistry::polygon_points_scratch`). The function's own arity crossed clippy's `too_many_arguments` default threshold (7) as a result; annotated with `#[allow(clippy::too_many_arguments, reason = "...")]` rather than restructured into a parameter struct, matching this workspace's own existing precedent for the same lint elsewhere (`tre-engine/src/canvas.rs`, `tre-engine/src/shapes.rs`, several `tre-python` sites) where a function's real arity reflects genuine caller-supplied inputs, not accidental sprawl.

Every real call site updated, matched to its own actual lifetime shape rather than one uniform pattern applied blindly:
- **Persistent per-renderer scratch** (a real, reused-every-frame render loop or long-lived renderer struct): `tre-ffi`'s `Renderer` (`crates/tre-ffi/src/renderer.rs`), `tre-python`'s `PyHeadlessRenderer` and `PyWindowedRenderer` (`crates/tre-python/src/renderer.rs`, `windowed_renderer.rs`) each gained a `clip_stack: Vec<ScissorRect>` field, initialized once and reused every `render`/`submit_and_read_bgra`/`submit_frame_to_window` call -- the same "built once, cleared/reset in place, never replaced" convention `canvas`/`arena`/`flattened` already follow on these same structs. `crates/tre-rhi-vulkan/examples/main_loop_demo.rs` and `shape_registry_zero_alloc_demo.rs` (the two real multi-frame render loops in the example suite) got a local `clip_stack` declared once outside their own frame loop instead, for the identical reason.
- **A closure called more than once**: `batching_equivalence_demo.rs`'s `render` closure (called twice, batched vs. unbatched) and `layer_oversize_regression_demo.rs`'s `run_frame` closure (called twice, two different layer sizes) each gained a `clip_stack` declared once before the closure and captured by unique reference -- both closures became `FnMut` (`let mut render`/`let mut run_frame`) as a direct, expected consequence, not an incidental side effect glossed over.
- **A genuine one-shot demo** (renders exactly one frame and exits, ~13 files: `arc_rounded_cap_demo.rs`, `blend_mode_demo.rs`, `canvas_batch_flattening_demo.rs`, `canvas_combined_scene_demo.rs`, `canvas_layer_composite_demo.rs`, `canvas_state_stack_demo.rs`, `canvas_sub_canvas_demo.rs`, `ellipse_sdf_fidelity_demo.rs`, `gradient_fill_demo.rs`, `layer_blur_demo.rs`, `path_and_polygon_demo.rs`, `shape_full_rendering_demo.rs`, `texture_fill_demo.rs`): a plain local `let mut clip_stack: Vec<ScissorRect> = Vec::new();` declared immediately before the `submit_frame` call -- correct and honest either way (a one-shot program has no "steady state" to reuse across), not treated as needing the same struct-field treatment as a real render loop.
- **`tre-engine`'s own 10 unit tests** exercising `execute_frame` directly (`crates/tre-engine/src/lib.rs`) each gained the identical local `let mut clip_stack: Vec<ScissorRect> = Vec::new();` alongside their existing `FakeCommandBuffer`/`FakeDevice` fixtures -- one `execute_frame` call per test, matching every other per-test fixture already declared fresh there.

**Verified.** Full workspace `cargo fmt --all -- --check` / `cargo clippy --workspace --all-targets -- -D warnings` / `cargo build --workspace --all-targets` clean in both debug and `--release`. `cargo test --workspace --no-fail-fast`: `tre-engine` 178/178 passing (unchanged), `tre-ffi`'s own real C harness test passing; release-mode run reproduces exactly the same pre-existing, unrelated `should_panic`/debug-assertion-reliant failures already documented elsewhere in this file (`tre-engine`'s three, `tre-memory`'s `alloc_guard` three) -- none new. Beyond the test suite: re-ran `canvas_state_stack_demo` (the one Rust example that actually exercises real `push_clip`/`pop_clip` GPU scissor clipping) against real GPU hardware -- unchanged, all pixel assertions pass; re-ran `shape_registry_zero_alloc_demo` (120 real frames) -- the zero-heap-allocation guarantee still holds exactly as before, confirming the new caller-owned `clip_stack` field doesn't reintroduce an allocation on the very path this fix was about; re-ran `main_loop_demo` (90 real frames, real multi-threaded canvas stitching) -- unchanged; re-ran both converted-to-`FnMut` closures' own demos (`batching_equivalence_demo`, `layer_oversize_regression_demo`) -- unchanged, all pixel assertions pass. Beyond Rust: rebuilt the `tre_python` extension and re-ran the Phase 12 Step 12.5 demo (the one Python demo that exercises `canvas.clip(...)` -> `PushScissor`/`PopScissor` through `PyHeadlessRenderer`, i.e. this exact `clip_stack` code path) and the Phase 12 Step 12.1 windowed-renderer demo (120 real frames through `PyWindowedRenderer::submit_frame_to_window`, including a real multi-window sequence) -- both passed with their own existing assertions unchanged, confirming the `self.field`-level disjoint-borrow arrangement `submit_frame_to_window`'s own resize-recovery loop needed (borrowing `self.clip_stack` mutably alongside `self.device`/`self.windows`/`self.ring_buffer` immutably) compiles and behaves correctly, not just type-checks.

## Finding #229 Fixed: `tre-perf-suite`'s Ramp Test Ring Buffer Was Too Small for Its Own Advertised Tier Ceiling

Building the new Phase 20 `tre-perf-suite` crate's Time-Ramp Stress Test, the first real run (not a simulated or assumed check) against real GPU hardware panicked partway through: `RING_BUFFER_CAPACITY = 16 * 1024 * 1024` (16MiB) looked generous on paper for a ring buffer holding one frame's vertex/index bytes, but the ramp reached a 65,536-primitive tier and `RhiDynamicRingBuffer::write` returned `None` ("ring buffer starved"), panicking the whole tool. The real cause: the buffer needs headroom for more than one frame's own raw bytes at once (multiple frames in flight before the GPU signals the fence freeing older regions), not just `primitive_count * bytes_per_primitive` for a single frame -- 65,536 rectangles alone need roughly 10MB of vertex+index bytes for one frame, which the 16MiB buffer had no real margin for once that multi-frame overlap is accounted for.

**Fixed** by raising `RING_BUFFER_CAPACITY` to 64MiB and lowering `MAX_TIER_COUNT` from an untested, arbitrarily-generous `1 << 20` (1,048,576) to a verified-safe `1 << 17` (131,072) -- a real, tested ceiling rather than a number picked without checking its own memory implications. This is a real bug in the new tool's own buffer sizing, not a genuine engine breaking point the tool's own "identify the exact breaking points" purpose cares about -- disclosed as such rather than reported as if the engine itself broke at 65,536 primitives.

**Verified.** Re-ran the identical `shapes` workload for a full 40-second ramp (durations/tier-seconds long enough to reach the new 131,072-primitive ceiling) -- no crash, reaching the top tier cleanly with real, rising CPU% visible in the telemetry (near-0% at low tiers, a sustained ~58-62% -- briefly exceeding 100%, i.e. more than one CPU core, at 131,072 primitives) while FPS stayed largely vsync-capped around 58-62 throughout (this development machine's real GPU is not yet the bottleneck at this primitive count and window size -- a real, honest finding, not a tool defect). See `documentation/IMPLEMENTATION.md`'s Phase 20 Step 20.1 write-up for the complete account of this new crate.

## Finding #230 Fixed: `tre-perf-suite`'s Resize Test Crashed with `DeviceLost` on the Project Owner's First Real Interactive Resize

The project owner's own first real, manual run of the Interaction-Driven Resize Test (`--profile resize`) -- the exact scenario the sandbox itself could not exercise (no `xdotool`/`wmctrl`, disclosed in finding #229/IMPLEMENTATION.md Phase 20) -- crashed on the very first window resize: `thread 'main' panicked ... failed to create VulkanSwapchain: DeviceLost`, preceded by a real Wayland compositor error, `wp_fifo_manager_v1#63: error 0: Attempted to create a second fifo surface for the wl_surface`.

**Root cause**: `Surface::create`'s replacement (`surface = Surface::create(&device, &connection, window, width, height)`) constructs the *new* `Surface` -- which requests a brand-new `VkSurfaceKHR`/Wayland fifo surface from the compositor -- while the *old* `Surface` (still bound to the same underlying `wl_surface`) has not been dropped yet, since Rust only drops a variable's previous value *after* the assignment's right-hand side is fully evaluated. Wayland's `wp_fifo_manager_v1` protocol allows only one fifo surface per `wl_surface` at a time, so the compositor correctly rejected the second request, corrupting the surface and surfacing downstream as `DeviceLost` on `VulkanSwapchain::new`. `crates/tre-python/src/windowed_renderer.rs`'s own `WindowSlot`-via-`HashMap::insert` resize path has this exact same latent ordering issue (the old `WindowSlot` is likewise still alive while the new one's `create_surface` call runs) -- not fixed here, since it's shared code outside this new crate's own scope, but real and worth a project-owner decision on a future pass.

**Fixed** by making `resize_test.rs`'s `surface` an `Option<Surface>` and adding a `rebuild_surface` helper that explicitly sets it to `None` (running the old `Surface`'s `Drop`, which does call `surface_loader.destroy_surface` on its own `VkSurfaceKHR`) *before* constructing the replacement -- guaranteeing the old fifo-surface binding is fully released before the new one is requested.

**Verified** by the project owner's own second real run: an extended, real interactive session (~30+ live resize events over roughly 36 seconds of continuous window dragging, real rebuild latencies logged between 0.68ms and 2.25ms), no crash, and a clean exit (`Interaction-Driven Resize Test complete.`, exit code 0) on closing the window -- the exact scenario that crashed immediately before this fix.

**Follow-up, at the project owner's explicit request ("Fix that one too"): the identical bug in `crates/tre-python/src/windowed_renderer.rs` is now fixed too.** `PyWindowedRenderer::submit_frame_to_window`'s own `SwapchainOutOfDate` resize-recovery arm had the exact same ordering bug: `WindowSlot::create(&self.device, &self.connection, window, width, height)?` (which calls `device.create_surface`, requesting a new `VkSurfaceKHR`/fifo surface) ran while the stale `WindowSlot` for that same `window` was still present in `self.windows` -- only dropped later, when `self.windows.insert(window, fresh)` replaces it. Fixed with the same real remedy: `self.windows.remove(&window);` immediately before the `WindowSlot::create` call, explicitly dropping the stale slot (and its `VkSurfaceKHR`) first. `create_window`'s own `self.windows.insert(window, slot)` (a genuinely new `WindowId`, never previously in the map) was checked and confirmed to not share this bug -- there is no old entry there to conflict with.

**Verified.** Full workspace `cargo fmt --all -- --check`/`cargo clippy --workspace --all-targets -- -D warnings`/`cargo build --workspace --all-targets`/`cargo test --workspace` clean in debug and `--release` (unchanged pass/fail counts). Rebuilt the `tre_python` extension (`maturin develop --release`) and re-ran the Phase 12 Step 12.1 windowed-renderer demo -- still passes unchanged (120 real frames, a real `Resized` event observed, a real second-window multi-window sequence).

**Then verified decisively, at the project owner's request, with a real live interactive run.** A small ad hoc script (not a project demo asset) opened a real `WindowedRenderer` window and rendered continuously until closed, logging every `Resized` event and re-raising any exception from `render()`. The project owner dragged the window aggressively for the full session: **603 real resize events over 20.8 seconds, 1,217 frames rendered** -- more than 10x the resize-event volume the earlier `tre-perf-suite` verification produced -- with zero exceptions and a clean exit (`CloseRequested received after 20.8s, 1217 frames, 603 resize event(s) -- exiting cleanly`). This is the real, decisive confirmation the regression demo alone couldn't provide: the fix holds under sustained, aggressive real-world resize stress, not just in principle by analogy to `tre-perf-suite`'s own already-proven fix.

## Finding #231 Fixed: `tre-perf-suite`'s Resize Test Rebuilt the Swapchain Once Per Queued `Resized` Event Instead of Coalescing to the Latest

The project owner shared a sibling Python project's (`pyCopper`) `LESSONS_LEARNED.md`, which documents an almost identical real bug it already found and fixed in its own native-window resize handling: `rendercanvas`'s `_on_size_change` fired one full synchronous render per queued resize event, since `glfw.poll_events()` drains the whole backlog before returning -- "a genuine backlog, not a metaphor," fixed by coalescing/rate-limiting. `resize_test.rs`'s own `for event in connection.poll_events() { ... InputEvent::Resized => rebuild_surface(...) }` had the identical shape: every `Resized` event drained in one `poll_events()` call triggered its own full surface/swapchain/pipeline rebuild, even when a later event in the same batch immediately superseded it with a newer size -- real, measured waste, not hypothetical: the project owner's own prior interactive run (this finding's own predecessor, above) logged 603 rebuilds over 20.8 seconds of dragging.

**Fixed** by draining `poll_events()` into two plain local variables first (`close_requested: bool`, `latest_resize: Option<(u32, u32)>`) instead of acting inside the match arm directly, then rebuilding at most once per loop iteration -- against whichever size was still current by the time the whole batch was drained, not the first (likely already-stale) one seen. This changes nothing about *what* a resize rebuild does, only how often it's triggered; the rebuild itself, and the fix for finding #230 it already carries, are untouched.

**Verified** by the project owner's own second real interactive run, using the identical drag-heavy methodology as finding #230's own verification: **143 real rebuilds over roughly 69 seconds of dragging (~2/sec)**, versus the predecessor run's **603 rebuilds over 20.8 seconds (~29/sec)** -- roughly a 14x drop in rebuild rate under comparably aggressive dragging, with zero exceptions and a clean exit on closing the window. Full workspace `cargo fmt --all -- --check`/`cargo clippy --workspace --all-targets -- -D warnings`/`cargo build --workspace --all-targets` clean in debug and `--release`.

**A related, deeper finding surfaced while answering the project owner's own follow-up question** (whether pyCopper's *other* technique -- pinning the swapchain to a coarse size during an active drag, per its own `GPUCanvasContext.set_physical_size` trick, rather than reacting to every resize at all -- would help or hurt here): `VulkanSwapchain::new` takes an already-existing `VkSurfaceKHR` as a parameter; it does not create one. But `Surface::create` (both in `tre-perf-suite` and, per finding #230, `windowed_renderer.rs`) calls `device.create_surface(...)` fresh on every resize, tearing down and rebuilding the *surface* object itself, not just the swapchain -- not idiomatic Vulkan practice, where one `VkSurfaceKHR` normally lives for the whole window and only the swapchain is recreated on resize (typically via `oldSwapchain` for a smooth handoff). This is very likely the real, root-level reason finding #230's `wp_fifo_manager_v1` conflict existed at all -- the fifo-surface binding lives on the surface, not the swapchain -- and is a real, disclosed, **not yet fixed** architectural gap, left open for the project owner's own explicit direction on whether to pursue it as a follow-up (a bigger change than this finding's own coalescing fix, touching both `tre-perf-suite` and `windowed_renderer.rs`).

## Finding #232 Fixed (Explicit Go-Ahead): Resize Never Needs to Recreate the Surface -- Only the Swapchain

Finding #231's own writeup identified the real, root-level cause behind finding #230's `wp_fifo_manager_v1` crash: every resize path in the workspace tore down and rebuilt the entire `VkSurfaceKHR`, not just the `VkSwapchainKHR`, via a fresh `VulkanDevice::create_surface` call -- not idiomatic Vulkan practice, and the actual reason the Wayland fifo-surface conflict could happen at all (that protocol's binding lives on the surface, not the swapchain). Left open pending the project owner's own explicit direction, since it was a bigger, more invasive change than finding #231's own coalescing fix. The project owner's answer: **"Yes, scope that too."**

**Fixed** by adding a genuine new capability to the shared engine crate rather than patching each caller separately: `VulkanSwapchain::recreate(&mut self, device, width, height) -> Result<(), EngineError>` (`crates/tre-rhi-vulkan/src/swapchain.rs`). `new`'s own original body was factored into a private `build_swapchain` helper (surface-capability/format query through stencil-image-view creation) that both `new` and `recreate` now share; `recreate` waits for the device to go idle, destroys every one of the swapchain's own *dependent* resources (semaphores, image views, stencil image/view/memory, the old `VkSwapchainKHR` itself) in the same child-before-parent order `Drop` already uses, then calls `build_swapchain` again against the *same*, untouched `self.surface`/`self.surface_loader` -- the surface itself is never destroyed or recreated at any point. Deliberately does not use `VkSwapchainCreateInfoKHR::oldSwapchain`'s retire-but-still-must-destroy handoff mechanism: this project's own fully-synchronous, single-frame-in-flight render loop (each frame's own fence wait already ensures no GPU work is in flight by the time a resize is handled) has no in-flight-work case that mechanism exists to smooth over, so destroy-then-rebuild against the idle device is simpler and equally correct here.

Both real resize call sites updated to use it:
- **`crates/tre-perf-suite/src/resize_test.rs`**: `Surface` gained a `resize()` method calling `self.swapchain.recreate(...)` and updating `width`/`height` -- `pipelines` is left completely untouched (a swapchain's color format is a property of the surface, which `recreate` never touches, so re-registering shape pipelines on resize was always unnecessary work). The `Option<Surface>` wrapper and free `rebuild_surface` function finding #230 introduced are both gone -- no longer needed, since the surface (finding #230's own root cause) is never torn down at all now.
- **`crates/tre-python/src/windowed_renderer.rs`**: `WindowSlot.swapchain` reverted from `Box<dyn RhiSwapchain>` (REVIEW.md finding #216's own trait-object generalization) back to concrete `VulkanSwapchain` -- `recreate` is a Vulkan-specific capability with no `RhiSwapchain` trait equivalent, and finding #216's own stated premise for boxing it ("every real use of it already went through `&dyn RhiSwapchain`") no longer holds now that resize needs the concrete type directly. This mirrors, rather than contradicts, finding #216's own precedent: that finding already kept this exact struct's `device` field concrete for the identical "ongoing concrete-{device,swapchain} need" reason. `WindowSlot` gained a `resize()` method identical in shape to `tre-perf-suite`'s; `submit_frame_to_window`'s `SwapchainOutOfDate` retry arm now calls `self.windows.get_mut(&window)....resize(...)` in place instead of `self.windows.remove(&window)` + `WindowSlot::create` (a brand-new surface) + `self.windows.insert(...)`.

**Verified.** Full workspace `cargo fmt --all -- --check`/`cargo clippy --workspace --all-targets -- -D warnings`/`cargo build --workspace --all-targets`/`cargo test --workspace` clean in debug and `--release` (unchanged pass/fail counts -- this pass touched no test-covered logic paths, only construction/recreation code exercised solely via real GPU runs below). Every one of the four other real `VulkanSwapchain::new` call sites that never resize (`walking_skeleton.rs`, `input_demo.rs`, `main_loop_demo.rs`, `multi_window.rs`) re-run against real GPU hardware unchanged, confirming the `build_swapchain` extraction is behavior-preserving for ordinary construction, not just for the new `recreate` path. Then the real, decisive test: the project owner ran `tre-perf-suite`'s resize test again, dragging aggressively -- **168 real rebuilds over ~33.5 seconds**, zero exceptions, clean exit, and per-rebuild latency visibly *lower* than finding #231's own measurement (0.5-1.3ms here vs. 0.68-2.3ms there), consistent with no longer paying for a surface recreation or pipeline re-registration on every resize. Rebuilt the `tre_python` extension and re-ran both the Phase 12 Step 12.1 regression demo (unchanged, 120 real frames) and the same ad hoc interactive script finding #230/#231 used -- the project owner dragged the window through **636 real resize events over 17 seconds (997 frames)**, zero exceptions, clean exit. Both real call sites hold under real, aggressive interactive stress with the surface never once recreated.

## Finding #233 Confirmed (Not a Bug): Resizing a Window Does Not Squash or Stretch Already-Placed Content

The project owner stated a real requirement directly: "When a window resizes, the user does not expect the rendered content to squash and stretch. They expect the rendering to stay the same and just the window (viewport) to resize." Rather than assume this already holds from reading `sdf_rounded_rect.vert`'s `vec2 ndc = (in_position / pc.screen_size) * 2.0 - 1.0;` transform, this was checked the way this project checks everything real: rendered and measured, not reasoned about from code alone.

**Traced the full chain first.** `pc.screen_size` (the push constant every shape pipeline's vertex shader divides a shape's own absolute pixel position by) is set in `VulkanCommandBuffer::draw_indexed` from `self.width`/`self.height`, which `VulkanDevice::begin_frame` sets on every newly-built command buffer from `let (width, height) = swapchain.extent();` -- queried fresh, every single frame, from whatever the swapchain's *current* real extent is (which finding #232's own `VulkanSwapchain::recreate` keeps accurate across a resize). The same `width`/`height` also drives the real GPU viewport/scissor set in that same function. Mapping an absolute pixel coordinate to NDC by dividing by the *current* viewport's own real extent is the standard, correct pixel-space transform: it keeps a shape's own physical on-screen pixel position and size invariant to the viewport's size, precisely because NDC space is redefined every frame to match whatever the real framebuffer currently is.

**Verified directly, not just traced.** New example `crates/tre-rhi-vulkan/examples/resize_invariance_demo.rs`: renders one `Rectangle` at a fixed absolute pixel position/size (`x=60, y=40, w=80, h=50`), completely unchanged, into two independent `HeadlessSwapchain`s of different widths (300px and 600px, same height) and reads back real pixels from both. Confirms, by direct sample: the rectangle's own interior is red at the identical absolute coordinate in both images; the region just past its real right edge is background in both; and a coordinate that only exists in the wider canvas (x=500, outside the 300px-wide image entirely) is background, not stretched red content. All four checks passed on the first real run.

**Conclusion: this was already correct by design, confirmed by measurement rather than assumed.** No code change was needed to satisfy the stated requirement -- `resize_invariance_demo.rs` is kept as a new, permanent regression example (wired into `ci.yml`'s `vulkan-validation` job, matching every other real example) guarding this specific, now-explicitly-stated invariant going forward, since nothing in the existing test suite checked it directly before this.

## Finding #234 Fixed: A Real, User-Confirmed Squash/Stretch Bug -- `tre-python`'s Windowed Resize Path Was Purely Reactive

Despite finding #233 confirming the core engine's own pixel-space coordinate math is resize-invariant, the project owner reported real, visible squash/stretch when resizing a window and asked specifically to check `tre-ffi` and `tre-python`. `tre-ffi` was ruled out immediately: its own module doc comment states "no windowed rendering yet" -- there is no live-window resize code path there at all, so this could not be its bug. `tre-python`'s `PyWindowedRenderer` was the only real windowed-resize code path in the workspace remaining, and it had one.

**Root cause.** `PyWindowedRenderer::poll_events` updates `slot.width`/`slot.height` (bookkeeping fields) the instant a real `InputEvent::Resized` arrives, but never touched the *swapchain itself* -- this module's own doc comment for `poll_events` stated the actual design intent outright: "a subsequent `render()` call recreates that window's swapchain automatically if needed," i.e. purely reactive, only resizing after a real `EngineError::SwapchainOutOfDate` eventually surfaced from `vkAcquireNextImage`/`vkQueuePresent`. Wayland does not report that error promptly: instead of erroring, it silently stretches the still-stale-sized swapchain image to fit the window's already-resized real surface bounds -- real, visible squash/stretch for as long as the swapchain lagged behind the real window size, not a one-frame glitch. This is a materially different bug from findings #230-#232 (which were about *how* a resize gets carried out, a crash and inefficiency in the mechanism itself); this one is about *when* -- the mechanism was never triggered promptly enough in the first place.

**Fixed** by adding a proactive check at the top of `submit_frame_to_window` (the function every one of `render`/`render_canvas`/`render_parallel`'s own window-submission paths already shares): before ever attempting to submit a frame, compare `slot.swapchain.extent()` (the swapchain's own actual current size) against `slot.width`/`slot.height` (the latest real size `poll_events` already recorded), and call the existing `WindowSlot::resize` (finding #232) immediately if they differ. By the time `submit_frame` ever runs, the swapchain's real extent already matches the window's real size -- Wayland is never handed a stale-sized buffer to stretch in the first place. The existing reactive `SwapchainOutOfDate` retry-once fallback is left in place unchanged, as a safety net for any other cause of the same error (a minimize/restore, a compositor-forced resize outside the normal event stream) that this proactive check doesn't cover.

**Verified.** Full workspace `cargo fmt --all -- --check`/`cargo clippy --workspace --all-targets -- -D warnings`/`cargo build --workspace --all-targets`/`cargo test --workspace` clean in debug and `--release` (unchanged pass/fail counts). Rebuilt the `tre_python` extension and re-ran the Phase 12 Step 12.1 regression demo unchanged. Then the decisive test: the project owner ran the same ad hoc interactive resize script used for findings #230/#232 again (221 real resize events over 12.3 seconds this run, clean exit) and this time specifically watched the rendered shapes -- not just whether it crashed. **Confirmed by the project owner directly: "yes it's fixed now, no more stretching."** This is the one verification in this whole resize-fix arc that genuinely could not be automated or inferred from pixel math alone (`resize_invariance_demo.rs`'s own headless test proves the coordinate math is correct in isolation, but cannot exercise a live compositor's own buffer-scaling behavior against a real, temporarily-stale-sized swapchain) -- a real human had to watch a real window resize and say so.

## Finding #235 In Progress: A Separate Real Symptom -- "The Window Slowly Trails Then Jumps to the Cursor" During an Active Drag, Isolated to `vkAcquireNextImageKHR`

With finding #234 confirmed fixed, the project owner asked to load `tre-perf-suite`'s resize test again and stay ready to evaluate live telemetry, then reported a second, distinct real symptom observed during active dragging: "the window slowly trails then jumps to the cursor." This is a frame-pacing/latency complaint, not a correctness one (finding #233 already proved the coordinate math itself is resize-invariant) -- the window border visibly lags the cursor during continuous drag motion, then snaps forward, rather than tracking smoothly.

**First instrumentation pass: localize the stall to one pipeline stage.** Added a diagnostic-only `write_frame_stall` telemetry record (`crates/tre-perf-suite/src/telemetry.rs`) and wrapped `resize_test.rs`'s own frame loop in `std::time::Instant`-based timers around its two coarse stages -- `poll` (draining `poll_events()` and coalescing to the latest resize, finding #231's own logic) and everything from `registry.mark_all_dirty()` through `submit_frame`'s return (`record` + the GPU submit, originally left as one combined span) -- logging only when either stage exceeded an 8ms threshold (well beyond a 60Hz frame budget). The project owner dragged the window through a real live session and confirmed "done resizing." **Result: `poll_ms` was consistently sub-1ms throughout, `record_ms` frequently spiked into the hundreds of milliseconds during active dragging** -- conclusively ruling out event-polling itself (finding #231's own coalescing) as the cause, and narrowing the search to whatever `record_ms`'s single combined span was actually measuring: CPU-side scene flattening/ring-buffer writes, or the GPU submit/present call bundled in the same span.

**Second instrumentation pass: split the GPU submit into acquire vs. present.** `record_ms`'s span was narrowed to cover only the real CPU-side work (`registry.flatten_into`/`canvas.flatten_into`/ring-buffer writes), and two new spans were added around the previously-bundled GPU call: `acquire_ms` (`device.begin_frame`, which wraps `vkAcquireNextImageKHR`) and `present_ms` (`device.submit_and_present`, which wraps `vkQueuePresentKHR`) -- including both retry-arm calls in the `SwapchainOutOfDate` resize-recovery fallback, so a stall hiding inside a rare recovery path wouldn't be missed. The project owner ran a second live drag session and confirmed "done resizing." **Result: conclusive.** `record_ms` was consistently sub-1ms (ruling out CPU-side scene work entirely); `present_ms` was consistently sub-1ms every single frame (ruling out `vkQueuePresentKHR`); `acquire_ms` -- and only `acquire_ms` -- spiked to between 470ms and 1,865ms on individual frames during active dragging. The "trails then jumps" visual symptom is exactly what a multi-hundred-millisecond block inside `vkAcquireNextImageKHR` looks like from the user's side: the render loop (and therefore the window's own redraw) freezes mid-drag while the call blocks, then several queued resize events land and get processed back-to-back the instant it returns, producing the observed "catches up in a jump."

This connects directly to a real, previously-solved analogous bug in the project owner's sibling Python project: `pyCopper`'s own `LESSONS_LEARNED.md` documents "Wayland resize trailing -- the swapchain is pinned to a coarse size during a drag (acquire cost 1.8ms -> 0.05ms)," the same root mechanism (an oversized/mismatched swapchain making acquire expensive under Wayland's own resize-negotiation protocol during a live drag) causing the same visible symptom in a different codebase.

**Candidate fix (Option 1) tested and disproven: increasing swapchain image count.** The cheapest candidate remedy -- more images in flight give the compositor more slack before `vkAcquireNextImageKHR` has to block waiting for one to free up -- was tried at the project owner's explicit request ("Yes, try option 1 first"): `crates/tre-rhi-vulkan/src/swapchain.rs`'s `image_count` calculation changed from `capabilities.min_image_count + 1` to `+ 2` (still clamped to `capabilities.max_image_count`). Re-run against a real live drag session: mean `acquire_ms` was statistically unchanged (53.0ms after vs. 55.1ms before, across comparably aggressive dragging) -- a genuine, honest negative result, not a measurement fluke. **Reverted** (`git checkout` back to `min_image_count + 1`, confirmed via `git diff` to be the experiment's only footprint) rather than keeping an unproven change in the tree.

**Option 2 implemented and measured, at the project owner's explicit direction ("Yes, try option 2").** `resize_test.rs`'s resize handling (`crates/tre-perf-suite/src/resize_test.rs`) now rounds every `Resized` event's requested size up to the nearest `DRAG_COARSE_STEP` (64px) bucket and resizes the swapchain to that coarse size instead of the exact live size; once no new `Resized` event has arrived for `DRAG_SETTLE_MS` (150ms), one final resize snaps the swapchain to the exact last-requested size, so the window is pixel-accurate whenever the drag isn't actively in progress. This is pyCopper's own proven fix for the identical symptom (`LESSONS_LEARNED.md`: "the swapchain is pinned to a coarse size during a drag"), and deliberately, temporarily re-introduces finding #234's own squash/stretch for the duration of an active drag only -- while the swapchain sits at a coarse size that doesn't match the window's real size, the compositor visibly stretches that buffer to fit, exactly the tradeoff finding #231's writeup already flagged when this option was first floated.

**Verified with a real live drag session (33.6 seconds, 276 total resize events).** Of those 276, 269 landed on the coarse 64px grid (needing zero swapchain recreation apiece whenever consecutive drag positions shared a bucket) and 7 were exact settle-snaps triggered when the drag paused -- confirming both halves of the mechanism fired for real, not just in principle. The result on the metric findings #235's own instrumentation was built to isolate: **peak single-frame `acquire_ms` dropped from the pre-fix range of 470-1865ms down to 80-114ms** -- a roughly 4-16x reduction in the worst-case stall, measured on a comparably aggressive real drag session. This does not fully eliminate elevated acquire time during an active drag (80-114ms is still well above the steady-state ~16ms `vkAcquireNextImageKHR` wait this same log shows during ordinary FIFO-vsync-paced rendering), but it is a substantial, measured improvement over both the unfixed baseline and the disproven Option 1.

**Full workspace verification clean:** `cargo fmt --all -- --check`/`cargo clippy --workspace --all-targets -- -D warnings`/`cargo build --workspace --all-targets` clean in debug and `--release`; `cargo test --workspace --no-fail-fast` clean (unchanged pass/fail counts -- this fix touched no test-covered logic paths, only `resize_test.rs`'s own interactive loop, exercised solely via the real GPU run above).

**Option 3 implemented and measured, at the project owner's explicit request to compare it against Option 2 before deciding ("I am leaning towards option 2, but I want to test option 3 before I make up my mind").** Unlike Options 1/2, this required real, additive engine-level API surface, not just a change inside `resize_test.rs`: `RhiSwapchain::acquire_next_image_with_timeout`/`RhiDevice::begin_frame_with_timeout` (`crates/tre-engine/src/rhi.rs`), each a new trait method with a default implementation delegating to the existing unbounded method -- so every other backend/consumer in the workspace (`HeadlessSwapchain`, the test-only `FakeDevice`, and all ~20 `tre-rhi-vulkan` examples plus `tre-ffi`/`tre-python`) compiles and behaves exactly as before with zero changes. `VulkanSwapchain::acquire_next_image_with_timeout` (`crates/tre-rhi-vulkan/src/swapchain.rs`) is the one real override, passing `timeout_ns` straight through to `vkAcquireNextImageKHR` and mapping `VK_TIMEOUT` to a new `EngineError::AcquireTimedOut` variant (added to `crates/tre-engine/src/lib.rs`, with the required mechanical `TreErrorCode::AcquireTimedOut` shadow-enum addition in `crates/tre-ffi/src/error.rs`, appended last so no existing FFI discriminant shifts).

**A real correctness hazard, found and fixed before this could ship at all:** `VulkanDevice::begin_frame`'s own body (`crates/tre-rhi-vulkan/src/device.rs`) reset the single-frame-in-flight fence immediately after waiting on it, *before* attempting the acquire. That ordering is silently safe under the old unbounded acquire (it always eventually succeeds), but a bounded acquire that times out and lets its caller skip the frame would leave the fence reset with no corresponding `submit_and_present` ever signaling it again -- deadlocking the *next* frame's own wait forever. Fixed by extracting the original body into a new `begin_frame_impl(swapchain, timeout_ns)` (both `begin_frame` and `begin_frame_with_timeout` are now thin wrappers around it) and moving the fence reset to run only *after* a successful acquire -- on a timeout, the fence is left exactly as the wait found it (signaled), so the next iteration's own wait returns immediately. Verified behavior-preserving for the existing unbounded path: `tre-engine`'s 178/178 unit tests still pass unchanged, and the reorder touches only the relative order of two calls with nothing else able to observe the fence in between (this device's own single-threaded, fully-synchronous submission model).

`resize_test.rs` gained a `ResizeStrategy` (`Coarse`/`Timeout`) selectable via a new `--resize-strategy` CLI flag, so Option 2 and Option 3 can be run and compared independently rather than combined: `Timeout` always resizes to the exact live size (no coarse bucketing, isolating Option 3's own effect) and bounds every primary acquire to a new `ACQUIRE_TIMEOUT_MS` (20ms), skipping that tick's render entirely (a new `write_acquire_skip` telemetry record) when it's exceeded, rather than the belt-and-suspenders `SwapchainOutOfDate` retry arm's own acquire, which deliberately stays unbounded (it runs immediately after a fresh, correctly-sized swapchain rebuild -- not the stale-vs-live-size mismatch this option targets, and already a rare fallback path).

**A second real bug, caught on the very first live run, before the project owner ever saw it:** the loop's existing structure did its CPU-side scene recording *and wrote fresh vertex/index data into the shared ring buffer* unconditionally, before the acquire call -- a holdover from the unbounded-acquire design, where every iteration was always going to submit. With a bounded acquire that can now legitimately skip a frame, every skip still wrote into the ring buffer's current frame-in-flight segment without ever calling `submit_and_present` to advance `frame_sync.frame_index` and free that segment back up. The very first real drag session hit this immediately: 41 skips in ~12.7 seconds exhausted the segment and crashed with `thread 'main' panicked ... ring buffer write failed for vertices`. **Fixed** by reordering the loop so recording/ring-buffer-writing happens only *after* a successful acquire -- a skipped frame now writes nothing, matching the "drop the frame" semantics Option 3 is actually supposed to have. Full workspace `cargo fmt --all -- --check`/`cargo clippy --workspace --all-targets -- -D warnings`/`cargo build --workspace --all-targets`/`cargo test --workspace --no-fail-fast` clean in debug and `--release` after the fix (unchanged pass/fail counts).

**Verified with a real live drag session after the fix (41.2 seconds, clean exit, no crash): 796 resize events, 1,353 skipped frames.** Peak single-frame `acquire_ms` was capped at 18.2ms -- never exceeding the 20ms bound, confirming the timeout mechanism itself works exactly as designed, and a dramatically tighter worst case than both the unfixed baseline (470-1865ms) and Option 2 (80-114ms). But the cost is real and large: skips landed at a sustained ~35-40 per 5-second window throughout the whole drag (not a rare edge case), meaning the render loop dropped a very substantial fraction of its frames rather than merely running one or two of them slow. Unlike Option 2, the swapchain always matches the real window size exactly, so Option 3 never reintroduces finding #234's squash/stretch -- but a very high, sustained frame-drop rate is its own real, different visible cost (likely a stutter/freeze-then-catch-up feel, rather than Option 2's brief blur), and this tool's own instrumentation cannot judge which of the two *feels* better without the project owner actually watching the window, matching every other perceptual judgment call in this investigation.

**Decided: Option 2 (coarse-bucketed resizing, `ResizeStrategy::Coarse`), at the project owner's explicit direction ("Option 2 is the way I want to go") after directly comparing both.** It remains `resize_test.rs`'s default strategy (`--resize-strategy coarse`, matching what shipped before Option 3 was even built) -- no code change was needed to make this the shipping choice. Option 3 (`--resize-strategy timeout`) is kept in the tree rather than reverted: it is real, correct, and fully verified (unlike the disproven and reverted Option 1), so it stays available as a documented, working alternative and a direct comparison baseline for any future revisit of this tradeoff, rather than being discarded now that a decision has been made.

**Status: closed for `tre-perf-suite`'s own resize test.** All three candidate remedies were tried, measured, and honestly disclosed (Option 1 disproven and reverted; Option 2 selected; Option 3 built, measured, and kept as an alternative); the "trails then jumps" symptom in the perf-suite's own diagnostic tool is resolved by Option 2. Finding #231's own writeup already flagged that `crates/tre-python/src/windowed_renderer.rs` shares the identical resize-handling shape this whole investigation targeted -- whether to port Option 2's coarse-bucketing fix into that real production render path is a separate, not-yet-made decision, since it would (deliberately) reintroduce finding #234's own squash/stretch into the actual product during a drag, the exact behavior the project owner originally asked to eliminate there.

**Ported to `crates/tre-python/src/windowed_renderer.rs` at the project owner's explicit request ("Yes, port it to windowed_renderer.rs"), and then genuinely corrected after a live test surfaced a real gap.** The first port carried over only half of Option 2's mechanism (coarse-bucketed swapchain resizing) and, run live, produced exactly the squash-then-jump the project owner described directly: "the shapes begin to squash, then jump suddenly to the right size" -- a real, correctly-reported defect, not a matter of taste.

**Root-caused by reading pyCopper's actual source, not just its lessons file, per the project owner's own explicit push ("are you sure you did a complete look at how pyCopper fixed this issue?").** `pycopper/src/pycopper/runtime/engine.py`'s `_pin_surface`/`_upload` (and its own `tests/test_resize_pin.py`) reveal the real mechanism this project had missed: content is always projected using the **true, exact window size**, even while the swapchain buffer itself is deliberately held at a coarser size. The buffer being larger than the window is what lets the compositor's own scale-to-fit (the identical mechanism finding #234 fixed as a bug when it happened *unintentionally*) exactly cancel the resulting pre-stretch -- two inverse transforms, net zero visible distortion, at the cost of one real, disclosed resample (pyCopper's own test file: "that resample softens text... it does not crop"). This project's first port instead derived the NDC projection from the swapchain's own (coarse) extent, so there was nothing to cancel the compositor's stretch -- the *only* transform applied, hence the visible squash. Also found: pyCopper's actual validated constants are `resize_bucket = 256` (not this project's own untested 64px guess) and a **frame-count** settle (`SETTLE_FRAMES = 3`, decremented once per real frame via its own `surface_size_for` state machine) rather than a wall-clock timer -- its own doc comment explains why: "a resize draws synchronously per compositor configure, so this is a handful of frames after the drag stops, not a wall-clock delay."

**Fixed with a genuine new engine capability, not a `resize_test.rs`/`windowed_renderer.rs`-local patch.** `RhiDevice` gained `begin_frame_with_logical_size` (`crates/tre-engine/src/rhi.rs`), a default-delegating trait method exactly like Option 3's own `begin_frame_with_timeout` -- zero blast radius to every other backend/caller. `VulkanDevice`'s real override (`crates/tre-rhi-vulkan/src/device.rs`) is a small, surgical change: `begin_frame_impl` gained a `logical_size: Option<(u32, u32)>` parameter that, when `Some`, sets only `VulkanCommandBuffer::width`/`height` (the fields `draw_indexed`'s `screen_size` push constant reads) to the logical size -- the GPU viewport/scissor/render area and `swapchain_width`/`swapchain_height` are untouched, still the swapchain's own real physical extent. This reuses an *existing* distinction the codebase already had for a different reason (`begin_render_to_texture`/`resume_swapchain_rendering`'s own "active render target size" vs. "the swapchain's own stable extent" split), not a new concept. A parallel `submit_frame_with_logical_size` free function (mirroring `submit_frame`) was added so `windowed_renderer.rs` didn't need to inline `begin_frame`/`execute_frame`/`submit_and_present` by hand the way `resize_test.rs` already does for its own instrumentation reasons.

Both consumers now use pyCopper's own validated constants (`DRAG_COARSE_STEP = 256`, `SETTLE_FRAMES = 3`) and its exact `surface_size_for` state machine, ported as `coarse_target_for` in each (kept duplicated across the two crates deliberately -- both are small enough that a shared crate for this alone isn't warranted, matching this workspace's own established precedent, e.g. every example's own duplicated window/device bootstrap sequence). `windowed_renderer.rs`'s `submit_frame_to_window` now projects every frame against `slot.width`/`slot.height` (the true, exact window size) via `submit_frame_with_logical_size`, regardless of whether the swapchain itself currently sits at a coarse or exact size -- the two cases need no special-casing, since a settled (non-dragging) window has both values equal anyway.

**Verified via `tre-perf-suite`'s own `--resize-strategy coarse` first** (matching this whole investigation's established "cheap test tool before the real renderer" order): full workspace `cargo fmt --all -- --check`/`cargo clippy --workspace --all-targets -- -D warnings`/`cargo build --workspace --all-targets`/`cargo test --workspace --no-fail-fast` clean in debug and `--release` (`tre-engine`'s 178/178 unit tests unchanged, confirming the logical-size override is fully behavior-preserving when unused). A real 55-second live drag session: clean exit, no crash, 138 resize events, all landing on real 256px-bucket multiples (768x768, 1024x1024, etc.) or an exact settle size, confirming the ported state machine fires correctly.

**One honest, not-yet-resolved wrinkle found in that same run: pyCopper's own 256px bucket does not obviously transfer cleanly to this project's own environment.** Acquire time stayed at its normal ~16ms baseline for the large majority of the session (median 16.3ms, p95 17.2ms across 2,824 samples) -- but a concentrated ~14-second stretch of the drag (coinciding with the largest bucket sizes reached, 1024x1024/1280x768/1280x512) produced 19 individual stalls between 129ms and 415.7ms, a magnitude close to the *original*, unfixed problem this whole investigation started from. The smaller, earlier 64px-bucket test never reached those large absolute sizes and never showed stalls above ~114ms. Whether the cost scales with the absolute buffer size reached (not just how often it's rebuilt) is a real, open, environment-specific question this project's own single dev machine (KWin/KDE Plasma Wayland, the same compositor pyCopper's own numbers were measured on) has not fully answered -- disclosed here rather than assumed away, matching this project's own "verify empirically" standard. Bucket size may need its own further tuning pass independent of the projection fix, which is itself confirmed structurally sound.

**Status: the squash/stretch defect itself is fixed and mechanistically understood; the project owner has not yet confirmed it live in `windowed_renderer.rs` (extension rebuilt, ready to test); bucket-size tuning remains a separate, open, disclosed question.**

**A second real bug found on that first live test of the actual port -- the fix above was structurally correct but never actually took effect in `windowed_renderer.rs`.** The project owner's own live test showed real, visible squash ("the circle is squashed as is the hexagon"), still occurring after the projection fix landed. Root cause: `WindowSlot::resize` (unchanged from finding #232/#234's own era, when it was only ever called with the exact real size) unconditionally overwrote `self.width`/`self.height` with whatever size the swapchain was just resized to. Once the proactive resize path started calling it with the *coarse* target during an active drag, this silently clobbered the very fields `submit_frame_to_window` reads as "the true logical size" for `submit_frame_with_logical_size` -- after the first coarse resize, the logical size fed to the projection became the coarse size too, so there was nothing left for the compositor's scale-to-fit to cancel. This is exactly why `tre-perf-suite`'s own test of the identical mechanism looked clean: `resize_test.rs` was written with a separate `real_size` local specifically to avoid conflating "the swapchain's own target size" with "the true window size" -- `windowed_renderer.rs`'s port reused a single pair of fields for both, inherited from the pre-Option-2 code where that conflation was harmless. **Fixed** by no longer touching `width`/`height` in `resize()` at all -- those two fields are `poll_events`'s own exclusive domain now.

**Confirmed fixed by the project owner directly, live, after this second fix:** "That is perfext" / "perfect". Matching this investigation's own established pattern (finding #234's own resolution), this is the one verification a headless proof or telemetry log cannot substitute for -- a real human watching a real window resize and saying so.

**Further research, at the project owner's own request ("look for ways to make it more efficient and improved visual quality... do real research"), into whether an even-better technique exists.** [vkguide.dev's own resize chapter](https://vkguide.dev/docs/new_chapter_3/resizing_window/) was checked directly and found to address a different concern entirely -- a fixed-size offscreen draw image blitted into the swapchain on present, avoiding *render-target reallocation* on resize -- not the Wayland-compositor-stall problem this finding is about; no applicable technique there. Real-world confirmation that pyCopper's own "oversized buffer, compositor scale-to-fit" strategy is standard practice, not a one-off hack: Chromium's own Ozone/Wayland backend does the identical thing (an oversized buffer held for up to a second after the surface stops changing), and further uses `wp_viewporter`'s explicit *source-crop* (not just destination-scale) when it wants to avoid the resample entirely -- a real, protocol-level way to get zero-blur quality that pyCopper's own simpler "let the implicit whole-buffer stretch happen" approach does not use. A cross-platform survey (at the project owner's own explicit follow-up request) found real, precedented, but structurally *different* techniques on the other two target platforms, each disclosed here as deliberately deferred future work rather than built speculatively: **Windows** has `IDXGISwapChain2::SetSourceSize`, an official DXGI-level crop API built for exactly this -- but it is not reachable from a Vulkan swapchain at all (Vulkan's own Win32 WSI has no equivalent), so it would only ever apply if this workspace ships a real DirectX 12 backend (`tre-rhi-dx12`, currently unimplemented per TECHNICAL.md); **macOS** uses no crop technique at all -- `CAMetalLayer`'s own `presentsWithTransaction` + `waitUntilScheduled` synchronization trick is a materially different mechanism, confirmed via a direct engineering blog post (Tristan Hume, "Glitchless Metal Window Resizing"). No ready-made Rust crate or example combining `winit` + `wp_viewporter` + a Vulkan swapchain was found for the Wayland case either, confirmed via several further targeted searches (including Smithay's own `client-toolkit` examples) -- building it would mean reaching around `winit`'s own Wayland connection via a raw, foreign-display-wrapped `wayland-client`/`wayland-protocols` (Smithay) binding.

**Attempted, at the project owner's explicit direction ("Yes, and so I understand the `wp_viewporter` would be on top of what we built not replacing it") -- and found to be a real, structural incompatibility, not a bug to fix.** A new `crates/tre-rhi-vulkan/src/wayland_viewport.rs` module was built and made to compile cleanly (`Backend::from_foreign_display` wrapping `tre_platform`'s own live `winit`-owned `wl_display`, in "guest mode" so it never closes the shared connection; the target window's own `wl_surface` referenced by reconstructing its `ObjectId` from the raw pointer `raw_window_handle` already exposes, never passed to `manage_object`, so it could never conflict with `winit`'s own management of that same object -- verified against `wayland-backend`'s own precise safety documentation before writing any code). Full workspace `cargo fmt`/`clippy -D warnings`/`build`/`--release`/`cargo test` all clean, pinned to the exact `wayland-backend`/`wayland-client`/`wayland-protocols`/`wayland-sys` versions this workspace's own `winit` dependency chain already resolves (`Cargo.lock`), so no divergent copy of any of the four entered the dependency graph.

**The first real run crashed immediately with a genuine Wayland protocol error, not a soft failure:** `wp_viewporter#63: error 0: the specified surface already has a viewport`. `winit`'s own Wayland backend (or Mesa's Vulkan WSI, or both) already binds its own `wp_viewport` to this exact surface -- almost certainly for its own fractional-scaling/HiDPI support, a real, ordinary use of the same protocol this module tried to use for an unrelated purpose. Wayland's protocol design makes any protocol error fatal to the *entire connection*, by design, with no recovery -- and since this module deliberately shares that one real connection with the actual rendering pipeline (the whole point of "guest mode," reusing `winit`'s own connection rather than a separate one), the error immediately corrupted the connection real rendering depends on: `thread 'main' panicked ... failed to recreate VulkanSwapchain: DeviceLost`, followed by `free(): double free detected in tcache 2` -- genuine heap corruption, not merely a crash. **Reverted completely and immediately** (the new module deleted; `Cargo.toml`/`Cargo.lock`/`lib.rs`/`resize_test.rs` all restored to their last-committed state) -- there is no safe way to query whether a surface already has a viewport before attempting to create one, so this specific approach (a second, independently-created `wp_viewport`) cannot be made safe by retrying, catching the error, or any other code-level mitigation; it is incompatible with how `winit` itself already uses this protocol on this real desktop (KWin/KDE Plasma Wayland), not merely unlucky.

**Status: closed, permanently, for this specific technique.** The already-shipped, already-confirmed-working fix (oversized swapchain + `submit_frame_with_logical_size`'s logical-size projection decoupling) remains the shipping solution on all platforms, unaffected by this reverted attempt. Eliminating the compositor's own resample entirely (this attempt's own goal) would need a different real integration point than a second, independent `wp_viewport` -- for instance, cooperating with whatever object `winit`/Mesa already bound rather than creating a competing one -- which is a materially different, and likely much deeper, undertaking than this attempt, and is not planned.

**Final confirmation, at real scale.** The project owner's own ad hoc Python verification script was updated (at their request) from 3 isolated shapes to a 300-shape grid mixing every primitive kind (rectangles, circles, and triangle/pentagon/hexagon/octagon polygons) filling the whole window -- a real, dense scene, not a toy case. A live session against `windowed_renderer.rs` with this scene: 69.3 seconds, 4,024 frames, 1,127 resize events, no crash, no error. The project owner's own verdict: **"That was smooth."** A follow-up question (whether the SDF shaders' real, `fwidth`-based per-edge antialiasing -- confirmed present in `sdf_rounded_rect.frag`/`sdf_ellipse.frag`/`sdf_rect_styled.frag` -- was responsible for a slight blur noticed while dragging) was answered directly: that per-edge AA is a ~1px soft transition at each shape's own boundary, unrelated to the whole-buffer resample the shipped fix's own tradeoff already discloses. Confirmed by direct observation to be scoped exactly as designed: **"only during the drag, clears up once I stop"** -- the blur is the coarse-buffer resample alone, never present at rest, exactly matching pyCopper's own documented cost and never lingering past the settle.

**Refined further, at the project owner's own request ("I want to see if we can minimize this blurring during resize... I think we can make this even better").** `coarse_target_for` always rounded UP to the next bucket multiple, so the worst-case mismatch between the coarse swapchain and the real window -- and therefore the compositor resample's own visible magnitude -- was up to `bucket - 1` pixels (255 at the shipped 256px bucket). Rounding to the *nearest* multiple instead (a new `round_to_nearest_bucket` helper, replacing the bare `div_ceil` call in both `resize_test.rs` and `windowed_renderer.rs`) halves that worst case to `bucket / 2`, at the identical rebuild frequency -- the same bucket boundaries are crossed at the same rate for a monotonic drag either way, so this is a real, essentially free reduction in blur magnitude, not a new tradeoff. Verified via `tre-perf-suite` first (this investigation's own established order): a real drag session, no crash, resize targets still landing cleanly on real 256px multiples (512x512, 768x512, etc.), and the project owner's own direct comparison against the previous (round-up) build: **"The blur was better."** Ported to `windowed_renderer.rs` and reconfirmed live against the same 300-shape scene.

**Reverted the very next day, at the project owner's own direct catch, before ever calling this "closed" again.** Round-to-nearest genuinely halves worst-case mismatch *magnitude*, but the project owner identified a real perceptual cost it introduces that magnitude alone doesn't capture: rounding to nearest means the coarse buffer is sometimes larger and sometimes smaller than the real window, depending which side of a bucket's own midpoint the live size falls on -- so as a single, continuous, monotonic drag sweeps across one bucket's own span, the compositor's own scale direction actually flips from shrink to stretch partway through. Round-up never does this: the buffer is always `>=` the real size for a bucket's entire span, so the compositor only ever shrinks, one consistent direction throughout. The project owner's own words: **"the up and down check just makes the blur more noticeable because it looks like the shapes are jittering up an down."** A smaller-but-direction-flipping mismatch reads as worse than a larger-but-consistent one. Reverted `round_to_nearest_bucket` back to plain `div_ceil` (always round up) in both `resize_test.rs` and `windowed_renderer.rs`, with the reasoning recorded directly in `coarse_target_for`'s own doc comment so this exact tradeoff isn't rediscovered blind later. Full workspace verification clean (fmt/clippy/build debug+release/test, `tre-engine`'s 178/178 unchanged).

**Status: closed, again, back at the 256px-bucket/round-up/3-frame-settle configuration this finding shipped with before the round-to-nearest detour** -- a real, instructive dead end (correct on magnitude, wrong on perceptual continuity), not wasted: the reasoning is now durably recorded for any future revisit of this exact lever.

**Re-opened once more: the project owner clarified the symptom is a real, visible size oscillation ("its not just a blur, its a real jitter"), not merely a soft resample -- consistent with the sawtooth mechanism round-up itself still has (the mismatch between the fixed coarse buffer and the continuously-changing real window ranges from a fresh bucket's own maximum down to zero and then resets, so the whole scene's apparent size continuously grows then snaps back throughout a drag, never reversing direction but still visibly pulsing). Tested the obvious next lever -- a much smaller bucket (32px, still round-up) to shrink the sawtooth's own amplitude -- against `tre-perf-suite`'s resize test first. **Result: a real, clear negative.** The project owner's own direct comparison: the jitter was **"about the same"** as 256px, while **"the window lag is an issue"** again (p95 79.5ms, max 693.4ms across a real drag session -- back to roughly the same severity finding #235's own original, unfixed problem had). Bucket size does not trade cleanly against jitter the way it does against rebuild frequency: shrinking it clearly costs lag without a corresponding, noticeable jitter improvement. Reverted back to 256px.

**Conclusion: bucket-size tuning is exhausted as a lever for the jitter specifically.** 256px (round-up) remains the best point found on the lag axis; no bucket size tested meaningfully reduces the jitter below what 256px already shows. Eliminating the jitter (not just capping it) requires never letting the swapchain buffer size differ from the real window size at all -- which is exactly Option 3's own property (a real, already-built, zero-distortion alternative, at the cost of dropped frames instead of visual distortion) or the crashed `wp_viewporter` crop (not viable, see above). Offered Option 3 back to the project owner for a fresh look now that the coarse-bucket technique's own real ceiling is better understood; not yet decided.

## Finding #236 Flagged, Not Fixed: No Viewport/Frustum Culling or Virtualization Anywhere in `ShapeRegistry::flatten_into`

Raised by the project owner as a direct question ("Are we doing any virtualization and culling?") outside any specific bug investigation -- checked against the real code (`crates/tre-engine/src/shapes.rs`'s own `ShapeRegistry::flatten_into`, not assumed from memory) rather than answered from recall.

**Confirmed: no.** `flatten_into`'s only two skip conditions are (1) a shape that is neither `layout_dirty` nor has any `active_animations`, and (2) `Visibility::Hidden`/`Collapsed`. Neither depends on where a shape actually sits relative to the current viewport -- a shape whose entire bounding box lies outside the visible window still gets fully tessellated (real CPU-side vertex/index generation, plus a real vertex-shader invocation for every one of its vertices) on every dirty frame. The GPU's own scissor/viewport prevents real fragment-shading cost for anything off-screen, but nothing today prevents the CPU-side work upstream of that. Cross-checked `ARCHITECTURE.md`/`REVIEW.md` for any prior decision either way: the only "culling" documented anywhere is GPU backface (winding-order) culling, deliberately disabled since 2D quads have no meaningful front/back distinction (Section 6.1) -- an unrelated concept. This is a genuine, previously-undecided gap, not a prior deliberate choice being revisited.

**Not fixed -- flagged as a future optimization item, at the project owner's own explicit direction ("Flag it as a future optimization item").** Immaterial at every scale this codebase has actually measured so far: finding #235's own instrumentation already proved a 500-shape scene's full CPU flattening cost is sub-1ms even every single frame, so culling would save time that isn't currently a bottleneck anywhere in this project. It would become real for a much larger scene (thousands of shapes, a meaningful fraction off-screen), which nothing in this codebase exercises today. Recorded in `ARCHITECTURE.md` Section 7 alongside `flatten_into`'s other disclosed gaps (the `lyon`-tessellation-allocation gap, finding #176) for future reference, not built speculatively here.

## Finding #237: The Resize-Stall Mechanism, Pinned Down Precisely via a Real Wayland Protocol Trace -- Compositor-Side DMA-BUF Import, Not GPU Work, Not Client-Side Pacing

A direct continuation of finding #235's own investigation, prompted by the project owner explicitly asking to verify the mechanism empirically rather than continue reasoning about it ("Is there a way to wait for a response saying KWin or the compositor is ready...", preceded by a working discussion ruling out several textbook Vulkan pitfalls one at a time).

**Two general Vulkan pitfalls the project owner raised were checked directly against the real code and ruled out, not assumed:**
- **Fence-wait-before-acquire ordering** ("if you call `vkAcquireNextImageKHR` before waiting on your inflight frame fence, the CPU can run several frames ahead"): `VulkanDevice::begin_frame_impl` already calls `wait_for_fences` before `acquire_next_image_with_timeout`, and always has, since the original Phase 0 code -- this project's single-frame-in-flight design has no "CPU races ahead" failure mode to begin with, since only one frame is ever outstanding.
- **Swapchain image count** (`minImageCount + 1`): already the shipped baseline (`crates/tre-rhi-vulkan/src/swapchain.rs`), and going further was already tried and disproven earlier in finding #235 (Option 1).
- **Decoupling "inflight frame slot" from "acquired image index"**: a real pitfall for multi-frame-in-flight engines, but inapplicable here -- this engine has no `currentFrame` ring counter at all (one persistent command buffer, one reused `image_available_semaphore`), and the one genuinely per-image resource (`render_finished_semaphores`) is already indexed by the real acquired image index, not an artificial frame counter.

**Pinned down with a real `WAYLAND_DEBUG=1` protocol trace, correlated directly against this crate's own acquire-timing instrumentation (`resize_test.rs`'s `STALL` records), not inferred from external precedent alone.** Every large stall found in the trace is immediately preceded by exactly 4 brand-new `zwp_linux_buffer_params_v1.create_immed` calls (one per swapchain image, each creating a fresh DMA-BUF-backed `wl_buffer` at the new size), matching the moment `resize_test.rs`'s own `resize --- window resized to WxH, swapchain rebuild took N.NNms` line fires. Immediately after those 4 creates, the trace shows a real, measured gap of 300-450ms with **nothing at all** on the Wayland wire -- confirmed twice, with the gap's own duration matching the corresponding `STALL` line's `acquire_ms` almost exactly (453.8ms wire gap vs. 453.3ms reported acquire; 335.2ms vs. 334.8ms). The gap resolves with the compositor delivering a burst of backlogged `wl_display.delete_id` calls (its own queued object-cleanup bookkeeping) immediately followed by a flood of previously-queued `xdg_toplevel.configure`/`xdg_surface.configure` events landing all at once -- the literal mechanism behind this whole investigation's original "trails then jumps" description.

**Conclusion, confirmed directly by the project owner ("yes, that's exactly the mechanism"):** recreating the swapchain doesn't just cost a client-side rebuild (measured at under 1ms) -- it hands the compositor a batch of brand-new DMA-BUF memory objects that must be imported (format/modifier validation, driver-level memory import, registering with KWin's own renderer) before any of them can be presented, and that import is not instant. `zwp_linux_buffer_params_v1.create_immed` is deliberately fire-and-forget by protocol design (unlike its sibling `create()`, which has real `created`/`failed` events) -- there is no cheaper "are you ready yet" signal to poll instead of `vkAcquireNextImageKHR` itself; the acquire call *is* the wait for compositor readiness, and it is slow specifically because KWin's own event/render loop falls behind during a fast interactive drag, not because of anything on the GPU or client side. This also newly explains, with a precise causal mechanism rather than just an empirical trend, why coarse bucketing helps (fewer resizes -> fewer batches of brand-new DMA-BUFs to import) and why more swapchain images (Option 1) didn't (more images per resize -> more, not fewer, new buffers to import per event).

**Candidate fix tried: `VkSwapchainCreateInfoKHR::oldSwapchain`, at the project owner's own explicit direction ("Yes, implement and test the oldSwapchain change") after a "double-swapchain" idea was discussed and narrowed to this concrete, standard Vulkan mechanism** (a literal two-live-swapchains-on-one-surface design was ruled out first -- it would hit the identical `wp_fifo_manager_v1` one-surface-per-`wl_surface` conflict that caused finding #230's own crash). `VulkanSwapchain::recreate` (`crates/tre-rhi-vulkan/src/swapchain.rs`) now builds the new swapchain first, passing the current (about-to-be-retired) one as `oldSwapchain`, and only destroys the old dependent resources after the new swapchain successfully builds -- reversed from this method's own prior design, which had deliberately omitted `oldSwapchain` reasoning (correctly, for a different question) that this project's single-frame-in-flight loop has no in-flight work needing its usual smoother-handoff purpose.

**Result: a real, honest negative -- no measurable improvement, and the underlying mechanism is unchanged.** Re-ran the identical `WAYLAND_DEBUG=1` trace: `create_immed` still fires exactly ~4 times per resize (908 creates / 226 resizes), identical to before -- Mesa's own WSI does not appear to reuse or cheapen the DMA-BUF import based on the `oldSwapchain` hint when the buffer's own pixel dimensions actually change (as they always do in this coarse-bucket-recreate scenario). Isolating the real drag activity within an otherwise 8-hour idle test session (the actual test window was left open far longer than intended, which would have otherwise silently invalidated the comparison): p95 126.2ms, max 532.2ms across 561 samples -- comparable to, not clearly better than, the pre-`oldSwapchain` baseline. This makes sense in hindsight: `oldSwapchain`'s real, spec-guaranteed benefit is avoiding a hard synchronization point for in-flight presents during the transition, not a promise that the driver can recycle differently-sized memory -- and every resize here is by definition a size change, so there is nothing of the old buffers' own memory to reuse regardless of the hint.

**Kept anyway, at the project owner's own explicit direction ("keep it").** Full workspace verification clean (fmt/clippy/build debug+release/test, `tre-engine`'s 178/178 unchanged) -- `oldSwapchain` is the standard, spec-correct idiom for swapchain recreation regardless of this specific performance question, and the reordering it required (build new, then destroy old, rather than destroy-then-build) is a strict safety improvement on its own terms: a failed recreate now leaves the prior swapchain's resources intact and usable, rather than already-destroyed, matching this method's own updated doc comment.

**Status: the precise mechanism is now confirmed via direct protocol-level evidence, not inference. `oldSwapchain` is kept for its own correctness merits but is a confirmed, disclosed non-fix for the underlying performance question. The compositor-side import cost itself remains outside this application's own direct control** -- every remaining lever (coarse bucketing, bounded-timeout skip) works around it rather than eliminating it, and this finding records that honestly rather than continuing to search for a client-side fix to what is, at its root, a compositor-side bottleneck.

## Finding #238 Fixed: `wp_viewporter` Resize Crop -- the Compositor-Resample Blur/Jitter from Finding #235 Is Eliminated, via a Patched `winit` and a Corrected Render Path

A direct continuation of finding #235's own "further pursuit" of the residual compositor-resample blur, at the project owner's own explicit direction ("Plan out the wp_viewporter implementation completely and from a cold start", followed by choosing "Patch/fork winit" over stopping at the already-shipped ceiling).

**The structural blocker from the first, crashed attempt (finding #235's own writeup above) was fixed at its real root, not retried: `winit` itself was patched, not worked around.** `winit`'s Wayland backend (`platform_impl/linux/wayland/window/state.rs`) creates exactly one `wp_viewport` per window, unconditionally, at construction, with no public API to reach it -- confirmed directly from `winit`'s own real source, not assumed. Forked `rust-windowing/winit` at the exact `v0.30.13` tag this workspace already resolved (commit `e9809ef54b18499bb4f2cac945719ecc2a61061b`), and added one small, purely additive patch (3 files, 46 lines) exposing `WindowExtWayland::set_viewport_source_crop(width, height)`, which calls `.set_source(...)` on the viewport `WindowState` already owns and already calls `.set_destination(...)` on every resize -- never a second, independently-created viewport, so the crash class from the first attempt (`wp_viewporter#63: error 0: the specified surface already has a viewport`) cannot recur. Pushed to a fork (`https://github.com/mindderivative/winit`, branch `tre-viewport-crop`, commit `1c2028620ceebb2aa5d132e6a69b57bfbb6d61af`) and pinned into this workspace via an exact-`rev` `[patch.crates-io]` entry in the root `Cargo.toml` (matching this workspace's own established exact-pin precedent elsewhere -- `rustc-hash`, `skrifa`). `tre_platform::PlatformConnection` gained a thin new method, `set_viewport_source_crop(window, width, height)`, delegating straight through to the patched `winit` call -- no new `wayland-client`/`wayland-backend`/`wayland-protocols` dependency at all this time, unlike the crashed first attempt.

**First live-drag test with this alone: a real, honest negative -- "it jittered more and stalled".** The crop was wired into `resize_test.rs` alongside the *existing*, unchanged `begin_frame_with_logical_size` (Option 2's stretch-then-let-the-compositor-resample-back-down mechanism). Diagnosed precisely by reading `VulkanDevice::begin_frame_impl`'s own real implementation, not by guessing: that method renders content proportioned for `logical_size`, but through a GPU viewport/scissor/render area that still spans the swapchain's own FULL (possibly oversized) extent -- so content is deliberately pre-stretched across the whole buffer, relying on the compositor's own `set_destination`-driven scale-to-fit to cancel that stretch back down to the correct on-screen size. Adding a `wp_viewport` source crop of `(0,0,real_size)` on top, with destination *also* `real_size`, makes source == destination -- zero compositor scaling -- which just presents the raw top-left `real_size` corner of the *stretched* buffer: a zoomed-in fraction of the scene, not the correctly-scaled whole scene, and one whose zoom factor changes continuously through the drag. The two mechanisms are mutually exclusive, not stackable, and this is exactly why it looked worse, not better.

**Real fix: a genuinely different render path for the crop case, replacing the stretch (not layering on top of it).** Added `RhiDevice::begin_frame_with_viewport_crop(swapchain, crop_size)` (`tre-engine/src/rhi.rs`, default-delegating to plain `begin_frame` for any backend that hasn't implemented it, exactly matching `begin_frame_with_logical_size`'s own existing pattern) plus a matching `submit_frame_with_viewport_crop` free function. `VulkanDevice::begin_frame_impl` (`tre-rhi-vulkan/src/device.rs`) now takes a `viewport_crop: Option<(u32, u32)>` alongside the existing `logical_size`; when present, it confines the GPU viewport, scissor, AND render area to that size (clamped defensively to the swapchain's own real extent) instead of the swapchain's full extent -- content renders 1:1, undistorted, into just that top-left sub-rectangle of a possibly-larger buffer, with the rest of the buffer left untouched and never presented. `resize_test.rs`'s `ResizeStrategy::Coarse` path now calls `begin_frame_with_viewport_crop(real_size)` (replacing `begin_frame_with_logical_size` entirely for this strategy, at both the primary and `SwapchainOutOfDate`-recovery call sites) paired with `connection.set_viewport_source_crop(window, real_size)` immediately before `submit_and_present` -- and `full_window`'s own root `ScissorRect` was corrected to `real_size` instead of `surface.width`/`surface.height`, since a scissor rect wider than the now-smaller render area is invalid for dynamic rendering.

**Full workspace verification clean with the patched `winit` in the dependency graph**: `cargo fmt --all -- --check`, `cargo clippy --workspace --all-targets -- -D warnings`, `cargo build --workspace --all-targets` (debug and `--release`), and `cargo test --workspace --no-fail-fast` all passed with zero warnings and zero failures, both before and after the render-path correction.

**Confirmed via two real, live-hardware drag tests through `tre-perf-suite`'s resize test, matching this whole investigation's own established methodology of a real interactive drag over inference or synthetic benchmarks alone.** The first test (crop stacked on the unchanged stretch path) ran a real sustained drag (149 resize events) and exited cleanly with no protocol crash -- confirming the structural winit-patch fix itself works -- but the project owner's own direct verdict was the honest negative recorded above. The second test, after the render-path correction, ran another real sustained drag (132 resize events) and again exited cleanly with no crash; the project owner's own direct verdict this time: **"it looked correct, no jitter this time."**

**Ported to `crates/tre-python/src/windowed_renderer.rs`** (the plan's own next step once `resize_test.rs` verified live) -- `submit_frame_to_window` now calls `submit_frame_with_viewport_crop` instead of `submit_frame_with_logical_size`, paired with the identical `PlatformConnection::set_viewport_source_crop` call, and `full_window`'s own root `ScissorRect` uses the window's logical size instead of the swapchain's own extent, for the identical reason as `resize_test.rs`'s own correction. Rebuilt via `maturin develop --release` and confirmed against the real 300-shape scene (`live_resize_check.py`, 25x12 grid mixing Rectangle/Circle/Polygon): the project owner's own direct verdict, after a real live drag, **"it looked correct, no jitter"** -- matching the perf-suite result exactly.

**Status: fixed and shipped in both `resize_test.rs` (`ResizeStrategy::Coarse`) and `windowed_renderer.rs` (`tre-python`'s real on-screen render path, the one pySilver's own GUI apps actually use).** This eliminates the compositor-resample blur/jitter finding #235 first identified and every subsequent bucket-tuning pass (round-to-nearest, 32px) could only ever cap, not remove -- zero resample now happens during an active drag, not merely a smaller one. `DRAG_COARSE_STEP`/`SETTLE_FRAMES` (governing how *often* the swapchain itself rebuilds, finding #237's own separate DMA-BUF-import-stall concern) are untouched by this change and remain exactly as shipped. Windows (`IDXGISwapChain2::SetSourceSize`) and macOS (`CAMetalLayer`'s `presentsWithTransaction`/`waitUntilScheduled`) remain deliberately deferred, real-but-different techniques, exactly as already disclosed above -- this fix is Linux/Wayland-only, gated implicitly by `winit`'s own existing per-platform `WindowExtWayland` availability (a no-op, not an error, on X11 or if the compositor never advertised `wp_viewporter` at all).

## Finding #239 Designed, Not Implemented: How `IDXGISwapChain2::SetSourceSize` Would Integrate with `begin_frame_with_viewport_crop` on a Future DX12 Backend

At the project owner's own explicit direction ("start on the Windows SetSourceSize deferred work"), scoped down (via `AskUserQuestion`, after `tre-rhi-dx12` was confirmed to be a genuinely empty stub -- `src/lib.rs` is 10 lines, no device/swapchain/pipeline code at all) to a design-only pass: how `SetSourceSize` would slot into the now-shipped `RhiDevice::begin_frame_with_viewport_crop` cross-platform contract, not a real DX12 backend build-out (a separate, multi-phase project on the scale of the existing Vulkan backend, out of scope here). **Nothing below has been built or tested -- there is no Windows development environment available in this session, and no line of `tre-rhi-dx12` code changed.** Every mechanism claim below is sourced directly from Microsoft's own current documentation (linked inline), not assumed, matching this whole investigation's own established "real research, not inference" practice.

**The real mechanism, confirmed via Microsoft Learn's own [`DXGI_SCALING`](https://learn.microsoft.com/en-us/windows/win32/api/dxgi1_2/ne-dxgi1_2-dxgi_scaling) and [`IDXGISwapChain2::SetSourceSize`](https://learn.microsoft.com/en-us/windows/win32/api/dxgi1_3/nf-dxgi1_3-idxgiswapchain2-setsourcesize) reference pages directly, not a summary or a blog post:** `DXGI_SCALING_NONE`, set on `DXGI_SWAP_CHAIN_DESC1::Scaling` at swap chain creation (supported by `IDXGIFactory2::CreateSwapChainForHwnd` -- the ordinary desktop Win32 path a `tre-rhi-dx12` backend would use -- and requires the flip-model swap effects, `DXGI_SWAP_EFFECT_FLIP_DISCARD`/`FLIP_SEQUENTIAL`, which DX12 swap chains are required to use regardless, so this is not an extra cost), makes "the back-buffer contents appear without any scaling when the presentation target size is not equal to the back-buffer size... [with] the top edges of the back buffer and presentation target... aligned together." Combined with `SetSourceSize(width, height)` -- which redefines the back buffer's own *active* sub-rectangle to `[0, 0, width, height]` "without calling the more-expensive `IDXGISwapChain::ResizeBuffers`" -- this is DXGI's own real, documented crop primitive: allocate the swap chain once at an oversized (bucket) size with `Scaling = DXGI_SCALING_NONE`, then call `SetSourceSize(real_width, real_height)` on every resize instead of `ResizeBuffers`. When `real_width`/`real_height` exactly match the HWND's own client-area size (as they would here, mirroring the Wayland design exactly), the result is a 1:1, undistorted presentation of just that sub-rectangle with zero letterboxing and zero resample -- the direct DX12 analog of `wp_viewport`'s `set_source`/`set_destination` pair set to identical sizes.

**A real architectural difference from the Wayland design, and a simplification, not a complication: no `winit` patch equivalent is needed on Windows at all.** On Wayland, the crop lives on a `wp_viewport` object `winit` itself owns (external to `tre-rhi-vulkan`), which is why reaching it required patching `winit` and adding a new cross-cutting `tre_platform::PlatformConnection::set_viewport_source_crop` method. On Windows, `SetSourceSize` is a method on the `IDXGISwapChain2` itself -- an object a `tre-rhi-dx12` backend would already fully own and create directly (via `raw-window-handle`'s HWND, exactly matching how `tre-rhi-vulkan::VulkanSwapchain` already owns its `VkSurfaceKHR`/`VkSwapchainKHR` directly rather than reaching through `tre_platform`). This means: no new `tre_platform`/`winit` surface at all would be needed for Windows -- the DX12 implementation of `RhiDevice::begin_frame_with_viewport_crop` would call `SetSourceSize(crop_size)` on its own swap chain internally, in the same place it sets the D3D12 viewport/scissor (`ID3D12GraphicsCommandList::RSSetViewports`/`RSSetScissorRects`) to `crop_size` -- mirroring `VulkanDevice::begin_frame_impl`'s own already-shipped `render_width`/`render_height` confinement line for line, just in D3D12 terms. No change would be needed to `resize_test.rs`/`windowed_renderer.rs`'s own call sites for Windows at all: the already-shared `begin_frame_with_viewport_crop(swapchain, crop_size)` call is enough, and the Linux-only `PlatformConnection::set_viewport_source_crop` call already degrades to a no-op off Wayland, so it needs no `#[cfg]` gating to coexist with a future DX12 path either.

**What a real implementation would still need, once `tre-rhi-dx12` exists at all (recorded here so this design survives until then, not to scope-creep into building it now):** a `Dxgi1_3`-or-later feature level to obtain `IDXGISwapChain2` (`IDXGISwapChain1::QueryInterface`, or `windows` crate's own equivalent cast); `Scaling: DXGI_SCALING_NONE` set once at `CreateSwapChainForHwnd` time (an immutable swap-chain-creation-time choice -- unlike Vulkan's `oldSwapchain`, DXGI has no way to change `Scaling` without a full recreate, so this needs to be right from the very first swap chain built, not toggled per-frame); and confirmation (once real hardware is available) that `SetSourceSize`'s own documented `E_INVALIDARG` failure mode (source size larger than the back buffer) can never trigger here, mirroring `begin_frame_impl`'s own defensive `.min(width)`/`.min(height)` clamp already shipped for the Vulkan/Wayland path.

Sources:
- [IDXGISwapChain2::SetSourceSize (dxgi1_3.h) - Win32 apps | Microsoft Learn](https://learn.microsoft.com/en-us/windows/win32/api/dxgi1_3/nf-dxgi1_3-idxgiswapchain2-setsourcesize)
- [DXGI_SCALING (dxgi1_2.h) - Win32 apps | Microsoft Learn](https://learn.microsoft.com/en-us/windows/win32/api/dxgi1_2/ne-dxgi1_2-dxgi_scaling)
- [Swap chain scaling and overlays - UWP applications | Microsoft Learn](https://learn.microsoft.com/en-us/windows/uwp/gaming/multisampling--scaling--and-overlay-swap-chains)

**Status: designed, not built.** No code changed. Remains deferred until `tre-rhi-dx12` is a real backend -- this finding exists so that work, whenever it starts, does not need to re-derive the mechanism from scratch.

## Finding #240 Fixed: Finding #189's Two RHI Trait Methods No Longer Swallow a Recoverable `Err` Into a Panic

At the project owner's own explicit direction ("start there", picking this out of the current open-findings punch list as the one real correctness gap worth prioritizing over the remaining nice-to-haves/deferred-optimization items).

**`RhiDevice::create_dynamic_ring_buffer`** (`tre-engine/src/rhi.rs`) now returns `Result<Box<dyn RhiDynamicRingBuffer>, EngineError>` instead of an infallible `Box<dyn RhiDynamicRingBuffer>`. `VulkanDevice`'s own real implementation (`tre-rhi-vulkan/src/device.rs`) propagates `VulkanRingBuffer::new`'s own `Result` via `?` instead of `.expect("failed to create dynamic ring buffer")` -- every failure path inside `VulkanRingBuffer::new` already mapped to `EngineError::DeviceLost` (buffer/memory allocation, memory-type selection, mapping), so this is a genuine, real recoverable condition under VRAM pressure, not a programmer-error invariant. All 8 real call sites across the workspace were updated to match their own surrounding function's existing error-handling convention rather than a single mechanical pattern: `tre-ffi/src/renderer.rs` and both `tre-python` renderers (`renderer.rs`/`windowed_renderer.rs`) already propagate every other setup failure via `.map_err(...)?` in the same constructor, so the ring buffer now does too -- closing a real, previously-silent gap where this one call alone could still panic a fully `Result`-based constructor. The two `tre-perf-suite` binaries and three `tre-rhi-vulkan` examples keep `.expect(...)`, matching their own established demo/binary convention (no `Result`-returning `main`, and every sibling setup call in the same file already panics on failure the same way).

**`RhiCommandBuffer::apply_layer_blur`** (`tre-engine/src/rhi.rs`) now returns `Result<Box<dyn RhiTexture>, EngineError>`. `VulkanCommandBuffer`'s own real implementation (`tre-rhi-vulkan/src/command_buffer.rs`) propagates each of its own 4 internal `acquire_transient_target` hops (the dual-Kawase blur's L1/L2/U1/U0 chain) via `?`/an explicit `match` instead of `.expect(...)` -- and, since a later hop can fail after an earlier one already succeeded, each failure path now releases whatever was already acquired-but-not-yet-returned-to-the-pool before propagating `Err`, so this fix does not introduce a transient-target leak on the newly-reachable error path (verified by inspection: L2's failure releases L1, U1's failure releases L2, U0's failure releases U1 -- exactly mirroring the release ordering the success path already used).

**One real, disclosed nuance, not silently smoothed over: `apply_layer_blur`'s own single production call site, inside `execute_frame`'s `PopLayer` handling, still panics.** `execute_frame` itself has no `Result` return type to propagate a mid-frame failure through -- an already-disclosed, deliberately out-of-scope gap this same document's own finding #141 records (widening `execute_frame` itself is real, separate future work, not attempted opportunistically here). The `.expect(...)` at that one call site is therefore kept, but as an explicit, visible decision at the call site itself now (with an inline comment cross-referencing both findings), not a panic hidden inside the trait implementation two layers down -- and any *other*, direct caller of `RhiCommandBuffer::apply_layer_blur` (not routed through `execute_frame`) now gets the real `Result` and can choose to handle it, which is the actual, complete fix this finding's own trait-signature widening enables.

Both trait methods' 2 test-double (`FakeDevice`) implementations (`tre-engine/src/lib.rs`, `tre-engine/src/shapes.rs`) were updated to match the widened signatures -- both bodies (`unimplemented!`, and a literal `FakeTexture`) needed only a signature change/`Ok(..)` wrap, no behavioral change, since neither is exercised by a test that hits the failure path.

**Full workspace verification clean**: `cargo fmt --all -- --check`, `cargo clippy --workspace --all-targets -- -D warnings`, `cargo build --workspace --all-targets` (debug and `--release`), and `cargo test --workspace --no-fail-fast` all passed with zero warnings and zero failures -- `tre-engine`'s own 178/178 unit tests unchanged, confirming no behavioral regression from the signature widening itself.

**Status: fixed.** Both real, recoverable `Err`s finding #189 flagged are now reachable as `Result`s to any caller able to act on them, closing the last remaining item from the original documentation-audit pass (`documentation/REVIEW.md`'s own index note: "#188-189 are documented, deliberately-not-fixed gaps... whose real fix needs a trait-signature change out of scope for an audit pass"). Finding #141's own separate, larger `execute_frame`-fallibility gap remains open and undiminished by this fix -- it was never this finding's own scope.

## Finding #241 Fixed: `execute_frame` No Longer Panics on the Recoverable Failures Finding #141 Named

At the project owner's own explicit direction ("yes, start there"), after this same document's own updated punch list named finding #141 as the highest-priority open item -- the natural, larger continuation of #240 above, which had just run directly into this exact, already-disclosed boundary and could not close it without widening `execute_frame` itself.

**`execute_frame`** (`tre-engine/src/rhi.rs`) now returns `Result<(), EngineError>` instead of `()`, exactly as finding #141's own writeup specified. Its three internal `.expect()`-guarded calls (`acquire_transient_target` for `PushLayer`, `apply_layer_blur` and `register_bindless` for `PopLayer`) now propagate their real `Err` instead of panicking. The two `PopLayer`-side failures needed the same "release what's already held before returning `Err`" discipline finding #189/#240 established inside `apply_layer_blur` itself, applied one level up: if `apply_layer_blur` fails, the original layer's own still-held texture is released back to the pool before returning; if `register_bindless` fails, whichever texture is actually being composited (the just-blurred one, or the original if no blur was requested) is released the same way. `PushLayer`'s own `acquire_transient_target` failure needs no such cleanup -- nothing has been acquired yet at that point in the loop. The three remaining panics inside this function (an unregistered `pipeline_state_id`, a nested `PushLayer`, a `PopLayer` with no active `PushLayer`) are untouched and deliberately still panic -- all three are static caller-setup bugs, not resource-exhaustion conditions an `Err` return would let anyone recover from, exactly as this function's own (now-updated) doc comment explains.

**Every real call site across the workspace was updated** -- 34 in total (confirmed via a direct grep before starting, matching the scope disclosed to the project owner before they said "start there"): 10 inside `tre-engine/src/lib.rs`'s own test module (9 identical calls fixed in one `replace_all` pass, one with distinct buffer offsets fixed separately -- all now `.unwrap()`, appropriate for test assertions), and 24 across `tre-ffi`, both `tre-python` renderers, both `tre-perf-suite` binaries, and 19 `tre-rhi-vulkan` examples. Every one of these 24 calls `execute_frame` from inside a `submit_frame`/`submit_frame_with_viewport_crop` closure whose own trait bound is `FnOnce(&mut dyn RhiCommandBuffer)` (implicitly `-> ()`) -- or, for `resize_test.rs`'s two call sites and `main_loop_demo.rs`'s one, directly inside a plain `fn main()`'s own render loop -- so none of them could propagate the new `Result` any further up without *also* widening `submit_frame`'s own closure signature, a materially larger, cascading change finding #141 never asked for and this fix deliberately did not attempt. Every one of the 24 gets `.expect("execute_frame failed")` instead, matching this codebase's own established convention for exactly this situation (compare `submit_frame(...).expect("submit_frame failed")`, already used at every one of these same call sites) -- a real `Err` still surfaces as a panic at these sites today, identically to before this fix, but now as an explicit, visible call-site decision rather than one hidden two layers down inside `execute_frame` itself; a future caller with a genuine need to recover gracefully (e.g. a real windowed renderer wanting to skip a frame under transient pool pressure rather than crash) now has a real `Result` to build that on, without needing another trait-signature change first.

`execute_frame`'s own doc comment was restructured to match: the previous, single `# Panics` section (which incorrectly cited a nonexistent IMPLEMENTATION.md disclosure -- finding #141's own original discovery) is now split into a real `# Errors` section (naming exactly which three calls can fail and why, plus the cleanup-on-failure guarantee) and a narrowed `# Panics` section (the three genuine caller-misuse conditions only). Needed a new `#[allow(clippy::too_many_lines)]` (with its own `reason`, matching this function's existing `too_many_arguments` allow's style) -- the two new cleanup-on-failure `match` blocks pushed it a handful of lines over clippy's default threshold.

**Full workspace verification clean**: `cargo fmt --all -- --check`, `cargo clippy --workspace --all-targets -- -D warnings`, `cargo build --workspace --all-targets` (debug and `--release`), and `cargo test --workspace --no-fail-fast` all passed with zero warnings and zero failures -- `tre-engine`'s own 178/178 unit tests unchanged. Beyond the automated suite, `layer_blur_demo` and `canvas_layer_composite_demo` (the two real GPU examples directly exercising `PushLayer`/`apply_layer_blur`/`PopLayer` -- the exact code path this fix touched most) were both re-run for real against actual hardware and passed every one of their own pixel-level assertions, confirming the refactor changed no successful-path behavior, only the failure path's shape.

**Status: fixed.** `execute_frame` no longer panics on any of the three failures `EngineError`'s own doc comments already call recoverable, closing finding #141 for real -- the gap finding #240 had to explicitly work around (rather than close) moments earlier in this same document.

## Finding #242: Findings #54, #188, #221, #236, and #239 Worked Through in Order -- Two Real Fixes, One Rediscovered-Already-Fixed, Two Confirmed Already at Their Correct Disposition

At the project owner's own explicit direction ("fix them in order"), working through this document's own current open-findings punch list from top to bottom.

**Finding #54 (two small robustness gaps): both fixed.** `crates/tre-rhi-vulkan/src/swapchain.rs`'s `build_swapchain` no longer falls back to `formats[0]` (a panic on an empty format list) -- it now falls back to `formats.first()` and propagates `EngineError::DeviceLost` if even that is `None`, matching every other surface/device query failure on this same call path. `crates/tre-engine/src/input.rs`'s `InputEventQueue::push` doc comment was corrected to honestly disclose that its drop-on-full behavior applies uniformly to every `InputEvent` variant, not just the `PointerMoved` coalescing the rest of that comment describes -- a documentation-only fix, not a runtime change: making a `CloseRequested` un-droppable under real queue exhaustion would need a materially different design (an unbounded queue, or a side-channel reserved for control events) for a risk this same entry's own text already judged "effectively unreachable" under real human/OS input at one drain per frame.

**Finding #188 (missing field/variant doc comments): fixed for the five types the finding itself named as examples.** Added field-level doc comments to `ScissorRect` (`tre-engine`), `UiDrawCommand` and its `CommandType` enum (`tre-engine`), `AccessibilityNode` and its `AccessibilityRole` enum plus the adjacent `FlattenedFrame` (`tre-engine`), `PackedRect` (`tre-atlas`), and `ShapedGlyph` plus its sibling `ShapedRun` (`tre-text`). Deliberately bounded to these named types (and their immediately-adjacent companion types already open in the same edit) rather than attempting the "dozens more" across the whole workspace this same finding's own text already judged out of proportion to add wholesale -- the disproportion judgment stands unchanged, this fix just closes the specific, named instances of it.

**Finding #221 (Text re-shape cost): already fixed, just never cross-referenced.** Investigating this entry's own proposed fix (a shaped-run cache keyed by `(text, font, px_size, wrap_width)`) found that exact design already built and shipped, under a different, later finding number: #225 (see that entry's own full write-up, including its own three dedicated unit tests and four re-run real GPU demos). The original `### 221.` entry was simply never updated to point to it -- the identical stale-cross-reference gap finding #145 already named as a recurring problem in this document's own preamble. Fixed here by adding that cross-reference; no code changed, since #225 already did the real work.

**Findings #236 and #239: confirmed already at their correct, requested disposition -- no further action taken.** #236 (no viewport/frustum culling) was explicitly resolved by the project owner's own direction to flag it as a future optimization item, not build it -- real frustum culling is substantial, speculative new feature work (bounding-box tracking, viewport-intersection tests, transform/rotation handling), not a mechanical fix, and building it now would contradict what was actually asked for. #239 (DX12 `SetSourceSize` integration) is a design note already completed to its full possible scope, blocked on `tre-rhi-dx12` existing as a real backend at all. Both were re-confirmed against their own current text rather than assumed unchanged, but neither warranted new code or documentation.

**Full workspace verification clean** for the two real fixes (#54, #188): `cargo fmt --all -- --check`, `cargo clippy --workspace --all-targets -- -D warnings`, `cargo build --workspace --all-targets` (debug and `--release`), and `cargo test --workspace --no-fail-fast` all passed with zero warnings and zero failures -- `tre-engine`'s own 178/178 unit tests unchanged.

**Status: this punch list is now fully worked through.** The document's own remaining genuinely open items are the long tail of early-phase `[Nice-to-have]`/`[Should-fix]` entries already resolved by later, cross-referenced "Fixed" sections throughout this document's own history, plus whatever the project owner raises next.

## Finding #243: Fresh Full `/review-project` Pass -- One Real Performance Fix, One Real Documentation Fix, Three Items Flagged for the Project Owner

At the project owner's own explicit instruction ("run the fresh full audit"), ran this document's own established four-lens (Performance/Architecture/Security/Modernizer) multi-agent review across the entire workspace, each lens instructed to cross-check every candidate against this document's own ~242 existing findings before reporting anything, to avoid re-flagging already-known/disclosed issues.

**Security: clean.** No new exploitable issues found. The `winit` fork's `[patch.crates-io]` pin (finding #238) was independently re-verified as a genuine immutable commit hash, not a moving branch/tag; `tre-ffi`'s C-ABI, `tre-python`'s PyO3 boundary, and `tre-svg`'s SMIL parser were all re-checked against their own already-disclosed hardening (findings #214, #218) and found to match. One area flagged for a future pass, not itself a finding: `tre-python/src/custom_shader.rs`/`sdf_shadow.rs` compile caller-supplied GLSL to SPIR-V via `shaderc` at runtime -- not examined this pass.

**Performance: one real, new finding, fixed.** `crates/tre-engine/src/text.rs`'s `flatten_text` cached shaped runs (finding #225) but not the word-wrap pass immediately after them -- `tre_text::wrap_lines` (a full UAX #14 line-break pass) plus two further heap allocations (`flat_glyphs`, and one `.to_vec()` per output line) ran unconditionally on every call, even on a shaping-cache *hit*, reintroducing exactly the waste class #225 fixed for shaping, one step later, for any wrapped (`wrap_width: Some(_)`) `Text` shape with an active cosmetic-only animation. Fixed by extending `TextShapeCache` with a new `wrapped: Option<WrappedLinesCache>` field (the wrapped lines plus their flat glyph list), populated once and reused across calls the identical way `runs` itself already is; a new `resolve_wrapped_lines` helper mirrors `resolve_shaped_runs`'s own shape and caching discipline exactly, and is independently unit-testable the same way (two new tests: `resolve_wrapped_lines_reuses_the_cached_lines_when_nothing_wrap_relevant_changed` asserts the *same* `Vec` pointer across two unchanged calls; `resolve_wrapped_lines_rewraps_when_the_text_content_changes` asserts a genuinely longer string produces more wrapped lines, not a stale reuse). `tre-engine`'s own suite grew from 178 to 180.

**Architecture: three findings.** (1) `ARCHITECTURE.md` Section 6 had not been annotated for any of the four RHI trait additions findings #235/#238 made (`begin_frame_with_timeout`/`begin_frame_with_logical_size`/`begin_frame_with_viewport_crop`, `acquire_next_image_with_timeout`) -- a real break in what had, up to that point, been a perfectly-maintained "every trait-surface change gets an annotation" convention. Fixed: annotation added. (2) The bounded-acquire-timeout mechanism (`begin_frame_with_timeout`, `acquire_next_image_with_timeout`, `EngineError::AcquireTimedOut`) is fully designed and implemented in the one real backend, but has zero production callers now that viewport-crop shipped instead of it -- real, tested Vulkan code with no caller except the A/B-test harness that compared it against the strategy that actually won. (3) `crates/tre-python/src/windowed_renderer.rs`'s `PyWindowedRenderer::new` duplicates `WindowSlot::create`'s own swapchain-construction/pipeline-registration/struct-assembly sequence inline, verbatim past the one genuine, unavoidable difference (the first window's `(surface_loader, surface)` come bundled out of `VulkanDevice::new` itself, a real Vulkan-bootstrapping constraint, not an artifact of the duplication) -- the exact class of bug this project has already been bitten by twice (findings #196, #213).

Findings (2) and (3) were investigated, not fixed, and are flagged below for the project owner rather than fixed autonomously: (3)'s own natural fix (factor the shared construction logic into one method both call) turns out to need a parameter type (`ash::khr::surface::Instance`/`vk::SurfaceKHR`) that `tre-rhi-vulkan` does not currently re-export and `tre-python` deliberately has no direct dependency on any more (finding #203/#204 removed it) -- sharing the code as-is would either reintroduce that removed dependency or add new public API surface to `tre-rhi-vulkan`, a real design choice, not a mechanical extraction. (2) has no single obviously-correct disposition (remove the unused mechanism; keep it as tested-but-dormant infrastructure; or unify the trait surface so timeout and crop compose, matching how `VulkanDevice::begin_frame_impl`'s own internal signature already treats them) -- a real product/API decision, not a bug.

**Modernizer: two dependency-currency gaps, flagged, not bumped.** Every other version-sensitive dependency in this workspace carries an inline comment recording that it's pinned to the newest version this workspace's `rust-version = 1.75` MSRV can resolve (confirmed via `cargo add --dry-run` at the time). Two dependencies break that pattern with no such rationale: `tre-python`'s `pyo3 = "0.27.2"` (two minor versions behind 0.29.2) and `glam = "0.29"` (`tre-tween`/`tre-python`, three minor versions behind 0.32.1, a low-MSRV math crate where the gap looks like a plain missed bump rather than an MSRV constraint). Not bumped autonomously -- a dependency version change is exactly the kind of thing this project's own standing practice treats as needing explicit confirmation and a real compatibility check, not something to change opportunistically inside a review pass.

**Full workspace verification clean** for the one real code fix: `cargo fmt --all -- --check`, `cargo clippy --workspace --all-targets -- -D warnings`, `cargo build --workspace --all-targets` (debug and `--release`), and `cargo test --workspace --no-fail-fast` all passed with zero warnings and zero failures -- `tre-engine`'s own suite grew from 178 to 180 (the two new wrap-cache tests), no other test count changed.

**Status: one real fix shipped (text wrap-caching), one documentation gap closed (ARCHITECTURE.md annotation), three items awaiting the project owner's own direction** (the `WindowSlot` construction duplication's real fix path, the bounded-acquire-timeout mechanism's disposition, and whether/how to bump `pyo3`/`glam`).

## Finding #244 Fixed (Explicit Go-Ahead): `begin_frame_with_timeout`/`begin_frame_with_viewport_crop` Unified Into One Composable `begin_frame_with_options`

At the project owner's own explicit direction ("unify the trait surface for begin_frame_with_timeout and crop"), picking the third of finding #243's own three disposition options for the bounded-acquire-timeout mechanism.

**The real problem, confirmed by inspection before changing anything**: `VulkanDevice::begin_frame_impl` already took its bounded-acquire timeout and its viewport-crop size as two fully independent, freely-combinable parameters internally -- but the public `RhiDevice` trait exposed them through four separate, single-axis methods (`begin_frame`, `begin_frame_with_timeout`, `begin_frame_with_logical_size`, `begin_frame_with_viewport_crop`), so no caller could ever ask for a bounded wait *and* a viewport crop in the same frame even though the underlying mechanism never prevented it. This was the real reason the bounded-acquire path (finding #235's Option 3) ended up with zero production callers once viewport-crop (finding #238) shipped as the chosen resize-drag fix: the two were never composable, only exclusive.

**Fixed**: `tre-engine/src/rhi.rs` gains a new `BeginFrameOptions` struct (`timeout_ns: Option<u64>`, `crop_size: Option<(u32, u32)>`, both independent, both `None`-by-default matching plain `begin_frame`'s own behavior) and one new `RhiDevice::begin_frame_with_options(swapchain, options)` method, default-delegating to plain `begin_frame` for any backend that hasn't implemented it (the same pattern every prior `begin_frame_with_*` addition already used). `begin_frame_with_timeout` and `begin_frame_with_viewport_crop` are removed from the trait entirely -- not deprecated or kept as compatibility shims, since this is workspace-internal API with no external consumers to preserve. `begin_frame_with_logical_size` (Option 2's original stretch-based fix, itself now also without a real production caller since finding #238 shipped the crop-based replacement instead) was deliberately left untouched -- the project owner's own instruction named "timeout and crop" specifically, not logical-size, and folding in a third method not asked about would have been scope creep beyond what was requested.

`VulkanDevice`'s own implementation (`tre-rhi-vulkan/src/device.rs`) collapses its previous two separate one-line delegating methods into one `begin_frame_with_options` implementation, mapping `options.timeout_ns.unwrap_or(u64::MAX)`/`options.crop_size` directly onto `begin_frame_impl`'s own already-orthogonal parameters -- no new mechanism, only a single composable entry point onto one that already existed. `submit_frame_with_viewport_crop`'s own public signature and every one of its real call sites (`tre-python/src/windowed_renderer.rs`) were left completely unchanged -- only its internal body now builds a `BeginFrameOptions { crop_size: Some(crop_size), ..Default::default() }` and calls the new unified method, so this fix needed zero changes anywhere in `tre-python`. `tre-perf-suite/src/resize_test.rs`'s two real direct call sites (the `Coarse`/`Timeout` A/B-test strategies, plus the rare `SwapchainOutOfDate`-recovery fallback path) were updated to call `begin_frame_with_options` with only the one axis each strategy actually wants set -- deliberately kept as separate, single-axis calls rather than combined, since this harness's own whole purpose is comparing the two strategies in isolation, not compositing them.

**Full workspace verification clean**: `cargo fmt --all -- --check`, `cargo clippy --workspace --all-targets -- -D warnings`, `cargo build --workspace --all-targets` (debug and `--release`), and `cargo test --workspace --no-fail-fast` all passed with zero warnings and zero failures -- `tre-engine`'s own 180/180 unit tests unchanged. Beyond the automated suite, both `resize_test.rs` strategies (`Coarse` and `Timeout`) were re-run live against real hardware through the new unified call path and both started and ran cleanly with no crash, matching their pre-refactor behavior exactly.

**Status: fixed.** The trait surface now matches what the implementation already did internally -- `begin_frame_with_timeout`/`begin_frame_with_viewport_crop` are gone, replaced by one composable `begin_frame_with_options`, closing finding #243's own second flagged item.

## Finding #245 Fixed: `PyWindowedRenderer::new`/`WindowSlot::create` No Longer Duplicate Their Construction Logic

At the project owner's own explicit direction ("resolve the WindowSlot construction duplication finding"), closing finding #243's own third and final flagged item.

**The real constraint, confirmed before writing anything**: the natural-looking fix (one shared method both call sites invoke with their own `(surface_loader, surface)`) would need that method's own signature to name `ash::khr::surface::Instance`/`vk::SurfaceKHR` explicitly -- and `tre-python` has had no direct `ash` dependency since REVIEW.md findings #203/#204 deliberately removed it. Reintroducing it just to share this code would be a real regression against that decision, not a mechanical extraction.

**Fixed by sharing one step later than that**: both call sites already end up with an already-constructed `VulkanSwapchain` in hand -- a type `tre-python` already imports and uses directly, needing no `ash`/`vk` types anywhere. `crates/tre-python/src/windowed_renderer.rs`'s `WindowSlot` gains a new private `from_swapchain(device, swapchain, width, height) -> PyResult<Self>`, holding exactly the logic that was duplicated verbatim: registering this window's own shape pipelines against the swapchain's format, and assembling the `WindowSlot` struct itself. `WindowSlot::create` (every window after the first, obtaining its own surface via `device.create_surface`) now builds its swapchain and calls `from_swapchain`; `PyWindowedRenderer::new` (the very first window, whose surface comes bundled out of `VulkanDevice::new` itself -- a real Vulkan-bootstrapping requirement, not an artifact of the duplication) does the identical thing. The one line that still appears at both call sites (`VulkanSwapchain::new(device, surface_loader, surface, width, height)`) is a single, uniform constructor call with no room to diverge -- not the kind of duplicated *logic* findings #196/#213 were about, so it was left as-is rather than forcing it through a third layer of indirection for no real safety gain.

**Full workspace verification clean**: `cargo fmt --all -- --check`, `cargo clippy --workspace --all-targets -- -D warnings`, `cargo build --workspace --all-targets` (debug and `--release`), and `cargo test --workspace --no-fail-fast` all passed with zero warnings and zero failures -- `tre-engine`'s own 180/180 unit tests unchanged (this fix touched only `tre-python`). Beyond the automated suite, rebuilt the `tre_python` extension via `maturin develop --release` and ran a real live script exercising both construction paths in the same process -- the constructor's own main window, then `create_window`'s own second window -- both succeeded, both windows created and pumped through `poll_events` with no crash, confirming the shared code path works identically for the case it always handled (`create`) and the case that used to duplicate it (the constructor).

**Status: fixed.** The last of the three items REVIEW.md finding #243 flagged for the project owner's own direction is now resolved -- all of #243's follow-on work (findings #244, #245) is complete.

## Finding #246: `glam` Bumped to Its Real Newest MSRV-Compatible Version; `pyo3` Confirmed Already There, Not Bumped

At the project owner's own explicit direction ("bump pyo3 and glam"), closing finding #243's own last flagged item -- and finding, on real investigation before touching anything, that the two dependencies were not in the same situation at all.

**`glam` (`tre-tween`, `tre-python`): genuinely bumped, `0.29` to `0.33.7`.** Confirmed via `cargo add glam@0.33 --dry-run` before touching either `Cargo.toml`: `glam`'s own MSRV is `1.68.2` at every version up to and including latest, well under this workspace's `rust-version = 1.75`, so no conflict at any point in the range. Both crates' own real usage (`Vec2`/`Vec3`/`Quat` construction and basic arithmetic in `tre-tween`'s interpolation math and `PyTween`'s Python-facing construction) needed zero code changes across the four-minor-version jump. Full workspace verification clean, and beyond the automated suite, the `tre_python` extension was rebuilt via `maturin develop --release` and two real demos re-run (`demo/phase13_step13_2/demo.py`'s `Tween`/`Easing`/`Spring` suite, `demo/phase13_step13_3/demo.py`'s `Timeline` sequencer) -- both passed with their own exact, hand-computed numeric assertions unchanged.

**`pyo3` (`tre-python`): investigated, confirmed already at the newest version this workspace can use, NOT bumped.** Real research, not assumption, before concluding anything: `cargo info pyo3@0.29.2` (the version finding #243 itself named as "current") shows `rust-version: 1.83`. Bisecting the actual published version history between `0.27.2` (this workspace's current pin) and `0.29.2` found the real MSRV cliff precisely: `0.27.2` itself requires Rust `1.74` (compatible), while `0.28.2` -- the very next real release after it (`0.28.0`/`0.28.1` were checked directly and confirmed never published at all) -- already requires Rust `1.83`, and every release from there through `0.29.2` carries the identical `1.83` requirement. `0.27.2` is therefore already the newest version this workspace's own `rust-version = 1.75` MSRV can resolve -- finding #243's own "fallen behind" framing was correct about the raw version gap but wrong about the cause: this was never a missed update, and bumping it would require raising the entire workspace's MSRV by 8 points first, a materially larger, separate decision left to the project owner's own explicit direction rather than folded into what looked like a routine dependency bump.

**Documentation fix, not a version change**: `tre-python/Cargo.toml`'s `pyo3` entry gained the rationale comment this investigation itself produced (the exact MSRV cliff, confirmed directly, with the version numbers on both sides of it), so a future reader doesn't have to re-derive why this pin looks stale but genuinely isn't. `tre-tween`/`tre-python`'s `glam` entries gained the matching rationale for the real bump, following this workspace's own established "record the `cargo add --dry-run` confirmation inline" convention every other version-sensitive dependency already carries.

**Status: `glam` fixed (bumped); `pyo3` confirmed correct as-is, real reason now documented.** Both halves of finding #243's third-flagged item are closed -- one by a real version change, one by discovering there was nothing to change. Raising the workspace's own MSRV to unlock `pyo3` 0.29+ remains open, undecided, and unrequested.

## Finding #247: Fresh Full `/review-project` Pass -- Four Real Fixes, Ten New Tests, One Hygiene Consolidation, and the MSRV Claim Proven False

At the project owner's own instruction ("Do a complete review of the project and make it better"), ran this document's established four-lens (Performance/Architecture/Security/Modernizer) multi-agent review across the entire workspace, each lens instructed to grep this document's own 246 prior findings before reporting anything and to state what it grepped. Baseline before touching anything: `cargo clippy --workspace --all-targets` clean, `cargo test --workspace` 377/377.

**Security: one real hardening fix, one design exposure flagged.** Finding #243 had explicitly left `tre-python/src/custom_shader.rs`/`sdf_shadow.rs`'s runtime GLSL-to-SPIR-V path for a future pass; this pass examined it thoroughly. *Fixed* (`crates/tre-python/src/renderer.rs`): `HeadlessRenderer.create_custom_shader` handed caller-supplied GLSL of any length straight to `shaderc`'s C++ `glslang` front end -- a recursive-descent parser with no input-size or nesting limit of its own, where an adversarially large or deeply-nested string is a C++ stack overflow (a hard `SIGSEGV`, unreachable by Rust `catch_unwind` or PyO3's panic hook), not a clean compile error -- and did so with the GIL held for the whole compile, stalling every other Python thread in the process. Now capped by a new `MAX_CUSTOM_SHADER_SOURCE_BYTES` (1 MiB, the same class of bound as `MAX_DIMENSION`/`MAX_POLYGON_SIDES`/`tre-svg`'s byte caps; the crate's own largest built-in shader is under 4 KiB) and compiled under `py.detach`, the identical discipline `render`'s own GPU round trip already follows -- `RhiDevice: Send + Sync` and `Box<dyn RhiDevice>` made that a two-line change. Verified live after `maturin develop --release`: `demo/phase13_step13_8` passes unchanged; a 1,050,644-byte source is rejected with `ValueError` before `shaderc` sees it; a real shader still compiles; broken GLSL still surfaces `shaderc`'s own diagnostic. Disclosed honestly: a size cap bounds but cannot eliminate deep-nesting cost inside the budget. *Flagged for the project owner, not fixed*: every custom pipeline is linked against the engine's one universal pipeline layout (`create_universal_pipeline_layout`), which always includes the shared `UPDATE_AFTER_BIND`/`PARTIALLY_BOUND` bindless texture array, and `VulkanDevice::new` never requests `robustBufferAccess`/`robustImageAccess` -- so a caller's own fragment shader can redeclare `bindless_textures[]` verbatim and index it with an arbitrary value, which is undefined behavior per the Vulkan spec (anything from sampling another renderer's texture to a driver hang), not a defined zero. Three real mitigation layers exist (request the robustness features; give custom pipelines their own restricted descriptor-set layout without the bindless array; reflect the compiled SPIR-V and reject bindings outside the intended interface) and choosing among them is a design decision with real performance and API-surface trade-offs. Also confirmed clean: no `#include` file-read vector (`set_include_callback` is never registered, and vendored `shaderc` rejects every `#include` with a compile error when none is); compile errors are surfaced, not swallowed; the path is not reachable from `tre-ffi`'s C ABI at all (`TreErrorCode::ShaderCompilationFailed` exists but nothing wires it up -- whoever does must land the fixes above first, since `tre-ffi` has no Python exception layer). Low, flagged not changed: `Swatinem/rust-cache@v2` and the `actions/*` steps in both workflows are pinned by mutable tag rather than commit SHA, against this project's own exact-pin precedent elsewhere. Every other area re-audited (`tre-ffi`'s `extern "C"` surface, `tre-memory`'s `unsafe`, `tre-svg`'s SMIL parser, texture-size overflow arithmetic, `build.rs` subprocess arguments, secrets) matched its already-disclosed hardening.

**Performance: one real, new finding, fixed.** `crates/tre-engine/src/text.rs`'s multi-line render loop -- the consumer of the very wrap cache finding #243 added -- still built a throwaway owned `tre_text::ShapedRun` per visual line, `.to_vec()`-copying a sub-range of the already-cached, contiguous `flat_glyphs` purely to satisfy `RenderingCanvas::draw_text`'s `&ShapedRun` parameter type, on every call, including a full cache hit. `draw_text` only ever reads `shaped.glyphs` (never `text_range`/`direction`). Fixed additively, with zero call-site churn across `draw_text`'s ten existing callers: `crates/tre-engine/src/canvas.rs` gains `draw_glyphs(&mut self, glyphs: &[tre_text::ShapedGlyph], ...)` holding the real body, `draw_text` becomes a one-line delegation to it, and the multi-line loop passes `&wrapped.flat_glyphs[line.start_glyph..line.end_glyph]` straight through -- the same "share one step later than the obvious place" move finding #245 used. Verified live: `demo/phase15_step15_1`'s three real-ink-band assertions pass unchanged. Every other hot path (`ShapeRegistry::flatten_into`, `FlattenedFrame` radix sort, `tre-memory`'s orderings and `fetch_update` reservation, the atlas owner, both `tre-python` renderers' per-frame reuse and GIL release, the Vulkan command-recording path) was re-read and found either already optimal or already covered by an existing finding (#136, #210, #219, #222/#228, #236, #243).

**Architecture: one high-severity finding flagged, four mechanical findings fixed, three flagged.** *(1) `rust-version = "1.75"` is false, not merely unverified -- High, flagged.* Proven empirically, not inferred: installing `1.75.0` and running `cargo +1.75.0 check -p tre-math` fails immediately with `error[E0658]: lint reasons are experimental` at `crates/tre-math/src/lib.rs:797` -- the `reason = "..."` field on `#[allow]` (RFC 2383) stabilized in Rust 1.81.0 and this workspace uses it 151 times across 42 files, in library code every consumer compiles. Going one step further, `cargo +1.81.0 check --workspace` fails before compiling anything: `clap 4.6.6`, pulled in through `cbindgen` (`tre-ffi`'s build dependency) and `criterion`, declares `rust-version = 1.85`, so the resolved `Cargo.lock` cannot even be parsed below 1.85 -- edition 2024's own floor. No CI job catches any of this because every job runs `rustup show`, which installs `rust-toolchain.toml`'s 1.98.0 pin; the pin actively hides the violation. The cost is real: `pyo3 0.27.2`, `usvg =0.45.1`, `rustc-hash =2.1.1`, `skrifa =0.33.2`, `accesskit =0.17.1`, `wide 0.7.33` are each frozen with an inline comment citing 1.75 as the reason, and finding #246's whole `pyo3` investigation concluded against a constraint the code does not satisfy. Not changed here: finding #246 explicitly reserved the MSRV decision for the project owner, and correcting a false claim still requires choosing the new value -- see "Items awaiting the project owner" below for the options with their real unlock lists. *(2) `ARCHITECTURE.md` Section 6 contradicted the post-#244 trait -- Med, fixed.* The annotation paragraph finding #243 had just added still described `begin_frame_with_timeout`/`begin_frame_with_viewport_crop` as live methods; finding #244 deleted both one finding later and never annotated it -- the second consecutive break of the convention #243 had just repaired. A new annotation records the collapse into `begin_frame_with_options`/`BeginFrameOptions`, states that timeout and crop now compose, and narrows the surviving mutual-exclusivity sentence to what is still true (stretch vs. crop). *(3) Three Vulkan-extension-shaped RHI methods had no default body -- Med, half fixed.* `RhiSwapchain::supports_local_read_input_attachment`, `RhiDevice::local_read_blend_supported`, and `RhiCommandBuffer::insert_blend_read_barrier` are named and doc-specified in `VK_KHR_dynamic_rendering_local_read`'s own vocabulary inside the crate whose whole purpose is keeping backends out of the core (`tre-engine` is `#![forbid(unsafe_code)]` with no `ash` dependency), and -- unlike `acquire_next_image_with_timeout` and every `begin_frame_with_*` addition -- were required methods, so a future DX12/Metal backend could not compile without implementing a Vulkan concept D3D12 lacks entirely. Fixed the mechanical half: all three gained `false`/`false`/no-op defaults (behavior-preserving; `VulkanDevice`, `VulkanSwapchain`, `VulkanCommandBuffer`, `HeadlessSwapchain`, and both `FakeDevice` mocks all still override them). The vendor-neutral rename (e.g. `supports_framebuffer_fetch`) is a naming decision, flagged. *(4) `tre-rhi-vulkan` had zero tests -- Med, tier one fixed.* The largest, `unsafe`-heaviest crate reported `running 0 tests`; its only correctness gate was ~40 `examples/` runs that `cargo test` can never execute and a contributor without a GPU cannot run. Ten `#[test]` functions added for its GPU-free helpers: `align_up`, `texture_format_to_vk`, `bytes_per_pixel` (`lib.rs`); `is_local_read_rejected_by_layer`/`is_known_false_positive` (`device.rs`, gated on `debug_assertions` exactly as the functions are -- these two string predicates were finding #208's own root-cause fix and decide whether the validation callback aborts, i.e. whether CI catches a real Vulkan error at all, and had never been tested; one test pins the subset invariant the callback relies on); and `BindlessRegistry`'s allocate/release/exhaustion/LIFO-reuse behavior (`transient_pool.rs`, what `create_texture`'s "bindless array exhausted" error depends on). Tier two -- promoting the self-asserting headless examples into `tests/` with the graceful-skip-when-no-device pattern `tre-a11y` already proved -- touches ~30 verified-working files, the same scope objection that deferred #140; flagged. *(5) "This workspace's only C ABI boundary is Vulkan" was false in three places -- Low, fixed.* `IMPLEMENTATION.md` Step 4.1, `docs/rust-api/text-svg-atlas.md`, and `tre-text/Cargo.toml` all carried it; the same Step 4.1 that wrote the sentence added `fontconfig = "=0.8.0"`, which wraps the system `libfontconfig`, and later phases added `libshaderc`, GTK 3 + `libayatana-appindicator`, and `winit`'s Wayland/XCB client libraries -- exactly the `apt-get` list `ci.yml` maintains. All three corrected to say what is true. *Flagged, not fixed (design decisions):* `VulkanDevice::new` is a 657-line constructor with six clearly demarcated internal seams (instance, debug messenger, physical-device selection, logical device + pools, the ~230-line bindless subsystem, transient pool + GC thread) -- a pure extraction, but of the workspace's most `unsafe`-dense code, verifiable only on live hardware; 87 of `tre-engine/src/lib.rs`'s 90 tests exercise `canvas.rs`/`rhi.rs`/`input.rs` (a `#[cfg(test)] pub(crate) use canvas::{...}` at `lib.rs:32-35` exists only to let them reach across), with `FakeDevice` hand-written twice -- a pure relocation with an exact test-count success criterion, but ~3,200 lines of it; and no crate declares any `[features]`, so `tre-engine` unconditionally pulls the full font/MSDF/tessellation stack and a `libfontconfig` link into `tre-perf-suite` and every other shapes-only consumer. Confirmed clean: no dependency cycles, correct layering, no leaked concrete backend types, every `pub use` list matches its module, and `EngineError`'s lack of `#[non_exhaustive]` is correct (`tre-ffi/src/error.rs`'s exhaustive `match` is a deliberate compile-time net).

**Modernizer: one real hygiene gap fixed, small idiom sweep, MSRV unlock table for the owner.** *No `[workspace.dependencies]` table existed.* `[workspace.package]` already used the `field.workspace = true` idiom for `version`/`edition`/`rust-version`/`license`, but that pattern was never extended to dependencies: `bytemuck`, `raw-window-handle`, `thiserror`, `skrifa`, and `rustybuzz` were repeated as literal version strings across 22 lines in 8 manifests, identical today only by discipline -- one `cargo add` in one crate could silently make `tre-python` and `tre-engine` disagree on what version of `skrifa` counts as "the" font stack, the exact hazard `tre-python/Cargo.toml`'s own comment on those pins warns about. Consolidated into a root `[workspace.dependencies]` table (versions and `=` pin styles carried over unchanged; each crate's inline MSRV/pin rationale left in place next to the crate that established it; `tre-engine`'s `bytemuck` `derive` feature composes on top as `{ workspace = true, features = ["derive"] }`); `Cargo.lock` is byte-identical afterward and `cargo tree -d` shows no new duplicate. *Idiom sweep:* three `.map(f).unwrap_or(x)` → `.map_or(x, f)` (`tre-atlas/src/lib.rs`, `tre-perf-suite/src/telemetry.rs`), two `u32 as u64` → `u64::from` (`tre-rhi-vulkan/src/command_buffer.rs`), one missing `#[must_use]` (`CpuMemSampler::new`), one collect-then-`len` → `count` in a `tre-engine` test, and six `clippy::redundant_clone` hits, all in `examples/`, applied by `cargo clippy --fix`. Deliberately *not* applied: five `clippy::needless_collect` hits that are correct as written (three `alloc_guard` tests whose whole point is that the `collect` allocates; one thread-handle collect that must complete before joining), and all 14 `match_same_arms` hits (`tre-platform`'s two-variant `Wayland`/`X11` dispatch, where explicit arms are the safer shape). Already-idiomatic, confirmed: zero manual `Display`/`Error` impls remain after #215, zero `let-else` candidates, zero `&Vec`/`&String` parameters, zero `static mut` (edition-2024-clean on that axis). *MSRV unlock table*, bisected via `cargo info` the same way #246 did, for the owner's decision: 1.77 unlocks only `rustc-hash 2.1.3`; 1.80 adds `skrifa 0.34-0.36`; 1.83 adds `pyo3 0.28.2+/0.29.x`; 1.85 (edition 2024's floor, and -- per Architecture (1) above -- already the real floor of the resolved lockfile) adds `skrifa` through latest `0.47`, `usvg 0.48.1`, `accesskit` currency, and edition 2024 itself; `usvg 0.46/0.47/0.48.0` need 1.87/1.87/1.89 and `wide 1.x` needs 1.89, out of reach even at 1.85. Recommended pedantic lints worth enabling workspace-wide (currently only `tre-engine`/`tre-math`/`tre-svg` opt in): `needless_pass_by_value` (25 hits), `missing_errors_doc`/`missing_panics_doc` (7), `cast_lossless`, `map_unwrap_or`; recommended against: the ~160-hit cast-lint family as a blanket flip (extend per-crate instead, matching the existing precedent), `match_same_arms`, `doc_markdown`, `too_long_first_doc_paragraph`, `mul_add` (FMA changes rounding), and `missing_const_for_fn` (#220 already declined it).

**Full workspace verification clean**: `cargo fmt --all -- --check`, `cargo clippy --workspace --all-targets -- -D warnings`, `cargo build --workspace --all-targets` (debug and `--release`), and `cargo test --workspace` all passed with zero warnings and zero failures -- 387 tests, up from 377 (the ten new `tre-rhi-vulkan` tests; every pre-existing per-crate count unchanged, `tre-engine` still 180/180). Beyond the automated suite: `maturin develop --release` rebuilt the `tre_python` extension and `demo/phase15_step15_1` (wrapped text, the `draw_glyphs` path) and `demo/phase13_step13_8` (custom shaders, the GIL-released compile) both passed with their own pixel-level assertions unchanged, plus the three-case shader-cap smoke test described under Security.

**Items awaiting the project owner** (each a real decision, none a bug to fix silently): (a) the MSRV -- the claim is false today; options are raise `rust-version` to the empirically real floor (1.85, per the lockfile), raise it higher deliberately to unlock the frozen dependencies listed above, or drop the field and state "we track the pinned toolchain" as policy -- and in every case add a CI job that actually builds at the declared floor with `--locked`, so the pin can never hide a violation again; (b) the custom-shader bindless exposure -- which of the three mitigation layers (or combination) to adopt; (c) the vendor-neutral rename of the three local-read trait methods; (d) promoting the self-asserting Vulkan examples into real `tests/`; (e) extracting `VulkanDevice::new`'s six seams; (f) relocating `tre-engine`'s 87 misplaced tests and unifying `FakeDevice`; (g) a `[features]` story for `tre-engine`; (h) SHA-pinning the GitHub Actions; (i) which pedantic lints to enable workspace-wide.

**Status: four real fixes shipped (shader cap + GIL release, `draw_glyphs`, three RHI defaults, three C-ABI-claim corrections), one documentation gap closed (`ARCHITECTURE.md` #244 annotation), one hygiene consolidation (`[workspace.dependencies]`), ten new tests, a twelve-site idiom sweep; nine items awaiting the project owner's own direction, the MSRV correction first among them.**

## Finding #248 Fixed (Explicit Go-Ahead): `rust-version` Raised to 1.88 -- the Real, Now CI-Enforced Floor -- With the 1.85 Asked For Explained Rather Than Faked

At the project owner's own explicit direction ("Raise rust-version to 1.85 and add an MSRV CI job"), closing finding #247's first flagged item.

**Why 1.88 and not the 1.85 asked for -- found by doing, not assuming.** Finding #247's report called 1.85 "the real lockfile floor." That figure came from a bisection that stopped one step early: `cargo +1.81.0 check` had failed on `clap 4.6.6`'s manifest (declared MSRV 1.85), and 1.85 was inferred from that rather than tested. Installing `1.85.0` and running the check for real produced a hard error listing eighteen locked packages whose declared `rust-version` exceeds it -- `image 0.25.10` and all seven `icu_*` 2.3 crates at 1.88, `nalgebra 0.34.2` and the `zbus`/`zvariant` 5.x family at 1.87, `idna_adapter 1.2.2` and `wayland-protocols 0.32.13` at 1.86. Most of those are transitive and could be walked back in the lockfile alone (`zbus 5.11` is 1.77, `wayland-protocols 0.32.12` is 1.71, `icu_*` 2.2 is 1.86). Two are not: `image` and `nalgebra` are exact `=` pins in `tre-text/Cargo.toml`, placed there as `fdsm`'s own internal dependencies with a comment claiming 1.75 compatibility that was never true -- and `fdsm 0.8.0` itself requires `nalgebra ^0.34.1`, every release of which declares MSRV 1.87. So 1.85 is only reachable by downgrading the MSDF generator to a release with a different `nalgebra` line (none of `fdsm`'s published 0.7.x releases resolves as a usable candidate), a functional-library downgrade the instruction did not ask for and this finding did not make. The honest choices were therefore 1.87 (walk `image` back one patch to 0.25.9 and the `icu_*` crates back to 2.2, for a one-point gain) or 1.88 (the floor the committed lockfile already has, with zero dependency churn). 1.88 was chosen: it is exactly true today, changes no resolved version, and is also where let-chains stabilized, should edition 2024 ever be adopted. `cargo +1.88.0 check --workspace --all-targets --locked` passes; `Cargo.lock` is byte-identical.

**The CI job (`.github/workflows/ci.yml`, new `msrv` job).** Deliberately the one job that does *not* run `rustup show` -- every other job installs `rust-toolchain.toml`'s 1.98.0 development pin, which is precisely how a false `rust-version` went unverified for weeks. The job reads the declared floor out of `Cargo.toml` with a one-line `sed` (so the manifest and the job cannot drift apart), installs exactly that toolchain with `--profile minimal`, installs the same system packages every compiling job needs (`cargo check` still runs every `build.rs`: `glslc`, `shaderc`, `fontconfig`), and runs `cargo check --workspace --all-targets --locked`. `--locked` is the load-bearing flag: a future dependency whose own MSRV exceeds the floor fails the job instead of being silently re-resolved around. The YAML was parsed locally before committing; the job's own `sed` extraction was run locally and yields `1.88`.

**What raising the floor woke up.** With `rust-version = 1.88`, clippy's MSRV-gated lints activated across the workspace and `-D warnings` immediately failed. All were genuine, machine-applicable idiom upgrades for APIs that stabilized at or below the new floor, and all were applied (42 files by `cargo clippy --fix`, five sites by hand where the fix pass declined): `slice::as_chunks`/`as_chunks_mut` (stable 1.88) replacing `chunks_exact(N)`/`chunks_exact_mut(N)` in the BGRA-to-RGBA swap loop ~30 Vulkan examples share, two `ShapeRegistry` tessellation paths, `tre-svg`'s tessellator, `tre-text`'s `pad_rgb_to_rgba` (which lost a fallible `try_into().expect(..)` entirely, since a `[u8; 3]` destructures), and three `GpuShapeStyle` layout tests; `is_multiple_of` (1.87) in three examples' frame-count checks; `f32::midpoint` (1.85) in one test helper. 47 sites, 44 files. Behavior is identical by construction (`as_chunks` drops a trailing remainder exactly as `chunks_exact` did), and confirmed live: `shape_registry_demo` and `msdf_rendering_demo` -- the latter exercising both the read-only and mutable variants -- re-run on real hardware with their exact pixel assertions unchanged.

**Every stale 1.75 rationale corrected, no pin bumped.** Seven member manifests cited `rust-version = 1.75` as the reason a dependency was held back (`thiserror`, `accesskit`/`accesskit_unix`, `usvg`, `glam`, `pyo3`, `skrifa`, `fdsm`, `rustc-hash`); each comment now records that the floor is 1.88, that the old figure was never true, and that the pin has *not* been re-evaluated -- a dependency-series bump (`pyo3` 0.29, `usvg` 0.48.1, `skrifa` 0.47, `accesskit` 0.25, `rustc-hash` 2.1.3, all now in reach) is each its own migration, not a side effect of an MSRV correction. `fdsm`'s comment specifically now says what is true: its `nalgebra` requirement, together with `image` and `icu_*`, is what sets this workspace's floor. `rust-toolchain.toml` gained a paragraph stating that it is the development toolchain and not the MSRV, and where the MSRV actually lives; `TECHNICAL.md` Section 9.1 (which had said "pinned at 1.75+ via `rust-toolchain.toml`," wrong on both counts) and its "Cargo 1.75+" build-system line, plus two historical `IMPLEMENTATION.md` sentences, were corrected in place.
...
## Finding #250 Fixed (Explicit Go-Ahead): `usvg` Bumped From 0.45.1 to 0.48.1

Second of the five bumps the project owner directed. `tre-svg/Cargo.toml`'s exact pin moves from `=0.45.1` to `=0.48.1`, the newest published release under the 1.88 floor -- and specifically the *patch* release: `usvg 0.48.0` declares `rust-version = 1.89`, `0.48.1` declares `1.85`, a non-monotonic MSRV this document's Modernizer lens had already flagged as worth double-checking (finding #247). Checked directly against the real toolchain rather than trusted from the manifest field: `cargo +1.88.0 check --workspace --all-targets --locked` is clean after the bump, covering the five transitive crates it moved as well (`kurbo` 0.11.3 to 0.13.1, `tiny-skia-path` 0.11.4 to 0.12.0, `svgtypes` 0.15.3 to 0.16.1, `imagesize` 0.13.0 to 0.15.0, plus new `roxmltree 0.21.1` and `polycool 0.4.0`).

Zero source changes were needed. `tre-svg`'s entire `usvg` surface is `Tree::from_data`/`Options::default`, the `Node::{Group, Path, Image, Text}` walk with `Group`/`Path` accessors, and `tiny_skia_path`'s `PathSegment`/`Transform`/`Point` -- none of which changed shape across the three minor versions. Verified beyond compilation, since a parser's *defaults* can shift without a compile error: `cargo clippy -p tre-svg --all-targets -- -D warnings` clean; all 33 `tre-svg` tests pass (including finding #214's SMIL hardening suite and its arbitrary-bytes fuzz test, which run against the new parser); and both real Vulkan SVG examples (`svg_morph_demo`, `svg_tessellation_demo`) re-run live with their pixel assertions unchanged. The manifest comment was rewritten to say what is now true.

**Status: fixed.**

## Finding #251 Fixed (Explicit Go-Ahead): `skrifa` Bumped From 0.33.2 to 0.47.0

Third of the five bumps the project owner directed, and the largest version distance of the five: fourteen minor versions of the `fontations` metadata/outline crate, shared by four member crates through the one `[workspace.dependencies]` entry finding #247 introduced (`tre-text`, `tre-engine`, `tre-python`, and `tre-rhi-vulkan`'s examples), so a single line moved all four in lockstep. 0.47.0 is the newest published release; it declares MSRV 1.85. `read-fonts` (0.31.3 to 0.44.0) and `font-types` (0.9.0 to 0.12.5) moved with it; nothing else in the lockfile changed.

Zero source changes were needed -- confirmed by building every target rather than assumed from the changelog. The workspace's real `skrifa` surface is small and stable: `FontRef::new`, `MetadataProvider::{metrics, charmap, outline_glyphs}`, `instance::{Size::unscaled, LocationRef::default}`, `GlyphId::from`, and `tre-text/src/outline.rs`'s `OutlinePen` recorder driven by `OutlineGlyph::draw`. Verified beyond compilation, since a font stack can change *numbers* (advances, metrics, outline point placement) without changing *types*: `cargo clippy --workspace --all-targets -- -D warnings` clean; all 387 workspace tests pass -- including `tre-text`'s 41 (shaping, wrapping, caret, MSDF, real-font outline extraction), `tre-engine`'s 180, and `tre-python`'s font-validation tests; `cargo +1.88.0 check --workspace --all-targets --locked` clean; and the three pixel-asserting Vulkan text demos (`msdf_rendering_demo`'s exact interior/hole probes, `text_shaping_demo`, `canvas_draw_text_demo`) re-run live with every assertion unchanged, which is the real evidence that metrics and outlines came out identical. The three manifest comments citing 0.33.2/0.46.2 were rewritten.

**Status: fixed.**

## Finding #252 Fixed (Explicit Go-Ahead): `accesskit` 0.17.1 to 0.25.0 and `accesskit_unix` 0.13.1 to 0.23.0, With `zbus` 4 to 5 Alongside

Fourth of the five bumps the project owner directed, and the first of the five to need real source changes -- and the first whose breakage only a live D-Bus round trip could reveal, not the compiler.

**Two real API changes in the core crate, both handled in `tre-a11y/src/lib.rs`.** (1) `accesskit::Tree` is now a deprecated alias for `TreeInfo`, and `TreeInfo` has no `app_name` field at all -- the application name is no longer part of the core tree description. Read from `accesskit_unix` 0.23's own `context.rs` rather than assumed: the Linux adapter now derives the name itself, once per process, from the running executable's file name (`std::env::current_exe`). `A11yBridge::connect` keeps its three-parameter signature for API stability (`tre-python`'s own `A11yBridge` constructor passes `app_name` straight through, and changing a public Python signature for a parameter the platform stack stopped consuming is the wrong trade), with the parameter documented as accepted-but-no-longer-forwarded. Only `toolkit_name`/`toolkit_version` still reach the bus, and those are exactly what the round-trip test and both verification examples already identify the app by. (2) `TreeUpdate` gained a mandatory `tree_id: TreeId`; this bridge publishes exactly one tree, the application's root, which is what `TreeId::ROOT` (the nil UUID) is reserved for.

**The dev-dependency that stopped building, and why.** `accesskit_unix` 0.23 depends on `zbus ^5.5`. The workspace's own `zbus 4.4.0` (a `default-features = false, features = ["blocking"]` dev-dependency in `tre-a11y` and `tre-rhi-vulkan`, for the round-trip test and the two accessibility verification examples) immediately failed to compile with "Either `async-io` (default) or `tokio` must be enabled" -- its I/O backend had only ever been enabled *transitively*, by the old `accesskit_unix`'s own feature selection on the same `zbus` 4 package, and the moment that dependency moved to `zbus` 5 the unified feature set for 4.4 collapsed to nothing usable. A latent fragility, not a new one. Both dev-dependencies moved to `zbus 5.19.0` (MSRV 1.87) with `features = ["async-io", "blocking-api"]` declared explicitly this time -- `blocking-api` gates `zbus::blocking` in 5.x, `async-io` is the required backend -- and `Cargo.lock` now holds a single `zbus` major where it previously held two. The one source-level rename that came with `zbus` 5 (`blocking::ConnectionBuilder` is now `blocking::connection::Builder`) was applied at the import in all three files.

**The break only the live bus caught.** With everything compiling and clippy clean, `tre-a11y`'s real AT-SPI2 round-trip test failed: it asserted the published node's `AccessibilityNodeId(123)` appeared verbatim as the last segment of its AT-SPI2 object path, and the new adapter reported `/org/a11y/atspi/accessible/0/2268949521066274848768`. That number is exactly `123 << 64`. Confirmed from the source rather than the arithmetic: `accesskit_unix` 0.23's `atspi/object_id.rs` formats the segment as `u128::from(FullNodeId)`, and `accesskit_consumer` 0.39's `FullNodeId` packs `(node_id << 64) | tree_index`. The test and `canvas_accessibility_verify` (which located tagged children by the same bare-id suffix) each gained a small `node_id_from_atspi_path` decoder that recovers the node id from the high 64 bits, with the scheme's real source cited inline -- the assertion's *intent* (identity, not just presence) is unchanged, only its knowledge of the adapter's private encoding was refreshed. Both now pass live: the round-trip test against this machine's real accessibility bus, and the full `canvas_accessibility_demo` then `canvas_accessibility_verify` sequence CI's own `accessibility-validation` job runs, with all three tagged nodes' independently-queried `GetExtents`/`GetRole` matching the IR exactly.

**Full verification clean**: `cargo fmt --all -- --check`, `cargo clippy --workspace --all-targets -- -D warnings`, `cargo build --workspace --all-targets`, `cargo test --workspace` (387/387), and `cargo +1.88.0 check --workspace --all-targets --locked` (the eleven transitive crates that moved with this bump -- `atspi` 0.22 to 0.29, `accesskit_consumer` 0.26 to 0.39, `accesskit_atspi_common` 0.10 to 0.20, `zbus_xml`, `zbus-lockstep`, `foldhash`, four new `phf` crates -- all sit under the floor).

**Status: fixed.** The manifest comments in both crates were rewritten to record the bump, the API changes, and the real reason the `zbus` dev-dependency had to move with it.

## Finding #253 Fixed (Explicit Go-Ahead): `pyo3` Bumped From 0.27.2 to 0.29.2

Fifth and last of the bumps the project owner directed, and the one finding #246 had explicitly set aside as "its own real migration": two minor series of the Python binding layer, across all 23 `tre-python` source files that touch it.

**One real migration item, resolved by the compiler, not by guesswork.** PyO3 0.29 compiles this crate with zero hard errors but 25 deprecation warnings, all one kind: the *automatic* `FromPyObject` implementation every `Clone` `#[pyclass]` used to receive is becoming opt-in, and each class must now declare `from_py_object` (keep it) or `skip_from_py_object` (drop it). Under this workspace's `-D warnings` gate a deprecation is a failure, so all 25 had to be decided. Rather than blanket-keeping the derive (behavior-preserving but leaving dead conversion impls on eleven types) or guessing per type, the decision was made mechanically: every one of the 25 was first marked `skip_from_py_object`, the build was allowed to fail, and the 46 resulting `cannot be used as a Python function argument` errors named exactly the fourteen types Python really does pass by value into a Rust signature -- `WindowId`, `MouseButton`, `ElementState`, `InputEvent`, `CursorIcon`, `FocusableNode`, `AccessibilityRole`, `AccessibilityNode`, `TextureFormat`, `Texture`, `GradientId`, `CustomShaderId`, `FillRule`, `Easing`. Those fourteen carry `from_py_object`; the other eleven (`ShapeId`, the six shape classes, `Gradient`, the two SMIL record classes, `TrayEvent`), which this crate only ever constructs and returns, carry `skip_from_py_object`. A second build was clean with zero warnings, and no other deprecation of any kind fired -- `py.detach`, `Bound`, the `signature`/`getter`/`frozen`/`eq`/`hash` attribute forms, and `wrap_pyfunction` all crossed the two minor versions unchanged.

**Verified against the real Python runtime, not just the compiler.** `cargo clippy --workspace --all-targets -- -D warnings` clean; `cargo test --workspace` 387/387; `cargo +1.88.0 check --workspace --all-targets --locked` clean (the five `pyo3-*` crates moved together; nothing else in the lockfile changed). Then `maturin develop --release` rebuilt the `tre_python` extension and **all thirty** `demo/*/demo.py` scripts were re-run against it -- the 25 headless ones unattended, the five windowed ones (windowed renderer, cursor/IME passthrough, system tray, tray/clipboard follow-ups, focus/keyboard navigation) live on this machine's real Wayland session -- and every one printed its own `PASSED` with its assertions unchanged. That set is broader than any prior pass ran, deliberately: CI's own workflow never builds or exercises the Python extension at all (a gap this document's next review pass should weigh), so local coverage is the only coverage this bump gets. Those demos also re-exercise findings #250-#252's bumps through Python (`Svg`/`parse_smil` over the new `usvg`, `Font`/`Text` over the new `skrifa`, `A11yBridge` over the new `accesskit`), which is the first end-to-end confirmation of all five bumps composed together in one process.

**Status: fixed.** All five owner-directed bumps (#249-#253) are complete. Every dependency finding #248 listed as "now unlockable" is now at its newest published version under the 1.88 floor, and the manifest comments say so.

## Finding #254 Fixed (Explicit Go-Ahead): CI Now Builds the `tre_python` Wheel and Runs Every Python Demo

At the project owner's own explicit direction ("I want the CI to build and run the python extension"), closing the gap finding #253's own verification had exposed: nothing in `.github/workflows/ci.yml` had ever built or run the Python extension. Every Python-facing change through Phase 21 and findings #247-#253 -- thirty self-asserting `demo/*/demo.py` scripts' worth -- had been verified only by a human running `maturin develop` locally.

**The job (`python`, `.github/workflows/ci.yml`).** Deliberately built from recipes the workflow had already proven on GitHub's runners rather than anything new: the `vulkan-validation` job's software Vulkan ICD (`mesa-vulkan-drivers`) + `xvfb` + real fonts for GPU work with no GPU, and the `accessibility-validation` job's `dbus`/`at-spi2-core`/`dconf` recipe (finding #126's hard-won `toolkit-accessibility` GSettings write) for the one demo that needs a real AT-SPI2 bus. `actions/setup-python@v5` pins Python 3.12 (the wheel is a non-`abi3` build, tied to its exact Python minor, exactly as `docs/getting-started.md` discloses); `maturin build --release -m crates/tre-python/Cargo.toml` produces the real wheel from the crate manifest alone (there is no pyproject, and none is needed -- confirmed locally, where the same command produced an installable `cp314` wheel), `pip install dist/*.whl` installs it into the runner's Python, and `pillow` is installed because it is the single third-party import across all thirty demos (`phase17_step17_1` reads back the PNGs `TrayIcon.set_icon` writes). The `canvas_accessibility_single_process_check_verify` example that `phase18_step18_3` spawns as its independent AT-SPI2 client is built up front with the exact command that demo's own error message names. No `vulkan-validationlayers`: a `--release` wheel never requests the layer, so it would be dead weight. Then one step runs every demo, each inside its own fresh `dbus-run-session` + `xvfb-run -a` + a 600-second `timeout`, printing a per-demo `PASS`/`FAIL` line inside a collapsible log group and a final tally, and failing only at the end with the failed names listed -- so one broken demo is named, not hidden behind a fail-fast. Exactly one demo is skipped, with the reason stated at the skip site: `phase14_step14_3`'s third check asserts a native "Open File" dialog is *still* blocking after a real two-second wait, which is only true when a live XDG desktop portal exists to show one; with no portal the call returns immediately and the assertion's premise -- not the code under test -- is gone.

**Rehearsed locally as far as this machine allows.** The demo-loop step was extracted verbatim from the YAML (parsed with PyYAML, so the file is also confirmed well-formed) and run against a *fresh* virtualenv with the wheel installed via `pip` from the built `dist/` -- the CI install path, not `maturin develop` -- with two unavoidable substitutions: no `xvfb-run` (Xvfb is not installed here, and this session is Wayland; the X11-under-Xvfb path is the one part only the real run can confirm, though every windowed Rust example already takes it in CI daily), and the venv's own interpreter. Result: 28 `PASS`, 1 `SKIP` (as designed), 1 `FAIL` -- `phase18_step18_3`, and only under the nested session. Chased to a real local cause, not waved off: inside a `dbus-run-session` on this live KDE/dbus-broker desktop, the freshly activated `org.a11y.Bus` cannot start `at-spi2-registryd` ("Could not activate remote peer ... unit failed", a systemd-user-unit activation that only a real login session can satisfy), which is a nesting artifact of this desktop, not of the demo -- CI's `accessibility-validation` job makes the identical `Registry.GetChildren` call under `dbus-run-session` on Ubuntu's `dbus-daemon` and passes today. Re-run on the live bus through the same wheel, the demo passes in full ("SINGLE-PROCESS FEASIBILITY CONFIRMED"). So every one of the thirty demos has been exercised through the wheel-install path: 29 in the loop or individually, plus the skipped one, which passed through `maturin develop` in #253.

**A real, recorded side effect of that rehearsal, and the lesson.** The first nested-session attempt's own `at-spi-bus-launcher` recreated `/run/user/1000/at-spi/bus_0` -- the *live* session's socket path, shared through the same `XDG_RUNTIME_DIR` -- and when that throwaway session ended, the path was left pointing at a dead socket while the live launcher (running since login) still listened on the unlinked one. Every subsequent a11y connection on this machine, from either install path, failed with "Connection refused" until `systemctl --user restart at-spi-dbus-bus.service` recreated the socket (verified: socket timestamp, `IsEnabled` true, demo passing). Recorded here so nobody repeats it: never rehearse the AT-SPI2 demo inside a nested `dbus-run-session` on a live desktop; run it on the session bus. CI has no live session to collide with, which is exactly why the per-demo fresh session is right there and wrong here.

**Also in this commit**: `docs/getting-started.md` still said the extension was built with PyO3 0.27 (stale since #253) -- corrected to 0.29; `TECHNICAL.md` Section 9.2's CI sentence, which had claimed "the Python-binding integration tests" already ran on every commit (they never had), now describes the real job.

**Xvfb rehearsal, once the project owner installed `xvfb-run` mid-pass.** The same loop step re-run under `xvfb-run -a` with `WAYLAND_DISPLAY` unset (so `winit` takes the X11 path a headless runner takes), on the live session bus: 9 `PASS`, 1 `SKIP`, 20 `FAIL` -- and the split is exact: every demo that creates a Vulkan device failed, every demo that never touches the GPU (clock, tween, timeline, clipboard, both tray demos including the windowed one) passed. The failure is `TreError: GPU device lost` from `HeadlessRenderer.__init__`, and it is this machine's, not the job's: the Rust `headless` example -- which CI's `vulkan-validation` job runs under `xvfb-run` on every commit and passes -- fails here identically (`DeviceLost`, preceded by Mesa's own "Note: you can probably enable DRI3 in your Xorg config"). The only Vulkan driver installed here is the hardware RADV one, which cannot present to a DRI3-less Xvfb; the runners' `mesa-vulkan-drivers` package includes the software `lavapipe` driver, which is what every Rust example job actually renders with on Xvfb, and which this machine does not have (`vulkan-swrast` is not installed). So the X11 path itself is now confirmed locally for the non-GPU demos, the tray demos are confirmed to run on X11 (with a host present -- this desktop's own `StatusNotifierWatcher` is still reachable through the live session bus), and the GPU-under-Xvfb path rests on the same driver the Rust jobs already prove daily. Installing `vulkan-swrast` locally and re-running the loop with `VK_DRIVER_FILES` pointed at `lvp_icd.x86_64.json` would close that last gap without a CI round trip.

**Lavapipe rehearsal, once the project owner installed `vulkan-swrast` too.** With `VK_DRIVER_FILES` pointed at `lvp_icd.json` (Arch's package installs it without the architecture suffix the Ubuntu one carries), the Rust `headless` control example renders under Xvfb, and the loop step gives 27 `PASS`, 1 `SKIP`, 2 `FAIL` -- two real, CI-relevant failures that would have failed the first real run, found and fixed here instead. (1) `demo/phase16_step16_1` (SDF soft shadows) asserted `abs(spread - blur_px) <= 3` and measured a 12-pixel ink spread for a 16-pixel blur on lavapipe; on RADV it measures 13, which already sat exactly on that bound. The demo's own claim ("ink should reach almost exactly `blur_px`") was stronger than its shader guarantees: the smoothstep tail's last ~0.11 sigma (alpha ~3u^2 falling below one 8-bit sRGB step at u ~0.054) quantizes to background, plus up to a pixel of rasterization rounding, so the measured spread must *undershoot* by a tenth to a quarter and the exact figure legitimately differs per driver. The assertion now checks `0 <= blur_px - spread <= 0.25 * blur_px + 1`, with the derivation and both drivers' measured values in the demo; it passes on both, and the strict-monotonic check that actually proves tunability is untouched. (2) `demo/phase19_step19_4` asserts a real `WindowFocused(focused=True)` event on window creation, and observed only `[False]`: a bare Xvfb has no window manager, and on X11 a new window only ever receives focus from one (`winit`'s own `focus_window` is a `_NET_ACTIVE_WINDOW` request to the WM, so no in-process fix exists). The job now installs `openbox` and runs that one demo -- scoped, not every windowed demo, since a WM also decorates, places, and may resize windows the others were never rehearsed with -- inside its own Xvfb with `openbox --sm-disable` started first (`focusNew` is its default). Rehearsing that runner caught a third, self-inflicted bug before it ever reached CI: the first version spliced the script path into an `eval`'d `sh -c` string via `$0`, which the *outer* shell expanded first, so CI would have handed Python the step script itself. Replaced by a plain shell function that passes the path as the inner shell's positional argument; with a no-op standing in for `openbox` (not installed here), the demo now runs all the way to its focus assertion, which is exactly the one thing only a real WM changes. Final local tally on lavapipe under Xvfb after the fixes: 28 `PASS`, 1 `SKIP` (by design), 1 `FAIL` (the focus demo, for want of a local WM).

**Openbox rehearsal, once the project owner installed that too.** The focus demo passes under the real WM (`WindowFocused` observed `[False, True]`: mapped unfocused, then focused by openbox), five times out of five in isolation and in a three-demo tail -- and then failed once in the full 30-demo loop. A real race, reproduced under real load rather than explained away: after 28 software-rendered demos on llvmpipe the fixed `sleep 1` no longer guaranteed openbox was up before the window mapped, and nothing then ever granted it focus, while the demo itself only polled for about one second (60 x 1/60 s). Two fixes, neither weakening what is asserted: the runner now starts `openbox --startup "touch $ready"` and waits (up to 10 s, polling) for that file -- the WM signals its own readiness -- and the demo polls for up to five seconds but exits the instant a `WindowFocused(True)` arrives. Full loop re-run under exactly CI's conditions (fresh venv with the pip-installed wheel, Xvfb, lavapipe, openbox for the focus demo, per-demo timeouts): **29 `PASS`, 1 `SKIP`, 0 `FAIL`.**

**First real run (2026-09-14, run 34802658042): seven of eight jobs green -- including the new `msrv` job at 1.88 with `--locked` -- and the `python` job red at exactly the demo step, with everything before it (wheel build, `pip install`, verifier build, GSettings key) green.** 7 `PASS`, 1 `SKIP`, 22 `FAIL`, in two distinct shapes neither local rehearsal could have shown: every Vulkan demo died in ~0.1 s with winit's `XKBNotFound` (libxkbcommon failed to load at runtime, from inside `HeadlessRenderer.__init__`), and both tray demos segfaulted (exit 139) right after GLib's `cannot register existing type 'GdkDisplayManager'` -- the signature of two GTK/GObject copies in one process. Both are Python-process-only: the neighbouring Rust example jobs load xkbcommon and initialize GTK on the identical package list and Xvfb and pass. The one thing the Python process had that they did not: `actions/setup-python` exports `LD_LIBRARY_PATH=/opt/hostedtoolcache/Python/3.12.14/x64/lib` into every later step (confirmed in the job log). The job now uses Ubuntu 24.04's own Python 3.12 in a venv (no toolcache, no inherited library path; `maturin build -i python` targets it explicitly), which is also what `docs/getting-started.md` tells a Linux user to do -- and gains a read-only "environment evidence" step before the demos (interpreter, any library-path override, where the loader resolves xkbcommon/GTK/GObject, what the extension links, whether Xvfb advertises XKEYBOARD) so that if anything still fails, the next log answers it rather than a theory.

**Status: fixed locally; second real run pending.** Every part of the job has now been rehearsed locally under the runners' own conditions, and every demo the job runs passes. The one remaining unknown -- the two tray demos with no `StatusNotifierWatcher` host at all (this desktop's own is reachable through the live session bus even under Xvfb) -- is named here rather than assumed, and the first real run will settle it.

## Finding #255 Fixed (Explicit Go-Ahead): The Three `VK_KHR_dynamic_rendering_local_read` RHI Methods Renamed to Vendor-Neutral Names

Finding #247 gave these three `RhiDevice`/`RhiSwapchain`/`RhiCommandBuffer` methods default bodies but left the vendor-neutral rename it flagged (item (c)) to the project owner, who has now directed it ("use your own judgement ... the best Rust render engine for supporting GUI frameworks"). A backend-agnostic trait in a `#![forbid(unsafe_code)]`, `ash`-free core crate should not name one backend's extension in its own public API -- D3D12's analogue is ROV/programmable-blend framebuffer fetch, Metal's is `[[color(n)]]`, and both, like Vulkan's `VK_KHR_dynamic_rendering_local_read`, are instances of the one hardware capability *framebuffer fetch*.

Renamed across the whole workspace and its docs: `supports_local_read_input_attachment` -> `supports_framebuffer_fetch`, `local_read_blend_supported` -> `framebuffer_fetch_blend_supported`, `insert_blend_read_barrier` -> `insert_framebuffer_fetch_barrier`. The Vulkan backend keeps its own internal, `vk`-specific field and helper names (`swapchain.rs`'s `supports_local_read_input_attachment` capability field, `device.rs`'s `local_read_supported`) -- those describe the actual Vulkan extension being queried, correctly, one layer below the trait. Every implementor (`VulkanDevice`, `VulkanSwapchain`, `VulkanCommandBuffer`, `HeadlessSwapchain`), both `FakeDevice`/`FakeSwapchain` test mocks and their `AtomicBool` fields, the two example gate checks (`blend_mode_demo`, `shape_registry_zero_alloc_demo`), `ShapeRegistry::flatten_into`'s dispatch, `ARCHITECTURE.md` Section 6/8, and `docs/rust-api/rhi-and-backends.md` were updated in one pass; the doc prose still explains, in each method's paragraph, that Vulkan implements it via `VK_KHR_dynamic_rendering_local_read`, so the mapping is documented without the trait name asserting it. Two `.expect(...)` panic-message strings in the Vulkan backend that quoted the old names were updated to match. Verified: `cargo fmt --all -- --check`, `cargo clippy --workspace --all-targets -- -D warnings`, and the full suite (387/387) all clean.

**Status: fixed.** Finding #247's item (c) is closed.

## Finding #256 Fixed (Explicit Go-Ahead): Every GitHub Action Pinned to a Commit SHA

Finding #247's Security lens flagged (item (h)) that both workflows referenced third-party actions by a floating major tag; `docs.yml` in particular runs with `id-token: write`/`pages: write`, so a repointed tag there is a real supply-chain exposure. At the project owner's direction, every `uses:` in `.github/workflows/{ci,docs}.yml` is now pinned to a full 40-character commit SHA with the exact release it resolved to noted in a trailing comment, matching this workspace's own exact-pinning discipline for the `winit` fork `rev` and its `=`-pinned crates: `actions/checkout` v4.4.0, `Swatinem/rust-cache` v2.9.2, `actions/setup-python` v5.6.0, `actions/upload-pages-artifact` v3.0.1, `actions/deploy-pages` v4.0.5 (19 references total, resolved via the GitHub API at pin time). A leading comment in `ci.yml` records the rationale and that a bump means editing the SHA and its version note together. This is behavior-preserving -- each SHA is the current head of the tag it replaced -- and only removes the ability of an upstream tag move to change what CI runs.

**Status: fixed.** Finding #247's item (h) is closed.

## Finding #254 continued: First Two Real CI Runs of the `python` Job -- Diagnosed, Not Yet Green

Pushed (finding #254's job, plus #255/#256) and watched. Seven of the eight CI jobs pass on the first real run, including the new `msrv` job building the whole workspace at 1.88 with `--locked` and every existing Rust job. The `python` job fails at exactly the demo step -- wheel build, `pip install`, verifier build, and the accessibility-enable step all green before it -- with 22 of 30 demos failing in two shapes: every demo that constructs a Vulkan device dies in ~0.1 s with winit's `XKBNotFound` (an X11 xkb-extension setup failure at `winit .../x11/mod.rs:282`, from the 1x1 probe window `tre-platform` opens even for `HeadlessRenderer`), and both tray demos segfault (exit 139) after GLib's `cannot register existing type 'GdkDisplayManager'`.

Both are specific to the Python process: the neighbouring `vulkan-validation` job takes the identical winit X11 path (`walking_skeleton`, `headless`) on the same runner image, with the same `libxcb-xkb1`/`libxkbcommon-x11-0` (confirmed installed identically in both jobs' logs), and passes; and the same demos pass locally under Xvfb with lavapipe. The first fix attempt -- switching from `actions/setup-python` to Ubuntu's own Python in a venv, on the theory that setup-python's exported `LD_LIBRARY_PATH` was the cause -- did not change the result (the second run's evidence step shows `LD_LIBRARY_PATH` unset and `libxkbcommon` resolvable, yet the identical 22 failures), which rules that theory out. That first evidence step also inspected the package `__init__.py` rather than the compiled `.so`, so it proved nothing. The cause is therefore something in the Python process's runtime linking or X connection that a plain Rust binary does not reproduce, and it is not locally reproducible (the same code passes under Xvfb here).

Rather than guess across ~11-minute CI cycles, the evidence step was rebuilt to be decisive: it resolves the real extension module, `ldd`s it, reproduces winit's own two `xkbcommon` dlopens from inside the exact interpreter via `ctypes`, and runs one GPU demo under `LD_DEBUG=libs` filtered to xkbcommon resolution -- so the next run's log distinguishes a failed dlopen (a search-path/duplicate-library problem) from a successful dlopen with a failed XKB-extension negotiation (an X-connection problem), and the real fix follows from which it is.

**Root cause found (third run's `LD_DEBUG` trace) and fixed.** The instrumented run was decisive: the `ctypes` probe showed both `libxkbcommon.so.0` and `libxkbcommon-x11.so.0` dlopen fine, so it was never a search-path failure -- and the `LD_DEBUG=libs` trace of a GPU demo showed why anyway: the wheel maturin built on the runner was a *repaired manylinux wheel*, and auditwheel had vendored the entire GUI/X stack into `tre_python.libs/` with mangled sonames -- `libgdk-3-0b52fb81.so.0`, `libgtk-3-0611fd19.so.0`, `libgdk_pixbuf`, `libcairo-gobject`, `libfontconfig`, and `libxkbcommon-3eb6c731.so.0`. So the demo process held two copies of each library: the bundled one the extension links via `DT_NEEDED`, and the system one winit's `xkbcommon-dl` dlopens (and GTK's own initializer uses). Two `libxkbcommon` copies is exactly what makes winit's X11 xkb-extension setup return `XKBNotFound`; two `libgdk-3` copies is exactly what makes GLib report `cannot register existing type 'GdkDisplayManager'` and then segfault the tray demos. One mechanism, both symptoms -- and it appeared only in the wheel process (the Rust example jobs link system libs directly; a local `maturin develop` and a plain-tag build vendor nothing), which is why neither local rehearsal nor the neighbouring jobs ever showed it. The fix: build the wheel with `--auditwheel skip --compatibility linux`, a plain linux-tagged wheel that links the system libraries directly. Verified locally: the resulting wheel contains only its own extension `.so`, no `tre_python.libs/` at all. This is correct precisely because the wheel is built and run on the same runner -- a self-contained, portable manylinux wheel is exactly the wrong thing for a GUI-linked extension used in place.

**Status: fixed, final CI run pending.** The vendoring is disabled; every other part of the job passed the three real runs (wheel build, install, verifier build, accessibility enable) and every demo passes locally under the runners' own Xvfb/lavapipe/openbox conditions, so the plain-tag wheel is expected to close the last gap. Findings #255 and #256 are complete and independent of this.

## Finding #257: Third Real Run -- Vendoring Fix Confirmed (22 Failures -> 2), Both Survivors Addressed

The `--auditwheel skip --compatibility linux` build (finding #254) worked exactly as diagnosed: the third real run's `python` job went from 22 demo failures to 2, with `XKBNotFound` gone from every Vulkan demo, both tray demos passing (no more `GdkDisplayManager` double-registration segfault), and the focus and accessibility demos green. Two unrelated survivors remained, neither about the wheel:

**`phase12_step12_6` (render_parallel) -- a demo bug, fixed by exposing the cap.** It hard-coded 8 registries; `render_parallel` correctly rejects a batch larger than this machine's own concurrency cap (`available_parallelism() - 1`), which is 3 on the 4-core runner, so it raised `ValueError`. A many-core dev machine hid it. The guard is right; the demo shouldn't guess. Fixed at the source: a new `tre.max_parallel_registries()` free function returns exactly `RenderingCanvas::new().max_sub_canvases()` -- the same value the methods check against -- so a caller fanning work across registries (a real GUI-framework batching pattern) sizes its batch to the engine's own truth instead of catching an error or duplicating the core-count logic in Python. The demo now uses `min(8, tre.max_parallel_registries())`. Verified locally (cap 23 here, demo keeps 8, 6.2x speedup, passes).

**`phase12_step12_4` (gradient + texture fill) -- a native segfault, instrumented.** It crashes before its first output, only on the runner's Mesa/lavapipe, while passing on this machine's newer lavapipe and its hardware driver -- consistent with a software-driver bug on the gradient-or-texture path, but not yet proven, and not locally reproducible. Rather than guess, the demo loop now runs with `PYTHONFAULTHANDLER=1` and `RUST_BACKTRACE=1`, so the next run's log carries the actual crashing frame (Python C-traceback plus any Rust panic backtrace) instead of a bare "Segmentation fault", and the real fix -- an engine guard, a driver-capability check, or a documented skip like `phase14_step14_3` -- follows from what it names.

**Status: in progress.** The wheel-vendoring root cause is fixed and confirmed on real CI; `phase12_step12_6` is fixed; `phase12_step12_4` is instrumented for a decisive next run. Seven of eight jobs remain green.

## Finding #258 Fixed (Explicit Go-Ahead): Texture Teardown Use-After-Free Across the Python Boundary

The fourth real CI run (finding #257's `PYTHONFAULTHANDLER=1`) turned `phase12_step12_4`'s bare "Segmentation fault" into a real diagnosis: the demo printed its own `PASSED` -- every gradient/texture assertion correct -- and then aborted during process teardown with `malloc(): unsorted double linked list corrupted`, `Fatal Python error: Aborted`, `<no Python frame>`. A heap corruption in C during interpreter shutdown, i.e. while destroying GPU resources, not in the demo's logic.

**Root cause.** `VulkanTexture` frees its GPU image in `Drop` through a *cloned* `ash::Device` handle. An `ash::Device` clone copies the raw `VkDevice` handle and function pointers; it does not keep the underlying device alive. `PyHeadlessRenderer`/`PyWindowedRenderer` carefully order their own fields so the device drops last -- but a `Texture` handed out to Python, or one a `ShapeRegistry` still holds, is a *separate* Python object, and CPython finalizes objects in an unspecified order at shutdown. When the renderer (and its `VulkanDevice`, whose own `Drop` calls `vkDestroyDevice`) finalizes before a surviving texture, that texture's `Drop` then calls `vkDestroyImage`/`vkFreeMemory` on a destroyed device -- a use-after-free. A real GPU driver frees GPU-side memory and usually tolerates it silently; a software driver (lavapipe, which CI uses and a real GPU dev machine does not) allocates through the C heap, so the stale free corrupts `malloc`'s structures visibly. That is exactly why it appeared only on CI and only for the one demo that creates a texture, and why three prior "same code passes locally" observations were all true.

**Fix.** The keep-alive must travel *with* the shared texture value, so it protects both a loose `Texture` and a registry-held one. `crates/tre-python/src/texture.rs` gains a `SharedTexture { _texture: Box<dyn RhiTexture>, _device: Arc<dyn RhiDevice> }`, and `PyTexture.texture`/`ShapeRegistry`'s `ResolvedTexture` are now `Arc<SharedTexture>` instead of `Arc<Box<dyn RhiTexture>>`. Holding an `Arc<dyn RhiDevice>` guarantees the `VulkanDevice` -- and its `vkDestroyDevice` -- cannot run until every texture referencing it is gone, whatever order Python finalizes in; the field order (`_texture` before `_device`) frees the image while the device is still alive. Both Python renderers now store their device in an `Arc` (`Arc<dyn RhiDevice>` for headless, `Arc<VulkanDevice>` for windowed, which coerces to `Arc<dyn RhiDevice>` for the texture's keep-alive) and pass a clone into every `PyTexture::new`; the trait objects made this a small change, and every existing device use site (`&self.device` for the concrete-device `WindowSlot` calls, `&*self.device` for the `&dyn RhiDevice` ones) still resolves, so no rendering path changed. The texture Arc was only ever kept for lifetime (shapes reference textures by bindless index, never by `&dyn RhiTexture` at flatten), so bundling the device in cost nothing at the use sites.

Verified: `cargo fmt`/`clippy -D warnings`/`cargo test --workspace` (387/387) all clean, and the rebuilt extension runs `phase12_step12_4`/`_6`/`_8`/`_9` to a clean exit locally. The teardown crash itself is only reproducible on a software driver, which this machine no longer has as its default, so the CI run is the true confirmation.

**Flagged, not fixed (follow-up):** the same latent hazard exists one layer down, in `tre-rhi-vulkan` itself (any holder of a `VulkanTexture` that outlives the `VulkanDevice`), and in `tre-ffi`'s C-ABI texture handles, which this Python-boundary fix does not cover. The robust general fix is to give `VulkanDevice` an `Arc`-shared inner that every resource clones, so `vkDestroyDevice` is refcount-ordered at the RHI level -- a larger change worth its own pass.

**Status: fixed, confirmed on real CI.** The fifth real run (34808777077) is fully green -- all eight jobs, the `python` job included: 29 demos pass, 1 skipped by design (`phase14_step14_3`, needs a live XDG portal), 0 fail. CI now builds the `tre_python` wheel and runs every demo on every commit, which is what finding #254 set out to do. All five owner-directed dependency bumps (#249-#253), the vendor-neutral rename (#255), the action SHA-pins (#256), the `max_parallel_registries` accessor (#257), and this texture-teardown fix (#258) are complete and green. The RHI-level and `tre-ffi` generalization of the teardown fix remains flagged for a future pass.

## Finding #259 Fixed (Explicit Go-Ahead): RHI-Level Teardown -- `vkDestroyDevice`/`vkDestroyInstance` Are Now Refcount-Ordered

At the project owner's direction, this generalizes finding #258's Python-boundary texture-teardown fix down into `tre-rhi-vulkan` itself, so every consumer -- the C ABI, any future binding, and internal code -- is protected, not just PyO3. `VulkanTexture` frees its GPU image in `Drop` through a cloned `ash::Device` handle; that handle dangles the instant `vkDestroyDevice` runs, so any `VulkanTexture` that outlives its `VulkanDevice` is a use-after-free.

**The fix.** A new `Arc<DeviceOwner>` owns the objects whose destruction must be deferred until nothing references the device: the `VkDevice`, the `VkInstance` (its parent), the ash loader `entry` (which owns `libvulkan`), and the debug messenger. `DeviceOwner::Drop` performs `vkDestroyDevice`, then the messenger, then `vkDestroyInstance`, in that spec-required order. `VulkanDevice` holds an `Arc<DeviceOwner>` (plus cheap `ash::Device`/`ash::Instance` clones for its own calls), and every `VulkanTexture` holds a clone. `VulkanDevice::drop` still eagerly destroys all device-level *children* (pools, samplers, fences, buffers, the bindless set) -- it runs while the owner keeps the device alive -- but no longer destroys the device or instance itself; that waits for the last `Arc<DeviceOwner>`. So a texture handed out to Python or C, or held by a registry, keeps the real `VkDevice` alive until it drops, and its image is always freed before `vkDestroyDevice`.

**A real ordering trap, caught by a deterministic example, not by CI.** The first cut moved `device`/`instance`/messenger into `DeviceOwner` but left `entry` as a `VulkanDevice` field (to avoid touching `create_surface`, which uses `self.entry`). That segfaulted *every* teardown -- even the plain `headless` example -- inside `vkDestroyDevice`. Cause, from a `gdb` backtrace: `ash::Entry` owns the dlopen'd `libvulkan`, and dropping it unloads the library; as a `VulkanDevice` field it dropped *before* the deferred `owner`, so `libvulkan` was unmapped before `DeviceOwner::Drop` called `destroy_device` through function pointers living inside it. `entry` therefore had to move into `DeviceOwner` too, declared after `device`/`instance` so it drops only after the destroy calls; `create_surface` now reads `self.owner.entry`. This is exactly the class of bug that only shows at teardown and often only on some drivers -- so it got a deterministic regression example rather than being left to chance.

**`crates/tre-rhi-vulkan/examples/texture_teardown_check.rs`** reproduces the exact hazard: create a device, `create_texture`, then `drop(device)` *before* `drop(texture)`. It is an `example`, not a `#[test]`, because it must build a `winit` event loop, which panics off the main thread (`cargo test` runs bodies on worker threads); the `vulkan-validation` CI job now runs it on the same software driver where the corruption first appeared. Locally it passes on both the hardware driver and lavapipe, including under `MALLOC_CHECK_=3` (glibc's invalid-free trap); before the `entry` fix it segfaulted deterministically, and before the whole change the old drop order was the use-after-free itself.

**Verification.** `cargo fmt`/`clippy --workspace --all-targets -D warnings`/`cargo test --workspace` (387/387) clean; the `headless` and `texture_teardown_check` examples run clean on hardware and lavapipe (the latter under `MALLOC_CHECK_=3`); the rebuilt Python extension's texture demos still pass. The `tre-python` `SharedTexture` device keep-alive from #258 is now redundant with this RHI-level guarantee but left in place -- it is correct, costs nothing, and documents the boundary; removing it can be a later tidy-up.

**Status: fixed.** The teardown use-after-free is closed at the RHI level for every binding, and CI exercises it on the software driver.

## Finding #260 Fixed (Explicit Go-Ahead): Custom-Shader Robustness -- `robustBufferAccess` + `robustImageAccess` Requested at Device Creation

Closes finding #247's item (b) (the Security lens's High-severity item), by the mitigation the project owner chose: request GPU-side robustness so a caller-supplied custom fragment shader (`create_custom_pipeline`, exposed to Python as `create_custom_shader`) that indexes the engine's shared bindless resources out of range gets a *defined* result -- zeroed reads -- instead of undefined behavior (which the review noted could range from sampling another object's texture to a driver crash).

**What was enabled.** `VulkanDevice::new` now requests, in `VkDeviceCreateInfo`:
- **`robustBufferAccess`** (core `VkPhysicalDeviceFeatures`, via `enabled_features`) -- defines out-of-bounds *buffer* accesses (storage/uniform/texel buffers, vertex/index fetch), the buffer half of a custom shader's attack surface. Requested only after querying `get_physical_device_features` reports support (universally true in practice, but checked rather than assumed, so device creation can't fail on a driver that somehow lacks it).
- **`robustImageAccess`** (`VK_EXT_image_robustness`'s `VkPhysicalDeviceImageRobustnessFeatures`) -- defines out-of-bounds *sampled-image* accesses, which is exactly the bindless texture-array case the review flagged (`bindless_textures[nonuniformEXT(huge)]`). Enabled only when the device advertises the extension, mirroring the existing `local_read` conditional (requesting a feature for an unadvertised extension is invalid). Verified present on this project's real drivers -- RADV (hardware), lavapipe (CI's software driver), both report `VK_EXT_image_robustness` + `robustImageAccess = true` -- so on every driver this project actually runs on, the reported out-of-range bindless-texture read now returns defined zeros.

**Coverage and the honest residual.** On a device that advertises `VK_EXT_image_robustness` (RADV, lavapipe, and thus CI), both halves are on and the custom-shader OOB surface -- buffers and the sampled-image bindless array -- is fully defined. On a hypothetical device without the extension, buffer robustness still applies but the sampled-image OOB stays driver-dependent; there, mitigation (b) (a restricted descriptor-set layout that never exposes the bindless array to custom pipelines) or (c) (SPIR-V reflection rejecting stray bindings) would be the hard guarantee. This is defense-in-depth chosen for its simplicity and breadth; it is not a substitute for not linking untrusted shaders against the universal layout, but it removes the undefined-behavior teeth from the exposure on real hardware.

**Cost.** Robustness adds GPU-side bounds checks on the affected accesses -- a small, driver-dependent runtime cost the owner accepted in choosing this mitigation. It is a device-feature flag, so it adds no allocation and does not touch the CPU frame-processing path the CI performance gate measures.

**Verification.** `cargo fmt`/`clippy --workspace --all-targets -D warnings`/`cargo test --workspace` (387/387) clean; `VulkanDevice::new` creates a device successfully with both features on RADV and lavapipe (a driver that rejected a requested feature would fail `vkCreateDevice`); the rebuilt Python extension's custom-shader demo (`demo/phase13_step13_8`) passes on both drivers. A dedicated example that renders an intentionally out-of-range bindless read and asserts it comes back as defined zeros would strengthen this further and is noted as a possible follow-up; it was not added here to avoid hard-coding the private descriptor-layout binding numbers into a demo.

**Status: fixed.** Finding #247's item (b) is closed on every driver this project runs on; the residual for extension-less devices is documented.

## Finding #261 Fixed (Explicit Go-Ahead): 42 Self-Asserting Vulkan Examples Promoted to `cargo test`

Closes finding #247's item (d). `tre-rhi-vulkan`'s only correctness gate beyond the ten device-free unit tests (finding #247) was ~40 `examples/` binaries CI ran individually via `cargo run --example`; `cargo test -p tre-rhi-vulkan` could never execute them, and a contributor had to know to run each one by name.

**The real blocker, and why it's a `[[test]] harness = false` target, not a `#[test]` function.** Every one of these examples calls `VulkanDevice::new`, which builds a probe window through `tre_platform::PlatformConnection`, which builds a real `winit` `EventLoop` -- and winit permits constructing an `EventLoop` only on a process's genuine main thread, and only once per process, ever (confirmed directly: a first attempt using a plain `#[test]` function panicked immediately, "Initializing the event loop outside of the main thread is a significant cross-platform compatibility hazard," because libtest runs every `#[test]` fn on a worker thread, never the main one). A `[[test]]` target with `harness = false` sidesteps this cleanly: cargo still compiles and runs it as part of `cargo test`, but as its own separate process with a real main thread -- exactly what an `examples/` binary already was, just discovered and invoked through `cargo test` instead of `cargo run --example`.

**The conversion, done uniformly across all 42 files.** Every file shares the identical three-call bootstrap (`PlatformConnection::new()` -> `.create_window(...)` -> `VulkanDevice::new(...)`), confirmed by grep to occur exactly once each in every file (including the four windowed demos -- `walking_skeleton`, `multi_window`, `input_demo`, `main_loop_demo` -- which reuse a window-opening helper function defined earlier in the file, so the real first-call-in-execution-order was located by scanning forward from each anchor rather than by file position). All three `.expect("...")` calls were converted to `.unwrap_or_else(|e| { eprintln!("...{e:?}...-- skipping"); std::process::exit(0) })`, matching `tre-a11y/tests/round_trip.rs`'s own graceful-skip-when-no-device convention: a contributor with no display server or Vulkan-capable GPU sees `cargo test` skip these cleanly (exit 0) instead of panicking, while every real assertion *after* device creation succeeds stays a hard failure exactly as before -- this generic transform never touched anything past the third call. `examples/support/pixel_helpers.rs` (a shared pixel-readback helper 33 of the 42 include via `#[path = "support/..."]`, resolved relative to each including file's own directory) was copied, not moved, to `tests/support/pixel_helpers.rs`, since `canvas_accessibility_demo.rs` needs its own copy and stays in `examples/` (see below).

**What deliberately stayed put, and why.** The accessibility trio (`canvas_accessibility_demo.rs`, `canvas_accessibility_verify.rs`, `canvas_accessibility_single_process_check_verify.rs`) was NOT converted. `canvas_accessibility_demo` writes a file `canvas_accessibility_verify` later reads, with a real, required GSettings/dconf shell step from CI's own `accessibility-validation` job sandwiched strictly between them -- a genuine cross-process, cross-CI-step ordering dependency that `cargo test`'s default unordered (and potentially parallel) execution of `[[test]]` targets cannot safely preserve. Converting these would risk `verify` racing ahead of `demo`, or either running before the accessibility bus is enabled. These three stay exactly as they were: `examples/` binaries, invoked by name from specific, ordered `ci.yml` steps.

**CI.** `vulkan-validation`'s 42 individual `- run: xvfb-run -a cargo run -p tre-rhi-vulkan --example X` lines collapsed into one `- run: xvfb-run -a cargo test -p tre-rhi-vulkan`, carrying forward the three env-var frame-count overrides (`TRE_WALKING_SKELETON_FRAMES`/`TRE_MULTI_WINDOW_FRAMES`/`TRE_INPUT_DEMO_FRAMES`, all set unconditionally on the one step -- each test binary reads only its own name). The per-example explanatory comments that used to precede each `ci.yml` step were not preserved there (they would have had nothing left to sit next to) but were never unique content: every one restated what that example's own top-of-file doc comment already says in equal or greater detail, and those doc comments moved unchanged with their files into `tests/`. `documentation/TECHNICAL.md`'s Section 3.4 status note, which named the now-defunct exact `cargo run -p tre-rhi-vulkan --example main_loop_demo` invocation, was corrected.

**Verification.** `cargo build -p tre-rhi-vulkan --tests`, `cargo clippy --workspace --all-targets -- -D warnings`, and `cargo fmt --all -- --check` all clean. `cargo test -p tre-rhi-vulkan` runs all 42 promoted targets plus the existing 10 unit tests in one command, exit 0, on both the hardware driver and lavapipe under Xvfb (CI's own driver) -- every demo's own real assertions (exact pixel readbacks, byte-for-byte batching-equivalence comparisons, zero-allocation frame guards, the Phase 9 spring-decay/zero-alloc main-loop check, `texture_teardown_check`'s finding-#259 regression) still ran and passed identically to their pre-conversion `cargo run --example` output. `cargo test --workspace` remains 387/387 (unchanged -- these 42 are harness=false binaries, not `#[test]` functions, so they don't add to that count; they add 42 lines of their own to `cargo test`'s own "Running tests/X.rs" output).

**Status: fixed.** Finding #247's item (d) is closed for the 42 examples with no real ordering dependency; the accessibility trio's own conversion (if ever wanted) would need a different mechanism -- e.g. folding the whole three-step sequence into one `[[test]]` binary that does the demo, the GSettings write via `dconf`/`zbus` directly (no shell-out), and the verify in one process -- and is left as a separate, more invasive follow-up, not attempted here.
