...
| # | Finding | Doc(s)/Code | Severity | Resolution |
|---|---|---|---|---|
| 126 | `org.a11y.Status.IsEnabled`'s real mechanism, in the actual installed `at-spi2-core` 2.52.0 (not the `main`-branch source the ninth/tenth/eleventh corrections were reading): a plain `readwrite` property, initialized once at startup directly from the `org.gnome.desktop.interface` `toolkit-accessibility` GSettings key, with no `RegisterEvent`/`EventListenerRegistered` mechanism compiled into this version at all | tre-a11y, tre-rhi-vulkan (example code), .github/workflows/ci.yml | Should-fix | Open — real mechanism confirmed via source, and the direct fix (`dconf write toolkit-accessibility true`, confirmed to genuinely take via an independent `dconf read` in the same session) was tried for real, but a fresh session's `canvas_accessibility_verify` still saw `IsEnabled` false -- three live explanations (write doesn't persist to a later fresh session; something else blocks `IsEnabled` regardless; or our own `.unwrap_or(false)` masks a real D-Bus error) not yet disambiguated; next diagnostic queries both values, independent of our own code, inside the verify step's own session |
