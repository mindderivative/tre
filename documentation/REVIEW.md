...
| # | Finding | Doc(s)/Code | Severity | Resolution |
|---|---|---|---|---|
| 126 | `org.a11y.Status.IsEnabled`'s real mechanism, in the actual installed `at-spi2-core` 2.52.0 (not the `main`-branch source the ninth/tenth/eleventh corrections were reading): a plain `readwrite` property, initialized once at startup directly from the `org.gnome.desktop.interface` `toolkit-accessibility` GSettings key, with no `RegisterEvent`/`EventListenerRegistered` mechanism compiled into this version at all | tre-a11y, tre-rhi-vulkan (example code), .github/workflows/ci.yml | Should-fix | Open — `G_DBUS_DEBUG` traffic proved the a11y-bus self-connect from corrections nine-eleven is never even called in the installed 2.52.0 binary; diffing that exact tag's source against `main` confirms the whole `EventListenerRegistered` mechanism doesn't exist in it. Real mechanism identified (GSettings-backed `IsEnabled`, settable via plain `Properties.Set`); next diagnostic (not yet built) queries the real current property/GSettings values directly before proposing a fix |
