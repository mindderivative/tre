...
Verified by `cargo fmt`/`clippy -D warnings`/`test` clean across the workspace (11 new `tre-text` unit tests, all against real installed fonts rather than synthetic data) and `text_shaping_demo`'s three live proofs: bidi+script shaping of a real mixed Latin/Hebrew string (Hebrew run's glyphs confirmed in visually-reversed cluster order), real fallback resolution of an emoji codepoint absent from the primary font, and a real glyph's ('L') outline extracted, flattened via `tre-svg`'s now-`pub` flatten functions, and rendered through the unmodified pre-existing pipeline -- matched exactly against an independently-computed (in-demo) point-in-polygon check before the GPU render was trusted. All 11 pre-existing examples re-run manually, zero validation errors, confirming the new dependency graph disturbed nothing already built.
