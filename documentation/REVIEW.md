...
| # | Finding | Doc(s)/Code | Severity | Resolution |
|---|---|---|---|---|
| 126 | `accesskit_unix`'s own background thread cannot complete real AT-SPI2 registration inside a process that also links real Vulkan/X11 shared libraries, regardless of timeout length -- proven only after ordering, wrapper nesting, CPU contention, `IsEnabled`, and a 60s timeout were each tried in turn and directly disproved by real evidence | .github/workflows/ci.yml, tre-rhi-vulkan (example code) | Should-fix | Fixed — real two-process split: `canvas_accessibility_demo` (Vulkan/X11) renders and tags only; a new, genuinely Vulkan-free `canvas_accessibility_verify` binary does the real AT-SPI2 publish and verify |
