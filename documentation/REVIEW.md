...
**Verified.** Full workspace `cargo fmt --all -- --check`/`cargo clippy --workspace --all-targets -- -D warnings`/`cargo build --workspace --all-targets`/`cargo test --workspace` clean in debug and `--release` (unchanged pass/fail counts). Rebuilt the `tre_python` extension (`maturin develop --release`) and re-ran the Phase 12 Step 12.1 windowed-renderer demo -- still passes unchanged (120 real frames, a real `Resized` event observed, a real second-window multi-window sequence). This regression check does not, on its own, force the real `SwapchainOutOfDate` recovery branch this fix touches -- doing that needs a live, human-driven window resize during a real run, the identical tooling gap (no `xdotool`/`wmctrl`) already disclosed for `tre-perf-suite`'s own resize test; the fix itself is the same structural change already proven correct there under a real, extended interactive resize session.
