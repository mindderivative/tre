...
**Status: fixed, pending the first real run.** Every part of the job that can be verified from this machine has been; the X11-under-Xvfb path for the five windowed Python demos and the tray demos' behavior with no `StatusNotifierWatcher` host present are the two things only the first CI run will settle, and both are named here rather than assumed.
