...
**Status: objectively improved and measured; the project owner's own subjective read on whether the visual "trails then jumps" symptom is actually resolved (as opposed to merely less severe) is still pending** -- matching this investigation's own established pattern (finding #234 needed the same kind of direct human confirmation that pixel-level measurement alone couldn't provide). Option 3 (a bounded-timeout acquire, the largest architectural change of the three) remains untried and is not needed unless Option 2's real-world feel still isn't good enough.
