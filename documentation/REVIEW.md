...
| # | Finding | Doc(s)/Code | Severity | Resolution |
|---|---|---|---|---|
| 126 | `org.a11y.Status.IsEnabled` only flips true when a real client calls `Registry.RegisterEvent` (confirmed via `at-spi-bus-launcher`'s own real source) -- a fresh CI container has nothing that ever does, so `accesskit_unix`'s own adapter never activates, in any process, regardless of Vulkan/X11 linking (a two-process split was tried and also disproven) | tre-a11y, tre-rhi-vulkan (example code) | Should-fix | Fixed — both the round-trip test and the verify binary now call `RegisterEvent` themselves and confirm `IsEnabled` before proceeding, matching real AT-SPI2 practice |
