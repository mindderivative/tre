...
**Status: unresolved, options 2 and 3 not yet started.** Two untried remedies remain, both larger than the disproven Option 1 and not yet begun pending the project owner's own direction on which to pursue next: **Option 2**, pinning the swapchain to a coarse/rate-limited size during an active drag and only settling to the real final size on release -- pyCopper's own proven fix for the identical symptom, and finding #231's writeup already flagged this exact tradeoff (a visibly coarser render during the drag itself) as a real, disclosed cost specific to this remedy. **Option 3**, giving `vkAcquireNextImageKHR` a bounded timeout instead of blocking indefinitely and skipping/dropping a frame's render when it doesn't return in time -- the largest architectural change of the three, changing the render loop's own blocking-acquire contract.

**Verified (for the investigation itself, not yet for a fix).** Full workspace `cargo fmt --all -- --check`/`cargo clippy --workspace --all-targets -- -D warnings`/`cargo build --workspace --all-targets` clean in debug and `--release`; `cargo test --workspace --no-fail-fast` clean (unchanged pass/fail counts -- this investigation touched no test-covered logic paths, only diagnostic instrumentation and one since-reverted experiment, both exercised solely via real interactive GPU runs above).
