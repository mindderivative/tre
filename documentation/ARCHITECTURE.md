...
*Implemented in the `tre-platform` crate -- Linux only (Wayland/X11) for now; Windows/macOS bridges are later steps. As of Phase 1 Step 1 (2026-09-05), `PlatformConnection` owns native multi-window creation (one shared connection per backend, TECHNICAL.md Section 9.2). As of Phase 1 Step 2 (2026-09-05), it also owns the real OS input/event pump: pointer and keyboard events are translated into `tre_engine::InputEvent`s and pushed through `tre_engine::InputEventQueue` (TECHNICAL.md Section 8), which coalesces high-frequency pointer motion before a caller drains it. As of Phase 11 Step 11.1 (2026-09-10), the two hand-rolled Wayland/X11 protocol integrations originally built for this layer were replaced by a single `winit`-backed implementation (IMPLEMENTATION.md's Step 11.1 write-up) -- `PlatformConnection`'s own public API is unchanged, so this is an internal implementation swap, not an architectural one.*
...
*Updated for the RHI trait-object generalization (2026-09-12, REVIEW.md finding #216):* `RhiDevice`/`RhiSwapchain` both gain a `Send + Sync` supertrait bound, matching `RhiBuffer`/`RhiTexture`/`RhiPipelineState`'s own identical, earlier bound (Phase 10 Step 10.4/Phase 12 Step 12.3) -- needed so `tre-python`'s renderer types could hold their own device/swapchain behind `Box<dyn RhiDevice>`/`Box<dyn RhiSwapchain>` instead of the concrete Vulkan types, closing a real Architecture-review finding (both renderers stored concrete `VulkanDevice`/`VulkanSwapchain` despite the architecture otherwise committing to backend-agnostic trait objects). Two concrete-only capabilities were blocking that generalization and moved onto the trait surface to close it for real, not just narrow it: `RhiDevice::create_custom_pipeline` (moved from a `VulkanDevice`-only inherent method, now taking this crate's own backend-agnostic `TextureFormat` instead of a raw Vulkan format code) and `RhiSwapchain::read_pixels_bgra8` (a new method; `VulkanSwapchain`'s own real windowed swapchain has no CPU-visible staging buffer to read back from, so it returns the new `EngineError::PixelReadbackUnsupported` -- only `HeadlessSwapchain`, built with readback in mind, supports it for real). `tre-python`'s `PyHeadlessRenderer` now stores both fields fully generalized; `PyWindowedRenderer`'s per-window `swapchain` generalizes the identical way, but its own top-level `device` field stays a concrete `VulkanDevice` by real, disclosed necessity -- `register_shape_pipelines`/`VulkanDevice::create_surface` are backend-selection/window-surface bootstrapping this renderer repeats throughout its lifetime (`create_window`, resize recovery), not a one-time construction step the way `PyHeadlessRenderer`'s identical calls are, and neither reasonably belongs on the `RhiDevice` trait itself. See REVIEW.md finding #216 for the full account, including why this partial result is the actual, complete generalization achievable here, not a narrowed one.

*Updated for the resize-jitter investigation (2026-09-13, REVIEW.md findings #235/#238; annotation itself added by a `/review-project` Architecture finding, 2026-09-13, since this section had not been extended for either):* `RhiSwapchain` gains `acquire_next_image_with_timeout(&self, timeout_ns: u64) -> Result<AcquiredImage, EngineError>`, and `RhiDevice` gains three new methods -- `begin_frame_with_timeout` (a bounded-acquire variant, finding #235's own Option 3 experiment, real and tested but not the strategy ultimately shipped), `begin_frame_with_logical_size` (projects content at a caller-given logical size while the swapchain's own physical extent may be coarser -- Option 2's original resize-drag fix), and `begin_frame_with_viewport_crop` (confines the GPU viewport/scissor/render area to a caller-given `crop_size` instead of the swapchain's full extent, letting content render 1:1 into a sub-rectangle of an oversized buffer -- the mechanism finding #238 ultimately shipped, paired with `tre_platform::PlatformConnection::set_viewport_source_crop` on Wayland). All three `RhiDevice` additions default-delegate to plain `begin_frame`, so no other backend needed changes. A new `EngineError::AcquireTimedOut` variant backs the bounded-acquire path. `begin_frame_with_logical_size` and the viewport-crop path are mutually exclusive strategies for the identical underlying mismatch (stretch-then-let-the-compositor-resample vs. crop-and-present-unscaled), never meant to be called for the same frame -- see finding #238's own account of what happened the one time they were combined by mistake. See REVIEW.md finding #237 for the real, protocol-traced root cause these methods exist to work around, and finding #239 for how a future DX12 backend's own `SetSourceSize` would implement the crop without needing any new trait surface at all.

*Updated for REVIEW.md finding #244 (2026-09-13; annotation itself added by the `/review-project` Architecture lens, finding #247, which caught that the paragraph above still described the pre-#244 surface):* `begin_frame_with_timeout` and `begin_frame_with_viewport_crop` no longer exist on `RhiDevice`. Finding #244 replaced both with one composable method, `begin_frame_with_options(&self, swapchain: &dyn RhiSwapchain, options: BeginFrameOptions) -> Result<AcquiredImage, EngineError>`, where `BeginFrameOptions { timeout_ns: Option<u64>, crop_size: Option<(u32, u32)> }` (both `None` by default, `Default`-derived, matching plain `begin_frame`'s behavior exactly) carries the two axes as independent, freely-combinable fields -- `VulkanDevice::begin_frame_impl` had always treated them as orthogonal internally; only the four single-axis public methods ever forced a caller to choose one. The bounded-acquire timeout and the viewport crop therefore *do* compose in a single frame now; the mutual exclusivity the paragraph above still correctly describes is between `begin_frame_with_logical_size` (stretch) and the `crop_size` axis (crop), not between timeout and crop. `begin_frame_with_options` follows the same default-delegates-to-`begin_frame` pattern every prior `begin_frame_with_*` addition used, so a backend that implements only `begin_frame` still compiles. `begin_frame_with_logical_size` was deliberately left as-is (see finding #244 for why). `submit_frame_with_viewport_crop`'s own signature and every real call site (`tre-python/src/windowed_renderer.rs`) were unchanged; only its body now routes through the new method.
