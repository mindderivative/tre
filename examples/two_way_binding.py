...
checked`/`two_way: thumb_position` on a `Checkbox`/`Slider`, each also
bound one-way via the ordinary `bindings:` map -- `View._attach` wires
both directions: the existing forward apply (`Signal` -> widget, proven
by every prior `View`-based test) and this phase's new reverse write-
back (widget -> `Signal`), registered as the widget's own real
`EventKind::Change` handler.
...
`View.click` already established. A real user toggling the checkbox is
simulated the only way `Change` is Python-reachable for a `Checkbox`
(`Node.set_checked` -- see `tests/test_change_event.py`); the real
*mechanical* Slider-drag-release `Change` path has no Python entry point
at all (`tests/test_slider.py`'s own note), so this script proves the
Slider's forward direction only -- the same "prove what's Python-
reachable" split every M14 test file already uses.
...
round trip back into the Checkbox's own bound `Signal` after its value
changes.
"""
...
print("two_way_binding.py: exited cleanly, both directions of the two-way binding proved")
