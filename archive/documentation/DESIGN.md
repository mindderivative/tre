...
The engine remains **language-agnostic** with respect to any *other* UI framework driving it: the `Canvas` API (Section 6) is exposed across a stable C-ABI boundary (`extern "C"` functions, `#[repr(C)]` types only -- see TECHNICAL.md Section 9.4 for the binding mechanism, built by the `tre-ffi` crate, IMPLEMENTATION.md Phase 10 Step 10.3), so any UI framework written in a language capable of calling a C ABI -- a custom C++ GUI toolkit, a modern desktop UI runtime -- can bind to it without modifying the engine core.
