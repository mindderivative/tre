...
/// monotonically growing/shrinking drag). Clamped to never round below
/// one whole `bucket` -- a real window is never legitimately smaller
/// than that in practice, and a zero-size swapchain is a real crash, not
