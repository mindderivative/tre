//! REVIEW.md finding #261 (2026-09-14): a duplicate of `../../examples/
//! support/pixel_helpers.rs`, not a move -- `canvas_accessibility_demo.rs`
//! stayed in `examples/` (it has a real cross-process/cross-CI-step
//! ordering dependency on `canvas_accessibility_verify.rs` that a
//! parallel, unordered `cargo test` run can't safely preserve) and still
//! needs its own copy, since `#[path = "support/..."]` resolves relative
//! to each including file's own directory. Keep both copies identical.
//!
