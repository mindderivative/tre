...
void main() {
    ...
    uint texture_index = style_buffer.words[style_index + 9];
    ...
    if (fill_kind == 1u) {
        ...
        // Maps frag_uv (center-relative, [-half_extent, half_extent])
        // onto the shape's own bounding box, [0, 1] -- the real texture
        // is already stored linear-space (no srgb_to_linear needed),
        // matching bindless_textured.frag's own established convention.
        vec2 tex_uv = (frag_uv / half_extent + vec2(1.0)) * 0.5;
        fill = texture(
