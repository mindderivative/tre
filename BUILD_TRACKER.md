...
| M4 — Real Input Dispatch (§4, §7.3, §9, §10, §11.10) | `██████████` 100% | ✅ Phases 1-2 complete — no further phase yet scoped |

**Just closed:** M4 Phase 2 — assistive-technology action dispatch (§10). `engine-core`: `Tree::activate(node)` (the direct, non-`InputEvent` counterpart to a mouse click's `Activated` outcome) and `Tree::set_focus_to(node, ...)` (the direct-target counterpart to `move_focus`'s tab-order computation — what `accesskit::Action::Focus` actually needs). Both build on a new, verified-reversible `from_access_id` (the inverse of the existing `to_access_id`, using `slotmap::KeyData`'s own documented `as_ffi`/`from_ffi` round-trip guarantee) — a stale accessibility-client-supplied id fails this `Tree`'s own generation check safely, the same generational-safety property §5 already relies on elsewhere. `engine-platform`: `run_windowed_multi` gained `on_access_action`, firing on a real `accesskit_winit::WindowEvent::ActionRequested` — closing the exact gap step 7's own comment named since early in this project. `engine-py`: `App::run()`'s new `on_access_action` closure routes `Action::Click`/`Action::Focus` through the *same* `dispatch::run_activation` a mouse click and `Window.click()` already use — one mechanism, three real entry points now. New `Window.press_key(key, shift=False)` closes Phase 1's other stated gap: Tab-then-Enter/Space activation is now directly testable from Python. 5 new `engine-core` unit tests + 6 new pytest tests, all passed on the first run; both real windowed tests and both example scripts still pass unchanged. No live AT-SPI/UIA/NSAccessibility client was available to drive a genuine end-to-end screen-reader proof in this environment — stated explicitly, not silently skipped; every layer that *can* be unit-tested without one is. See `PLAN.md`/`LOG.md`.

**Up next:** No further M4 phase is yet scoped in `ARCHITECTURE.md`. Real, larger, later-milestone-shaped gaps: §11.9 transform composition (`PaintProperties.transform`, blocking transform-aware hit-testing), a real scrollable viewport for `VirtualList` (§11.8), and `NodeKind::Canvas` (blocking custom hit-testing, §11.10's own second half).
