...
| M4 — Real Input Dispatch (§4, §7.3, §9, §10, §11.10) | `████░░░░░░` 44% | 🚧 4/9 phases complete |
| M5 — Transform Composition & Custom Drawing (§11.9, §11.10, §11.11) | `░░░░░░░░░░` 0% | ⬜ Scoped 2026-09-15, not started |

**Just closed:** M4 Phase 4 — `View`'s declarative `on_click` handlers (§16.2) now reach real dispatch. `View::_attach` used to only eagerly validate a declared handler (`getattr` + callable check) without ever registering it anywhere — confirmed via direct code reading, and stated as its own scope boundary in `view.rs`'s own doc comment since before M4 existed. Now, after validation, an `on_click` handler is wired into the same `click_handlers`/`Tree::dispatch` mechanism `Node.set_on_click` already uses (`Node::set_on_click` widened from module-private to `pub(crate)` for the second in-crate caller). New `View.click(node)` mirrors `Window.click` (M4 Phase 1) exactly — real dispatch, no live window needed — using `AvailableSpace::MaxContent` layout sizing since `View` has no window width/height of its own. Scope deliberately narrowed during implementation: `View` still gets no live render loop (a real, separate, much larger feature no confirmed gap actually needs yet), only `on_click` is wired (other event names still validate but reach no mechanism until Phase 6's `EventKind` exists), and handlers are called with zero arguments, matching the existing `Node.set_on_click` convention rather than manufacturing an `Event` object early. 4 new pytest tests in `test_view_handlers.py`, all passed on the first run; full suite (41 passed, 1 skipped) and all three examples unaffected. Also corrected a factual error in the previous tracker update along the way — see the Known Gaps entry on M4 Phase 5. See `PLAN.md`/`LOG.md`.

**Up next:** M4 Phase 5 (corrected scope, see below) — a Python-facing opt-in into `InteractionState` so a real button can actually ripple/hover; the `engine-core` dispatch mechanism underneath is already real and correct, `engine-py` just never calls it. Four more M4 phases (6-9) remain scoped below, plus the separate Milestone 5 for the larger rendering-capability gaps (§11.9 transform composition, blocking transform-aware hit-testing and `NodeKind::Canvas`) that are exposed by hit-testing but aren't themselves input-dispatch work.
