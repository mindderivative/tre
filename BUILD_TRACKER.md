...
| M4 — Real Input Dispatch (§4, §7.3, §9, §10, §11.10) | `█⬜⬜⬜⬜⬜⬜⬜⬜⬜` 10% | 🚧 In progress — Phase 1 step 1 of a sketched 3 done |

**Just closed:** M4 Phase 1 step 1 — the real `InputEvent`/`AppHandler`/hit-testing dispatch core, `engine-core` only. New `input.rs` (`InputEvent`/`PointerButton`/`Key`/`AppHandler`/`DispatchOutcome`) plus `Tree::hit_test`/`update_hover`/`move_focus`/`dispatch`. `hit_test` (§11.10) is a reverse paint-order rect-containment walk, explicitly not yet transform-aware (`PaintProperties.transform` doesn't exist) or `Canvas`-aware (`Canvas` doesn't exist). `update_hover` is the literal fulfillment of §7.3's "hover falls out of hit-testing" text, opt-in-only per Design Principle 6. `move_focus` wires §10's minimal Tab/Shift-Tab model for real, keying "interactive" off the already-existing non-empty `access.actions` signal. Found a real doc/API drift: §10 says Enter/Space dispatches `accesskit::Action::Default`, which doesn't exist in the pinned `accesskit = "0.25.0"` — the real equivalent, already used by M3 step 7's own button, is `Action::Click`. 6 new tests, each isolating one claim; `engine-core` now at 32 unit tests. See `PLAN.md`/`LOG.md`.

**Up next:** M4 Phase 1 steps 2-3 — real `winit` event translation in `engine-platform`, then `engine-py`'s real `AppHandler` impl, which is what finally lets `Node.set_on_click` exist for real.
...
- Real drag-to-rearrange docking and real splitter-drag input both still need a real `winit` pointer source wired through `Tree::dispatch` (M4 step 2, not built yet) — `Tree::set_splitter_position`/`Tree::apply_active_tab` are exposed as direct, programmatic APIs in the meantime, matching every other interaction-dependent step's own precedent.
...
- `App`/`Node` are still a narrower slice of §8's full `PyApp`/`PyWindow`/`PyNode` design than the finished framework needs: no `set_on_click` yet (M4 step 3's own job, now that a real `AppHandler`/`dispatch` core exists for it to call into) and no `add_child` from Python. `PyWindow`'s own real `PyObject`-callback-storage + `__traverse__`/`__clear__` GC precedent (M3 step 15 Stage C's materializer) is already there for step 3 to reuse, not invent fresh.
- §10's minimal keyboard focus model (Tab/Shift-Tab traversal, Enter/Space activation) is now real (`Tree::move_focus`/`Tree::dispatch`, M4 Phase 1 step 1) but not yet *reachable* from a real keyboard: nothing in `engine-platform` translates a `winit` `KeyboardInput` event into `InputEvent`/calls `dispatch` yet (M4 step 2), and `ActionRequested`/`AccessibilityDeactivated` events from accesskit are still received and ignored (M4 step 3's own territory, once `AppHandler` exists to route them to).
