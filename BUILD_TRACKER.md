...
| M4 — Real Input Dispatch (§4, §7.3, §9, §10, §11.10) | `██████████` 100% | ✅ Phase 1 complete (3/3 sketched steps) — no further phase yet scoped |

**Just closed:** M4 Phase 1 steps 2-3 — wiring the step 1 dispatch core to a real `winit` window and a real Python callback. Step 2 (`engine-platform`): `WindowEvent::CursorMoved`/`MouseInput`/`KeyboardInput`/`ModifiersChanged` translate into real `InputEvent`s via a new `on_input` closure on `run_windowed_multi`, matching `on_frame`/`build_access_update`'s own existing shape. Found and fixed a real doc-comment error from step 1: `winit::event::MouseButton` has six variants, not the three `PointerButton`'s own comment claimed unverified. Step 3 (`engine-py`): `Node.set_on_click(callback)` exists for real — `click_handlers: Rc<RefCell<HashMap<NodeId, Py<PyAny>>>>` shared between a `Window` and every `Node` it hands out (the same sharing shape `tree` already uses), visited by real `__traverse__`/`__clear__` (M3 step 15 Stage C's precedent, reused not reinvented); also adds `Action::Click` to a node's `access.actions`, so registering a click handler makes it Tab-reachable for free (§10). New `Window.click(node)` is a direct, programmatic click entry point (no live window needed) proving the whole path; an uncaught handler exception is caught via `PyErr::print`, matching §9's "caught, logged, non-fatal" policy. 4 new `engine-platform` unit tests + 6 new pytest tests (including a real GC-cycle-collection proof for `click_handlers`), all passed on the first run; both real windowed tests and both real example scripts still pass unchanged. See `PLAN.md`/`LOG.md`.

**Up next:** No further M4 phase is yet scoped in `ARCHITECTURE.md`. Real gaps this phase's own scope explicitly left open: keyboard-triggered activation (Tab then Enter/Space) has no Python-facing test entry point yet (only `Window.click()`, pointer-only); transform-aware hit-testing needs `PaintProperties.transform` (§11.9, unbuilt); custom hit-testing needs `NodeKind::Canvas` (unbuilt).
...
- Real drag-to-rearrange docking and real splitter-drag input still have no `winit`-driven UX wired (real dispatch now reaches `Tree::dispatch`, M4 Phase 1, but nothing calls `set_splitter_position`/`apply_active_tab` from a real drag gesture yet) — both remain exposed as direct, programmatic APIs.
...
- §10's minimal keyboard focus model (Tab/Shift-Tab traversal, Enter/Space activation) is real end-to-end now (`Tree::move_focus`/`dispatch`, real `winit` `KeyboardInput` translation, M4 Phase 1) but `ActionRequested`/`AccessibilityDeactivated` events from accesskit (a screen reader driving focus/activation directly) are still received and ignored — nothing routes those into `Tree::dispatch` yet, a real, narrower gap than before but not yet closed.
...
**Status: ✅ Phase 1 complete.** Not sequenced anywhere in `ARCHITECTURE.md` — picked up the single most consistently recurring finding from every M3 interaction-dependent step (7, 9, 11, 12, 13, 14, 15, each checked directly): real pointer/keyboard `InputEvent`/`AppHandler` dispatch and hit-testing didn't exist. Scope sketched in `PLAN.md` as three steps; all three are done. No Phase 2 yet scoped.
