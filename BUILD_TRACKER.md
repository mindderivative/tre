...
| M3 — v2 Implementation | `█████⬜⬜⬜⬜⬜` 50% | 🚧 In progress — Phases 1-3 of 7 complete, Phase 4 underway (1 of 2 steps) |

**Just closed:** M3 Phase 4 step 6 (§14 step 6) — `engine-py` minimal: `App` (node creation, the one blocking `run()`) and `Node` (`animate()`, the one property setter, real two-level dispatch per §8's review note) — the first `pyo3` code in this project. Verified pyo3's real current API directly before writing anything (resolved `0.29.2`) and found a real rename: `Python::with_gil`/`allow_threads` are now `Python::attach`/`Python::detach`. A second finding: `detach` requires `Ungil`, which requires `Send` on stable Rust — `Rc<RefCell<Tree>>` is deliberately `!Send` (§9), so §9's own "wrap render in `allow_threads`, costs nothing" habit isn't actually free here; skipped it, documented why, matching §9's own underlying reasoning rather than contradicting it. Built and ran for real: `maturin develop`, `python -c "import tre"`, a 6-test pytest suite against the compiled extension, and `examples/animate_rect.py` — two Python-created rects, three real `animate()` calls, a real 60-frame windowed render loop. Third finding, caught by testing the graceful-exit path specifically: `App.run()` only handled "no GPU" gracefully, not "no display" — fixed to match this workspace's finding-#261 convention everywhere else. CI extended per §13's own "Decision recorded" (primary-OS CI starts at step 6): `maturin develop` + `import tre` + pytest + the real demo script, added to the existing job. See `PLAN.md`/`LOG.md`.

**Up next:** M3 Phase 4 step 7 (§14 step 7) — wire `accesskit`: confirm one button is correctly exposed to a screen reader. ARCHITECTURE.md's own first-hand review note warns this crate has had real breaking API changes; pin an exact version and re-verify its current API directly, the same discipline just applied to pyo3.
...
- Everything in `ARCHITECTURE.md` citing "verify at implementation time" has started resolving, not finished: steps 1–6 confirmed `vello_hybrid`/`wgpu`/`kurbo`/`peniko`/`winit`/`taffy`/`slotmap`/`parley`/`glifo`/`fontique`/`serde_yaml_ng`/`pyo3`/`maturin` are all real and compile/parse/build together (with real version-coupling/perf/API findings along the way, each handled, not just logged). `accesskit` remains unverified until step 7; `material-colors` until step 11.
...
**Status: 🚧 In progress.** Phases 1–3 complete, Phase 4 underway (step 6 of 2 done); this mirrors §14's Suggested Build Order (all 15 steps now sequenced, including `engine-spec`/§16).
