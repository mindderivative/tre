...
| M4 — Real Input Dispatch (§4, §7.3, §9, §10, §11.10) | `███░░░░░░░` 33% (3/9 phases) | 🚧 Phases 1-3 complete, Phases 4-9 scoped 2026-09-15 |
...
- `RippleState`/`spawn_ripple`/the `SmallVec<[RippleState;4]>` completion-queue mechanism (§7.3) have existed and been unit-tested since M3 Phase 5 step 9, but confirmed via grep to have zero real call sites — `Tree::dispatch`'s `PointerPressed` arm updates hover but never spawns a ripple, so a real click never visibly ripples today. Scoped as M4 Phase 5.
...
### Phase 5 — Real Ripple Spawn on Press (§7.3) ⬜
- Step 1: `Tree::dispatch`'s `PointerPressed` arm calls `interaction_mut(node).spawn_ripple(origin, ...)` at the local press point for any node already opted into `InteractionState` — `RippleState`/`spawn_ripple`/the completion-queue removal exist and are unit-tested since M3 Phase 5 step 9 but (confirmed via grep) have zero real call sites outside tests today — ⬜
- Step 2: `engine-render` pixel-readback proof that a real `Tree::dispatch`-driven click actually grows a visible ripple that later disappears via the existing completion-queue mechanism — ⬜
