...
**Just closed:** M4 Phase 3 — real pointer-drag dispatch (§11.5, §11.4). `Tree` gained a `dragging: Option<NodeId>` field; the flanking-siblings geometry logic `set_splitter_position` already had was factored into a shared `splitter_geometry` helper, reused by a new `update_drag(point, now)` that converts a pointer coordinate into a 0.0..=1.0 fraction and calls `set_splitter_position` with it — §11.5's own "one mechanism, two call sites" text, made real. `dispatch`'s `PointerPressed`/`PointerMoved`/`PointerReleased` arms now start, live-update, and end a splitter drag, reusing the existing hit-test call, not a new one. Because docking's own zone-resize already reuses `set_splitter_position` verbatim (M3 step 15 Stage B), dragging a dock-zone boundary is real now too, with zero docking-specific code touched. 4 new `engine-core` unit tests (live multi-position tracking, release ending the drag, a non-splitter press never starting one, a non-primary-button press never starting one) + a new `engine-render` pixel-readback test driving the whole sequence through real `Tree::dispatch` (not a direct call), all passed on the first run. This phase touched only `engine-core` (mechanism) and added one `engine-render` test — no `engine-platform`/`engine-py` changes were needed, since `Tree::dispatch` is the exact function `App::run()`'s existing real `winit` wiring already calls. See `PLAN.md`/`LOG.md`.

**Up next:** No further M4 phase is yet scoped in `ARCHITECTURE.md`. The single most concrete, immediately-useful next step if M4 continues: `Window.add_splitter` in `engine-py` — the drag mechanism is real end to end, but no Python API creates a `NodeKind::Splitter` yet, so a Python app author can't actually build a resizable-pane UI today. Larger, later-milestone-shaped gaps: §11.9 transform composition (`PaintProperties.transform`, blocking transform-aware hit-testing), a real scrollable viewport for `VirtualList` (§11.8), and `NodeKind::Canvas` (blocking custom hit-testing, §11.10's own second half).
...
- ~~Real splitter-drag input has no `winit`-driven UX wired.~~ **Fixed (M4 Phase 3)** — `Tree::dispatch` now starts/tracks/ends a real splitter drag from real pointer events. Still open: no Python API creates a `NodeKind::Splitter` (see "Up next"), and "drag-to-rearrange docking" (moving a whole panel between zones, not resizing) remains a separate, untouched, larger feature.
