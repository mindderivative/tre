...
**Just closed:** M3 Phase 7 step 15 (§14 step 15) — **M3's final step**: docking (§11.4) + virtualization (§11.7), plus their shared splitters prerequisite (§11.5), across three separately-committed stages. Stage A built `NodeKind::Splitter`/`Tree::set_splitter_position` (an instant, tick-applied drag along the parent's flex axis), proven via a headless test that a real pane boundary moves on screen. Stage B built `DockLayout`/`DockZone` and `Tree::apply_active_tab` (tabbed grouping via `Tree::detach`, not `Display::None`, so `build_tree_scene`'s existing children-walk excludes a hidden panel from paint with zero changes) — resizing between zones reuses Stage A's splitter mechanism verbatim, proven via a 3-zone docked-screen pixel test; found and fixed a real bug where the tab rects didn't stretch to fill a resized zone. Stage C built `NodeKind::VirtualList`/`Tree::set_virtual_list_window` (only the visible window is ever real `Node`s regardless of `item_count`, recycled via `slotmap`'s own generational `NodeId` invalidation — proven both at the `Tree` level and via pixel-readback that a recycled item genuinely stops painting) and `engine-py`'s new "materialize item N" FFI callback — the first place in this codebase to actually store a long-lived `PyObject` callback, so `PyWindow` now implements real `__traverse__`/`__clear__` cyclic-GC support (pyo3 0.29.2 needs no `#[pyclass(gc)]` flag, verified directly against its own test suite), proven by a real reference-cycle-collection pytest. The Risk Register's own named acceptance gate for this step — real GIL-overhead measurement against 100,000+ rows — is done: **~3.0us/call measured**, comfortably inside budget; batching the callback isn't needed. See `PLAN.md`/`LOG.md`.

**Up next:** No M4 is yet planned or sequenced in `ARCHITECTURE.md`. The most consistently recurring gap across every interaction-dependent M3 step (7, 9, 11, 12, 13, 14, 15) — real pointer/keyboard `InputEvent`/`AppHandler` dispatch and hit-testing (§11.10) — is the natural starting point for whatever comes next.
...
- Real drag-to-rearrange docking and real splitter-drag input both need §11.10 pointer dispatch (see "Up next") — `Tree::set_splitter_position`/`Tree::apply_active_tab` are exposed as direct, programmatic APIs in the meantime, matching every other interaction-dependent step's own precedent.
